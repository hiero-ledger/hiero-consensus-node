...
public class SortedDiffExporter {
    ...
    public SortedDiffExporter(
        ...
        serviceNameStateKeyList.forEach(p -> {
            ...
            state1DiffByStateId.computeIfAbsent(stateId, k -> new ConcurrentSkipListSet<>(comparator));
            state2DiffByStateId.computeIfAbsent(stateId, k -> new ConcurrentSkipListSet<>(comparator));
