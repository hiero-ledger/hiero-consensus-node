...
1. Download or build the State Operator jar file:
To download:
```
gcloud storage cp gs://hedera-ci-ephemeral-artifacts/hedera/hedera-state-validator/hedera-state-validator-0.77.jar .
```
To build:
In workspace `hiero-consensus-node`, run:
```shell
./gradlew assemble
cd hedera-state-validator
../gradlew shadowJar
````
2. Specify script parameters. You can either set them as environment variables or update the script.
```shell
