...
|---|---:|---|---|
| Preflight result | fatal | `NETWORK_DISEASE_FATAL`. This artifact must not be used for calibration and should be re-run. Existing extracted timing, counter, network, workload, and state rows below are retained as diagnostic context only. | `derived:formula=fatal_when_post_startup_active_to_checking_is_corroborated_by_missing_non_expired_other_parent;inputs=log:podlog_solo-mdlt-n4/network-node7_logs/swirlds.log:184-185,log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:144-150` |
| `ACTIVE -> CHECKING` platform churn | present | Post-startup `ACTIVE -> CHECKING` churn is present. Count by node log: `network-node7_logs=127`, `network-node1_logs=1`. Node7 begins repeated churn at `2026-05-29 21:12:33.231`, before the learner reconnect window, and continues through the first reconnect window. | `log:podlog_solo-mdlt-n4/network-node7_logs/swirlds.log:184-191`, `log:podlog_solo-mdlt-n4/network-node7_logs/swirlds.log:680-706`, `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:441-448`, `derived:formula=rg_count_oldStatus_ACTIVE_newStatus_CHECKING_by_node_log` |
| `CHECKING -> ACTIVE` churn context | present | Matching `CHECKING -> ACTIVE` churn is present. Count by node log: `network-node7_logs=127`, `network-node1_logs=2`, and `network-node2_logs` through `network-node6_logs` each have one startup `CHECKING -> ACTIVE` transition. | `log:podlog_solo-mdlt-n4/network-node7_logs/swirlds.log:184-191`, `derived:formula=rg_count_oldStatus_CHECKING_newStatus_ACTIVE_by_node_log` |
| Missing non-expired other parent | present | Missing-parent evidence is widespread. Count by node log: `network-node1_logs=53`, `network-node2_logs=227`, `network-node3_logs=223`, `network-node4_logs=234`, `network-node5_logs=233`, `network-node6_logs=229`. | `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:144-150`, `derived:formula=rg_count_Missing_non_expired_other_parent_by_node_log` |
| Extraction disposition | fatal | Stop normal artifact processing for future runs with this preflight result. For this already-extracted artifact, keep the rows below diagnostic-only and exclude them from calibration summaries, traversal ordering, and local `ReconnectBench` parameter selection. | `protocol:../../cluster-reconnectbench-artifact-processing-protocol.md#network-disease-preflight` |
...
| Stopped pod | inferred | `network-node1-0`. No exact `stoppedPod=` script line was emitted, but the manifest records intended learner `network-node1-0` / node `0`; workflow `Stopping java` / `Down for downtime=1800` is followed by reconnect-loop sampling on `network-node1-0`; `network-node1_logs` maps to node ID `0`; learner `settingsUsed.txt` records `HOSTNAME, network-node1-0`; and the first receiver reconnect is logged by node `0`. | `manifest:../../cluster-reconnectbench-artifact-manifest.md#2026-05-29-cluster-calibration;run=parallel-sync`, `workflow:../performance-tests-start.log:7170-7173`, `sampler:reconnect_network_samples_1.log:1-2;window=run_sampler`, `config:podlog_solo-mdlt-n4/network-node1_logs/config/settingsUsed.txt:950`, `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:213` |
...
| Evidence item | Status | Extracted value or observation | Source references |
|---|---:|---|---|
| Episode complete | present | Learner reached `ACTIVE` after the second receiver reconnect finish, confirming the catch-up episode completed. This remains diagnostic-only because the artifact failed network-disease preflight. | `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:320`, `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:434`, [Network Disease Preflight](#network-disease-preflight) |
| Iteration count | derived | `2` learner receiver reconnect iterations before first `ACTIVE` after the final reconnect. | `derived:formula=count_receiver_ReconnectStartPayload_before_ACTIVE;inputs=log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:213,log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:285,log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:434` |
| Complete catch-up start | present | `2026-05-29 22:50:04.496` UTC. | `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:213` |
| Complete catch-up end | present | `2026-05-29 22:59:43.644` UTC, the final learner receiver reconnect finish before first post-reconnect `ACTIVE`. | `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:320`, `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:434` |
| Complete catch-up duration | derived | `579.148 s`, diagnostic only. | `derived:formula=final_receiver_finish-first_receiver_start;inputs=log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:213,log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:320`, [Network Disease Preflight](#network-disease-preflight) |
| Active confirmation | present | First post-final-reconnect `ACTIVE` at `2026-05-29 23:03:54.174` UTC. Later `ACTIVE -> CHECKING -> ACTIVE` churn at `23:04:04.192` and `23:04:57.946` is part of the fatal network-disease context. | `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:434`, `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:441`, `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:448`, [Network Disease Preflight](#network-disease-preflight) |
| Iteration 1 | present | Receiver node `0`, teacher peer `1`, start `2026-05-29 22:50:04.496` UTC, finish `2026-05-29 22:56:15.846` UTC, duration `371.350 s`. | `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:213`, `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:240`, `derived:formula=end_timestamp-start_timestamp` |
| Iteration 2 | present | Receiver node `0`, teacher peer `4`, start `2026-05-29 22:56:19.941` UTC, finish `2026-05-29 22:59:43.644` UTC, duration `203.703 s`, synchronization stage `199.133 s`. | `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:285-320`, `derived:formula=end_timestamp-start_timestamp;inputs=log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:285,log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:320` |
...
| Additional iterations observed | present | One additional learner receiver reconnect was observed before first post-final-reconnect `ACTIVE`. | `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:213-240`, `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:285-320`, `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:434` |
...
| Manifest run | present | `parallel-sync` | `manifest:../../cluster-reconnectbench-artifact-manifest.md#2026-05-29-cluster-calibration;run=parallel-sync` |
| Commit | present | `eb37e5b6cd4d4388065f79ed4a9d91867bd92cc2` | `config:version_run.txt:key=hederahash;line=11` |
| Network disease preflight | fatal | `NETWORK_DISEASE_FATAL`; do not use this run for calibration. | [Network Disease Preflight](#network-disease-preflight) |
| Network disease reason if failed | present | Post-startup `ACTIVE -> CHECKING` churn is corroborated by widespread `Shadowgraph: Missing non-expired other parent` evidence. | [Network Disease Preflight](#network-disease-preflight) |
...
| Episode complete | present | yes, diagnostic only | [Reconnect Episodes And Iterations](#reconnect-episodes-and-iterations), [Network Disease Preflight](#network-disease-preflight) |
| Iteration count | derived | `2`, diagnostic only | [Reconnect Episodes And Iterations](#reconnect-episodes-and-iterations), [Network Disease Preflight](#network-disease-preflight) |
| Complete catch-up start UTC | present | `2026-05-29 22:50:04.496`, diagnostic only | [Reconnect Episodes And Iterations](#reconnect-episodes-and-iterations) |
| Complete catch-up end UTC | present | `2026-05-29 22:59:43.644`, diagnostic only | [Reconnect Episodes And Iterations](#reconnect-episodes-and-iterations) |
| Complete catch-up duration | derived | `579.148 s`, diagnostic only | [Reconnect Episodes And Iterations](#reconnect-episodes-and-iterations) |
| Active confirmation UTC | present | `2026-05-29 23:03:54.174`, diagnostic only | [Reconnect Episodes And Iterations](#reconnect-episodes-and-iterations) |
| Additional iterations observed | present | second learner receiver reconnect observed before `ACTIVE`, diagnostic only. | [Reconnect Episodes And Iterations](#reconnect-episodes-and-iterations) |
| Run accepted for calibration | derived | no | `derived:formula=network_disease_preflight_is_fatal;inputs=log:podlog_solo-mdlt-n4/network-node7_logs/swirlds.log:184-191,log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:144-150` |
| Reason if not accepted | derived | `NETWORK_DISEASE_FATAL`; post-startup `ACTIVE -> CHECKING` churn is corroborated by widespread missing-parent evidence. The earlier passive TCP/window coverage gap remains diagnostic context but is no longer the primary rejection reason. | [Network Disease Preflight](#network-disease-preflight) |

## Acceptance Notes

| Acceptance item | Status | Note | Source references |
|---|---:|---|---|
| Timing and work-shape calibration | diagnostic_only | First reconnect timing, roles, counters, state sizes, and workload evidence are source-referenced, but the artifact is invalidated by network disease and must not be used for calibration. | `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:213-240`, `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:237-239`, `log:client.log:2-9`, [Network Disease Preflight](#network-disease-preflight) |
| Calibration validity | fatal | The artifact is invalidated by network disease and must not be used for calibration, traversal ordering, or local benchmark parameter selection. The timing and work-shape rows remain diagnostic-only. | [Network Disease Preflight](#network-disease-preflight) |
| Independent-run interpretation | diagnostic_only | Workflow logs confirm nominal reconnect controls and skipped restored-state upload. These facts remain useful for diagnostics, but the run is not a calibration anchor because the preflight is fatal. | `workflow:../performance-tests-start.log:4791-4792`, `workflow:../performance-tests-start.log:6823-6828`, [Network Disease Preflight](#network-disease-preflight) |
| Reconnect episode handling | diagnostic_only | Two learner receiver reconnect iterations are documented, but the entire artifact is diagnostic-only after fatal preflight. | [Reconnect Episodes And Iterations](#reconnect-episodes-and-iterations), [Network Disease Preflight](#network-disease-preflight) |
| Network-inflight calibration | diagnostic_only | Stats throughput, ping, and attributed passive TCP/window evidence were extracted, but the run is invalidated by network disease. The earlier partial end-window sampler gap remains diagnostic only. | `derived:formula=avg_and_max_over_bounded_window;inputs=csv:podlog_solo-mdlt-n4/network-node2_logs/stats/MainNetStats1.csv:column=bytes_per_sec_sent_00;rows=2998-3122`, `sampler:network_sampler_network-node2-0.log:17455,17615-17616;window=2026-05-29T22:50:04Z..2026-05-29T22:56:16Z`, `sampler:network_sampler_network-node2-0.log:30507,30663-30664;window=latest_teacher_before_completion`, [Network Disease Preflight](#network-disease-preflight) |
