...
| Exact image tag or digest | missing | Version inputs are present, but no exact node image tag or digest was found in the documented run context files. | Search scope: `version_run.txt`, `client.log`, `podlog_solo-mdlt-n3/network-node1_logs/config/settingsUsed.txt`; patterns: `image`, `tag`, `digest`; reason: no authoritative image key found. |
| Baseline restore identifier | missing | No baseline restore identifier was found in the documented run context files. | Search scope: `version_run.txt`, `client.log`, `podlog_solo-mdlt-n3/network-node1_logs/config/settingsUsed.txt`; patterns: `baseline`, `restore`, `saved state`; reason: no authoritative baseline key found. |
...
| Stopped pod source | missing | The expected learner pod is inferred from the learner log directory, but the exact stopped-pod script output was not found. | Search scope: `version_run.txt`, `client.log`, top-level `reconnect_network_samples_1.log`, `reconnect_network_samples_1_summary.log`; patterns: `stopped`, `delete`, `network-node1-0`; reason: no ordinary script line identifying the stopped pod. |
| Controlled warmtime, downtime, and loop count | missing | No controlled `warmtime`, `downtime`, or `NofLoops` values were found. | Search scope: `version_run.txt`, `client.log`, `podlog_solo-mdlt-n3/network-node1_logs/config/settingsUsed.txt`; patterns: `warmtime`, `downtime`, `NofLoops`, `loop`; reason: no authoritative control keys found. |
...
| learnerBehindDuration | missing | No controlled `learnerBehindDuration` value was found. Files checked: `version_run.txt`, `client.log`, `reconnect_network_samples_1.log`, `reconnect_network_samples_1_summary.log`, learner `settingsUsed.txt`; patterns: `warmtime`, `downtime`, `NofLoops`, `learnerBehindDuration`, `behind duration`, `loop`; reason: no authoritative control key found. Observed log-only platform `BEHIND` duration was `3.1 m`; first reconnect started at `06:41:57.343`. | `log:podlog_solo-mdlt-n3/network-node1_logs/swirlds.log:160-208`, `log:podlog_solo-mdlt-n3/network-node1_logs/swirlds.log:164-164` |
...
|---|---:|---|---|---|
| Exact image tag or digest | missing | `version_run.txt`, `client.log`, `podlog_solo-mdlt-n3/network-node1_logs/config/settingsUsed.txt` | `image`, `tag`, `digest` | Version inputs exist, but no authoritative image tag/digest key was found. |
...
| Exact stopped pod script output | missing | `version_run.txt`, `client.log`, top-level sampler files | `stopped`, `delete`, `network-node1-0` | Learner node is known from logs, but no script line records the stopped pod. |
...
| learnerBehindDuration controlled/script value | missing | `version_run.txt`, `client.log`, reconnect sample logs, learner `settingsUsed.txt` | `learnerBehindDuration`, `behind duration`, `warmtime`, `downtime`, `NofLoops` | Observed BEHIND duration exists in logs, but no controlled learner-behind duration key was found. |
