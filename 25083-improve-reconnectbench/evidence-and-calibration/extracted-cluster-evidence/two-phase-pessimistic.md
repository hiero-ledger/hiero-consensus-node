...
| Stopped pod source | ambiguous | Workflow output has a generic `Stopping java` marker, but no exact stopped-pod script line identifies the learner pod. | `workflow:../performance-tests-watch.log:1842-1843`; search scope: `version_run.txt`, `client.log`, top-level `reconnect_network_samples_1.log`, `../performance-tests-watch.log`; patterns: `stopped`, `delete`, `network-node1-0`, `Stopping java`; reason: generic stop marker is present but exact pod identity is not. |
...
| learnerBehindDuration | missing | No controlled/script `learnerBehindDuration` value was found. The workflow has `downtime=1800`, but that is the scripted Java-down interval, not the observed platform `BEHIND` duration. Observed learner log duration is `2.9 m` in `BEHIND`, from `18:26:08.575` to `18:29:05.078`, but this is runtime evidence, not a controlled input. | `workflow:../performance-tests-watch.log:1494-1499`, `workflow:../performance-tests-watch.log:1842-1843`, `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:193-241` |
...
|---|---:|---|---|---|
| Exact stopped pod script output | ambiguous | `version_run.txt`, `client.log`, top-level sampler file, `../performance-tests-watch.log` | `stopped`, `delete`, `network-node1-0`, `Stopping java` | Learner node is known from logs and a generic stop marker exists, but no script line records the exact stopped pod. |
