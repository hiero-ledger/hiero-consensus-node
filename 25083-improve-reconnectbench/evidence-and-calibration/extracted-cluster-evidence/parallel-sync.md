...
| Stopped pod source | ambiguous | Workflow output has a generic `Stopping java` marker, and the top-level sampler output records a sampler stop marker, but no exact stopped-pod script line identifies the learner pod. | `workflow:../performance-tests-start.log:7170-7171`, `sampler:reconnect_network_samples_1.log:1-2;window=run_sampler`; search scope: `version_run.txt`, `client.log`, top-level sampler file, `../performance-tests-start.log`; patterns: `stopped`, `delete`, `network-node1-0`, `Stopping java`; reason: generic stop marker is present but exact pod identity is not. |
...
| learnerBehindDuration | missing | No controlled learner-behind duration was found. The workflow has `downtime=1800`, but that is the scripted Java-down interval, not the observed platform `BEHIND` duration. Observed first learner BEHIND interval was logged as `6.2 m`, from `22:50:04.408` BEHIND to `22:56:15.927` RECONNECT_COMPLETE, approximately `371.519 s`. | `workflow:../performance-tests-start.log:6823-6828`, `workflow:../performance-tests-start.log:7170-7171`, `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:206-206`, `log:podlog_solo-mdlt-n4/network-node1_logs/swirlds.log:257-257` |
...
|---|---:|---|---|---|
| Exact stopped pod script output | ambiguous | `version_run.txt`, `client.log`, top-level sampler file, `../performance-tests-start.log` | `stopped`, `delete`, `network-node1-0`, `Stopping java` | Sampler stop marker and a generic stop marker exist, but no script line records the exact stopped learner pod control. |
...
| learnerBehindDuration controlled/script value | missing | `version_run.txt`, `client.log`, `reconnect_network_samples_1.log`, learner `settingsUsed.txt`, `../performance-tests-start.log` | `learnerBehindDuration`, `behind duration`, `warmtime`, `downtime`, `NofLoops` | `downtime`, `warmtime`, and `NofLoops` are now known, but no controlled learner-behind duration key was found. |
