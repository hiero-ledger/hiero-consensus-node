...
Unless explicitly prefixed `network-node`, every node number in this report is an internal node ID.
...
| Receiver wall-duration aggregate | derived | `341` values; sum `119,230.759 s`, mean `349.650320 s`, minimum `219.739 s` at iteration `4`, maximum `1,053.714 s` at iteration `1`. | `derived:finish-start_per_iteration;input=log:podlog_solo-mdlt-n12/network-node1_logs/swirlds.log:181-34756;count=341;extrema=log:.../swirlds.log:181-206,452-484` |
...
| Unexpected buffer values | derived | None observed; every exact lifecycle value belongs to the expected sets: server receive `32768`, client PRE send/receive `32768`, client POST send `43520`, and client POST receive `32768`. | `derived:exact_value_group;scope=all SocketFactory lifecycle lines in seven logs` |
| Socket marker warnings | present | All telemetry lines use `WARN SOCKET_EXCEPTIONS`; separately, the marker contains 686 `NetworkUtils: Connection broken` warnings and two internal-node `2` / `network-node3-0` `SSLPeerUnverifiedException` warnings. There are no marker-level `ERROR` lines. | first/last internal-node `0` warnings `log:podlog_solo-mdlt-n12/network-node1_logs/swirlds.log:321,34794`; SSL examples `log:podlog_solo-mdlt-n12/network-node3_logs/swirlds.log:6467,8040`; `derived:exact_marker_count;scope=seven logs` |
...
| Endpoint mapping | present | Internal node IDs `0..6` map respectively to `network-node1-0` / `10.36.41.165`, `network-node2-0` / `10.36.58.95`, `network-node3-0` / `10.36.59.218`, `network-node4-0` / `10.36.61.57`, `network-node5-0` / `10.36.62.105`, `network-node6-0` / `10.36.47.167`, and `network-node7-0` / `10.36.71.156`. | `config:podlog_solo-mdlt-n12/network-node{1..7}_logs/config/settingsUsed.txt:lines=946,1141` |
...
| Missing full-window set | missing | Iterations `304`, `314`, `317`, `318`, and `320..342`. Iterations `304`, `314`, and `317` lack internal-node `3` / `network-node4-0` teacher coverage; iteration `318` lacks internal-node `2` / `network-node3-0` teacher coverage; later gaps include learner coverage. | `derived:join_iteration_roles_and_sampler_bounds;inputs=ordered iteration table,sampler table above;missing=304,314,317,318,320..342` |
| Iteration 341 | missing | Both the learner internal-node `0` / `network-node1-0` sampler and teacher internal-node `2` / `network-node3-0` sampler end before `01:49:37..01:55:58`. | `sampler:network-node1-0_network_sampler.log:829185`; `sampler:network-node3-0_network_sampler.log:822581` |
| Iteration 342 | missing | The learner internal-node `0` / `network-node1-0` and teacher internal-node `4` / `network-node5-0` samplers end before `01:56:08..01:57:22`. Internal-node `5` / `network-node6-0` is the only sampler spanning learner-log EOF, but it is not iteration 342's teacher. | `sampler:network-node1-0_network_sampler.log:829185`; `sampler:network-node5-0_network_sampler.log:820363`; `sampler:network-node6-0_network_sampler.log:900343` |
...
in one sample. All 315 fully covered windows were analyzed mechanically; the durable report retains aggregate results
and three source-anchored windows rather than exhaustive per-window socket and rate tables.

#### Socket-buffer comparison — lifecycle getters versus live kernel caps

The producing commit does not call `setReceiveBufferSize()` or `setSendBufferSize()` in the production gossip path.
It calls Java buffer getters around bind/connect and explicitly leaves the setters unused. Therefore `32768` and
`43520` are observed OS/JVM-selected lifecycle values, not sizes requested by this `SocketFactory` implementation.
The server value is also the listener's proposed receive value; the accepted socket's Java getter was not logged.

| Java/lifecycle observation | Closest `ss -tinm` field | What the sampler shows | Interpretation |
|---|---|---|---|
| Client receive POST CONNECT `32768` | learner `rb` | First attributed sample is `65536` in 242/315 windows and already larger in 73/315; per-window `rb` maxima have median `2,315,584`, nearest-rank p95 `10,814,016`, and maximum `30,648,664` at iteration 248. | Initial `ss` baseline is exactly `2×` the Java value. The maximum is `935.3×` raw, or `467.7×` after normalizing the kernel's `2×` accounting convention; the growth is consistent with receive autotuning. |
| Client send POST CONNECT `43520` | learner `tb` | First attributed sample is `87040` in 314/315 windows; iteration 5 is already at `3,037,696` when first sampled. Per-window `tb` maxima have median `1,549,312`, p95 `2,550,272`, and maximum `6,162,432` at iteration 83. | In 314 windows the first `ss` baseline is exactly `2×` the Java value; the iteration 5 sampling lag captures an already-grown cap. The maximum is `141.6×` raw, or `70.8×` after normalization; the growth is consistent with send autotuning. |
| Server listener receive POST BIND `32768` | teacher accepted-socket `rb` | Per-window `rb` maxima have median `4,120,904`, nearest-rank p95 `11,169,112`, and maximum `29,592,328` at iteration 161. | This is context, not a direct delta: the lifecycle getter is on the listener while `ss` observes the accepted established socket. |
| No accepted-socket send getter | teacher `tb` | Per-window `tb` maxima have median `974,848`, p95 `1,531,904`, and maximum `2,402,304` at iteration 73. | No SocketFactory value exists for a direct teacher-send comparison. |

The apparent mismatch is expected. Linux exposes `ss` `rb`/`tb` using kernel socket-memory accounting, including the
documented doubling for socket-buffer bookkeeping, while the Java lifecycle logs report the API-facing values. The
sampler then observes established sockets during bulk transfer, after TCP has been able to autotune the caps. `rb` and
`tb` are the total receive/send memory that may be allocated at that sample; `r`, `t`, and `w` are memory currently
allocated or queued, and `Recv-Q`/`Send-Q` are queue occupancy. The latter fields must not be compared as buffer sizes.

The artifact does not preserve `tcp_rmem`, `tcp_wmem`, `rmem_max`, or `wmem_max`, so it cannot show the host's exact
autotuning ceilings or whether any observed cap reached them. Semantics references: [Java 25 `Socket`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/net/Socket.html), [Java 25 `ServerSocket`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/net/ServerSocket.html), [Linux `socket(7)`](https://man7.org/linux/man-pages/man7/socket.7.html), [Linux `ss(8)`](https://man7.org/linux/man-pages/man8/ss.8.html), and [Linux TCP sysctls](https://docs.kernel.org/networking/ip-sysctl.html).

| Evidence item | Status | Extracted value or observation | Source references |
|---|---:|---|---|
| Producing SocketFactory behavior | present | Getter-only telemetry; no production gossip `setReceiveBufferSize()` or `setSendBufferSize()` call at the producing commit. | `source@09f7ef40e031fc3e1a06db6f7db5e7dcfe9abc73:platform-sdk/consensus-gossip-impl/src/main/java/org/hiero/consensus/gossip/impl/network/connectivity/SocketFactory.java:83-93,123-141`; commit-scoped source search |
| First learner cap comparison | derived | `rb=65536` in 242/315 first attributed samples and already larger in 73/315; `tb=87040` in 314/315 and already `3,037,696` in iteration 5. | `derived:first_attributed_reciprocal_sample;inputs=315 fully covered windows`; baseline example `sampler:network-node1-0_network_sampler.log:637-638`; grown first sample `:14415-14416` |
| Learner cap extrema | derived | Maximum `rb=30,648,664` at iteration 248; maximum `tb=6,162,432` at iteration 83. | `sampler:network-node1-0_network_sampler.log:621193-621194,189015-189016`; `derived:per-window component maxima;windows=315` |
| Teacher cap extrema | derived | Maximum `rb=29,592,328` at iteration 161; maximum `tb=2,402,304` at iteration 73. | `sampler:network-node4-0_network_sampler.log:383769-383770,167439-167440`; `derived:per-window component maxima;windows=315` |
| Host autotuning sysctls | missing | Exact TCP/core buffer defaults and maxima are not preserved. | files checked: complete artifact; patterns: `tcp_rmem`, `tcp_wmem`, `rmem_max`, `wmem_max`, `tcp_moderate_rcvbuf` |
...
| RTT / `minrtt` ranges | derived | `0.069..9.043 ms / 0.039..0.076 ms` | `0.070..5.460 ms / 0.039..0.061 ms` | learner `sampler:network-node1-0_network_sampler.log:638,3872,4936,6700`; teacher `sampler:network-node2-0_network_sampler.log:3816,4852,7904` |
...
| Socket-rate ranges | derived | `send 34.76 Mb/s..51.35 Gb/s`; pacing `69.50 Mb/s..61.10 Gb/s`; delivery `1.78 Mb/s..7.34 Gb/s` | `send 21.22 Mb/s..22.57 Gb/s`; pacing `42.43 Mb/s..26.88 Gb/s`; delivery `77.74 Mb/s..7.64 Gb/s` | learner extrema `sampler:network-node1-0_network_sampler.log:638,2458,6448,7162`; teacher `sampler:network-node2-0_network_sampler.log:3816,5286,11278,11292`; rate context only, not capacity |
...
| RTT / `minrtt` ranges | derived | `0.080..4.018 ms / 0.039..0.041 ms` | `0.077..5.503 ms / 0.039..0.041 ms` | learner `sampler:network-node1-0_network_sampler.log:403448,403854,403980`; teacher `sampler:network-node3-0_network_sampler.log:407012,407348,407950,408524` |
...
| `rwnd_limited` | derived | maximum `3,604 ms / 1.8%` | maximum `368 ms / 1.1%` | learner `sampler:network-node1-0_network_sampler.log:404946`; teacher `sampler:network-node3-0_network_sampler.log:408510` |
| Socket-rate ranges | derived | `send 28.83 Mb/s..17.24 Gb/s`; pacing `57.65 Mb/s..20.69 Gb/s`; delivery `210.62 Mb/s..4.04 Gb/s` | `send 21.05 Mb/s..6.32 Gb/s`; pacing `42.10 Mb/s..7.52 Gb/s`; delivery `307.54 Mb/s..1.59 Gb/s` | learner extrema `sampler:network-node1-0_network_sampler.log:403448,404512,404890`; teacher `sampler:network-node3-0_network_sampler.log:407012,407418,407950,408048,408524`; rate context only, not capacity |
...
| RTT / `minrtt` ranges | derived | `0.079..2.866 ms / 0.038..0.041 ms` | `0.078..5.614 ms / 0.039 ms throughout` | learner `sampler:network-node1-0_network_sampler.log:824608,825070,825406`; teacher `sampler:network-node2-0_network_sampler.log:828146,829756,830246` |
...
| `rwnd_limited` | derived | maximum `16,080 ms / 0.6%` | maximum `1,652 ms / 1.9%` | learner `sampler:network-node1-0_network_sampler.log:826680,826694`; teacher `sampler:network-node2-0_network_sampler.log:829700,830232` |
| Socket-rate maxima | derived | `send/pacing/delivery 28.17/33.70/4.42 Gb/s` | `10.25/12.16/2.70 Gb/s` | learner extrema `sampler:network-node1-0_network_sampler.log:826400,826554`; teacher `sampler:network-node2-0_network_sampler.log:829686,829756`; rate context only, not capacity |
...
| Iteration / endpoint | Status | Tuple samples | `rcv_space` | `snd_wnd` | `unacked` | `notsent` | `ssthresh` | `rwnd_limited` |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 learner | present | 548 | 548, throughout | 489, partial | 223, partial | 179, partial | 526, partial | 526, partial |
| 1 teacher | present | 548 | 548, throughout | 511, partial | 208, partial | 58, partial | 526, partial | 526, partial |
| 170 learner | present | 140 | 140, throughout | 140, throughout | 43, partial | 2, partial | 118, partial | 112, partial |
| 170 teacher | present | 140 | 140, throughout | 140, throughout | 17, partial | 0, absent | 118, partial | 33, partial |
| 319 learner | present | 183 | 183, throughout | 178, partial | 51, partial | 7, partial | 155, partial | 155, partial |
| 319 teacher | present | 183 | 183, throughout | 183, throughout | 22, partial | 1, partial | 155, partial | 133, partial |
...
| Growth-positive divergence | derived | Every sent target is larger than that attempt's learner state, and completed attempts contain clean and non-clean work. | path scan above; [Reconnect Work-Shape Counters](#reconnect-work-shape-counters) |
| Exact mutation composition | ambiguous | Append/modify/remove composition cannot be distinguished because dirty counters do not identify mutation cause. | path scan and counter rows above |
...
| Iteration | Teacher internal node | Receiver start UTC | Receiver result | Receiver finish UTC | Receiver duration | Synchronization payload |
|---:|---:|---|---|---|---:|---:|
| 1 | 1 | `2026-07-14 15:54:56.075` (`:181`) | finished | `2026-07-14 16:12:29.789` (`:206`) | 1053.714 s | 1034.318 s |
| 2 | 2 | `2026-07-14 16:12:36.244` (`:253`) | finished | `2026-07-14 16:17:55.302` (`:285`) | 319.058 s | 300.122 s |
| 3 | 3 | `2026-07-14 16:18:06.210` (`:348`) | finished | `2026-07-14 16:22:21.940` (`:380`) | 255.730 s | 236.473 s |
| 4 | 4 | `2026-07-14 16:22:30.804` (`:452`) | finished | `2026-07-14 16:26:10.543` (`:484`) | 219.739 s | 200.229 s |
| 5 | 1 | `2026-07-14 16:26:19.347` (`:556`) | finished | `2026-07-14 16:30:45.905` (`:588`) | 266.558 s | 246.666 s |
| 6 | 3 | `2026-07-14 16:30:56.113` (`:660`) | finished | `2026-07-14 16:34:48.508` (`:692`) | 232.395 s | 214.249 s |
| 7 | 5 | `2026-07-14 16:34:58.128` (`:764`) | finished | `2026-07-14 16:39:20.788` (`:793`) | 262.660 s | 244.139 s |
| 8 | 6 | `2026-07-14 16:39:29.104` (`:865`) | finished | `2026-07-14 16:43:23.991` (`:894`) | 234.887 s | 215.419 s |
| 9 | 3 | `2026-07-14 16:43:34.442` (`:966`) | finished | `2026-07-14 16:47:39.081` (`:995`) | 244.639 s | 230.746 s |
| 10 | 5 | `2026-07-14 16:47:48.985` (`:1067`) | finished | `2026-07-14 16:52:14.883` (`:1096`) | 265.898 s | 247.331 s |
| 11 | 6 | `2026-07-14 16:52:24.655` (`:1168`) | finished | `2026-07-14 16:56:54.578` (`:1197`) | 269.923 s | 251.185 s |
| 12 | 4 | `2026-07-14 16:57:05.324` (`:1269`) | finished | `2026-07-14 17:01:23.271` (`:1298`) | 257.947 s | 239.279 s |
| 13 | 1 | `2026-07-14 17:01:32.342` (`:1370`) | finished | `2026-07-14 17:05:55.148` (`:1399`) | 262.806 s | 246.036 s |
| 14 | 6 | `2026-07-14 17:06:05.801` (`:1471`) | finished | `2026-07-14 17:10:32.555` (`:1500`) | 266.754 s | 248.545 s |
| 15 | 5 | `2026-07-14 17:10:41.966` (`:1572`) | finished | `2026-07-14 17:15:13.088` (`:1601`) | 271.122 s | 253.185 s |
| 16 | 1 | `2026-07-14 17:15:22.756` (`:1673`) | finished | `2026-07-14 17:19:44.906` (`:1702`) | 262.150 s | 246.710 s |
| 17 | 6 | `2026-07-14 17:19:55.223` (`:1774`) | finished | `2026-07-14 17:24:25.044` (`:1803`) | 269.821 s | 250.392 s |
| 18 | 3 | `2026-07-14 17:24:35.112` (`:1875`) | finished | `2026-07-14 17:29:06.349` (`:1904`) | 271.237 s | 257.071 s |
| 19 | 1 | `2026-07-14 17:29:15.423` (`:1976`) | finished | `2026-07-14 17:33:48.319` (`:2005`) | 272.896 s | 257.126 s |
| 20 | 4 | `2026-07-14 17:33:57.839` (`:2077`) | finished | `2026-07-14 17:38:27.830` (`:2106`) | 269.991 s | 251.620 s |
| 21 | 6 | `2026-07-14 17:38:37.464` (`:2178`) | finished | `2026-07-14 17:43:08.744` (`:2207`) | 271.280 s | 252.962 s |
| 22 | 5 | `2026-07-14 17:43:18.467` (`:2279`) | finished | `2026-07-14 17:47:57.945` (`:2308`) | 279.478 s | 260.151 s |
| 23 | 3 | `2026-07-14 17:48:08.270` (`:2380`) | finished | `2026-07-14 17:52:47.040` (`:2409`) | 278.770 s | 263.813 s |
| 24 | 4 | `2026-07-14 17:52:57.379` (`:2481`) | finished | `2026-07-14 17:57:34.579` (`:2510`) | 277.200 s | 258.546 s |
| 25 | 1 | `2026-07-14 17:57:44.165` (`:2582`) | finished | `2026-07-14 18:02:19.882` (`:2611`) | 275.717 s | 259.334 s |
| 26 | 2 | `2026-07-14 18:02:30.336` (`:2683`) | finished | `2026-07-14 18:07:10.341` (`:2712`) | 280.005 s | 261.021 s |
| 27 | 5 | `2026-07-14 18:07:20.798` (`:2784`) | finished | `2026-07-14 18:12:00.331` (`:2813`) | 279.533 s | 260.475 s |
| 28 | 3 | `2026-07-14 18:12:10.710` (`:2885`) | finished | `2026-07-14 18:16:54.226` (`:2914`) | 283.516 s | 269.323 s |
| 29 | 1 | `2026-07-14 18:17:04.591` (`:2986`) | finished | `2026-07-14 18:21:47.520` (`:3015`) | 282.929 s | 267.512 s |
| 30 | 5 | `2026-07-14 18:21:58.062` (`:3087`) | finished | `2026-07-14 18:26:45.874` (`:3116`) | 287.812 s | 267.921 s |
| 31 | 2 | `2026-07-14 18:26:56.623` (`:3188`) | finished | `2026-07-14 18:31:46.416` (`:3217`) | 289.793 s | 270.344 s |
| 32 | 6 | `2026-07-14 18:31:56.154` (`:3289`) | finished | `2026-07-14 18:36:52.081` (`:3318`) | 295.927 s | 277.255 s |
| 33 | 5 | `2026-07-14 18:37:00.022` (`:3392`) | finished | `2026-07-14 18:41:58.539` (`:3421`) | 298.517 s | 279.772 s |
| 34 | 3 | `2026-07-14 18:42:09.060` (`:3493`) | finished | `2026-07-14 18:46:57.563` (`:3522`) | 288.503 s | 275.100 s |
| 35 | 2 | `2026-07-14 18:47:07.417` (`:3594`) | finished | `2026-07-14 18:52:01.931` (`:3623`) | 294.514 s | 279.578 s |
| 36 | 5 | `2026-07-14 18:52:10.468` (`:3697`) | finished | `2026-07-14 18:57:01.233` (`:3726`) | 290.765 s | 271.821 s |
| 37 | 3 | `2026-07-14 18:57:11.272` (`:3798`) | finished | `2026-07-14 19:02:07.958` (`:3827`) | 296.686 s | 282.459 s |
| 38 | 5 | `2026-07-14 19:02:18.506` (`:3901`) | finished | `2026-07-14 19:07:14.163` (`:3930`) | 295.657 s | 276.416 s |
| 39 | 4 | `2026-07-14 19:07:24.549` (`:4002`) | finished | `2026-07-14 19:12:21.789` (`:4031`) | 297.240 s | 277.573 s |
| 40 | 1 | `2026-07-14 19:12:31.371` (`:4105`) | finished | `2026-07-14 19:17:23.581` (`:4134`) | 292.210 s | 276.760 s |
| 41 | 6 | `2026-07-14 19:17:33.172` (`:4206`) | finished | `2026-07-14 19:22:38.306` (`:4235`) | 305.134 s | 286.305 s |
| 42 | 4 | `2026-07-14 19:22:47.806` (`:4309`) | finished | `2026-07-14 19:27:44.626` (`:4338`) | 296.820 s | 277.712 s |
| 43 | 2 | `2026-07-14 19:27:54.399` (`:4410`) | finished | `2026-07-14 19:32:54.126` (`:4439`) | 299.727 s | 283.883 s |
| 44 | 3 | `2026-07-14 19:33:03.596` (`:4511`) | finished | `2026-07-14 19:38:05.787` (`:4540`) | 302.191 s | 286.650 s |
| 45 | 2 | `2026-07-14 19:38:15.393` (`:4612`) | finished | `2026-07-14 19:43:16.505` (`:4641`) | 301.112 s | 285.873 s |
| 46 | 3 | `2026-07-14 19:43:25.934` (`:4713`) | finished | `2026-07-14 19:48:25.555` (`:4742`) | 299.621 s | 285.546 s |
| 47 | 1 | `2026-07-14 19:48:35.255` (`:4814`) | finished | `2026-07-14 19:53:43.141` (`:4843`) | 307.886 s | 290.686 s |
| 48 | 6 | `2026-07-14 19:53:52.571` (`:4917`) | finished | `2026-07-14 19:59:00.401` (`:4946`) | 307.830 s | 288.016 s |
| 49 | 5 | `2026-07-14 19:59:10.797` (`:5018`) | finished | `2026-07-14 20:04:23.455` (`:5047`) | 312.658 s | 293.272 s |
| 50 | 2 | `2026-07-14 20:04:34.088` (`:5121`) | finished | `2026-07-14 20:09:38.769` (`:5150`) | 304.681 s | 288.806 s |
| 51 | 4 | `2026-07-14 20:09:49.121` (`:5222`) | finished | `2026-07-14 20:15:00.584` (`:5251`) | 311.463 s | 293.263 s |
| 52 | 1 | `2026-07-14 20:15:09.330` (`:5325`) | finished | `2026-07-14 20:20:21.711` (`:5354`) | 312.381 s | 288.862 s |
| 53 | 4 | `2026-07-14 20:20:31.452` (`:5426`) | finished | `2026-07-14 20:25:42.675` (`:5455`) | 311.223 s | 291.852 s |
| 54 | 5 | `2026-07-14 20:25:53.268` (`:5529`) | finished | `2026-07-14 20:31:02.392` (`:5558`) | 309.124 s | 290.306 s |
| 55 | 6 | `2026-07-14 20:31:13.298` (`:5630`) | finished | `2026-07-14 20:36:31.806` (`:5659`) | 318.508 s | 298.989 s |
| 56 | 2 | `2026-07-14 20:36:40.535` (`:5733`) | finished | `2026-07-14 20:41:55.289` (`:5762`) | 314.754 s | 299.452 s |
| 57 | 3 | `2026-07-14 20:42:05.233` (`:5834`) | finished | `2026-07-14 20:47:33.030` (`:5863`) | 327.797 s | 313.858 s |
| 58 | 2 | `2026-07-14 20:47:42.817` (`:5937`) | finished | `2026-07-14 20:53:11.632` (`:5966`) | 328.815 s | 314.298 s |
| 59 | 5 | `2026-07-14 20:53:20.225` (`:6038`) | finished | `2026-07-14 20:58:46.720` (`:6067`) | 326.495 s | 306.312 s |
| 60 | 1 | `2026-07-14 20:58:56.299` (`:6139`) | finished | `2026-07-14 21:04:31.914` (`:6168`) | 335.615 s | 320.046 s |
| 61 | 2 | `2026-07-14 21:04:40.719` (`:6242`) | finished | `2026-07-14 21:10:09.730` (`:6271`) | 329.011 s | 314.883 s |
| 62 | 6 | `2026-07-14 21:10:19.193` (`:6343`) | finished | `2026-07-14 21:15:59.405` (`:6372`) | 340.212 s | 320.821 s |
| 63 | 2 | `2026-07-14 21:16:08.101` (`:6446`) | finished | `2026-07-14 21:21:43.972` (`:6475`) | 335.871 s | 320.098 s |
| 64 | 5 | `2026-07-14 21:21:52.486` (`:6547`) | finished | `2026-07-14 21:27:34.487` (`:6576`) | 342.001 s | 321.756 s |
| 65 | 6 | `2026-07-14 21:27:44.937` (`:6648`) | finished | `2026-07-14 21:33:21.449` (`:6677`) | 336.512 s | 322.885 s |
| 66 | 5 | `2026-07-14 21:33:31.393` (`:6751`) | finished | `2026-07-14 21:39:15.339` (`:6780`) | 343.946 s | 323.756 s |
| 67 | 2 | `2026-07-14 21:39:24.994` (`:6852`) | finished | `2026-07-14 21:44:55.693` (`:6881`) | 330.699 s | 315.314 s |
| 68 | 4 | `2026-07-14 21:45:05.387` (`:6953`) | finished | `2026-07-14 21:50:53.410` (`:6982`) | 348.023 s | 317.179 s |
| 69 | 3 | `2026-07-14 21:51:03.123` (`:7056`) | finished | `2026-07-14 21:56:37.225` (`:7085`) | 334.102 s | 318.917 s |
| 70 | 1 | `2026-07-14 21:56:46.082` (`:7157`) | finished | `2026-07-14 22:02:21.954` (`:7186`) | 335.872 s | 318.640 s |
| 71 | 3 | `2026-07-14 22:02:31.793` (`:7260`) | finished | `2026-07-14 22:08:18.117` (`:7289`) | 346.324 s | 331.012 s |
| 72 | 2 | `2026-07-14 22:08:28.918` (`:7361`) | finished | `2026-07-14 22:14:10.867` (`:7390`) | 341.949 s | 325.736 s |
| 73 | 3 | `2026-07-14 22:14:19.848` (`:7464`) | finished | `2026-07-14 22:20:05.218` (`:7493`) | 345.370 s | 330.347 s |
| 74 | 5 | `2026-07-14 22:20:14.285` (`:7565`) | finished | `2026-07-14 22:26:03.366` (`:7594`) | 349.081 s | 328.130 s |
| 75 | 2 | `2026-07-14 22:26:13.949` (`:7668`) | finished | `2026-07-14 22:31:53.642` (`:7697`) | 339.693 s | 324.309 s |
| 76 | 1 | `2026-07-14 22:32:03.731` (`:7769`) | finished | `2026-07-14 22:37:43.766` (`:7798`) | 340.035 s | 322.908 s |
| 77 | 6 | `2026-07-14 22:37:53.099` (`:7872`) | finished | `2026-07-14 22:43:35.336` (`:7901`) | 342.237 s | 327.212 s |
| 78 | 1 | `2026-07-14 22:43:45.385` (`:7973`) | finished | `2026-07-14 22:49:29.059` (`:8002`) | 343.674 s | 326.279 s |
| 79 | 3 | `2026-07-14 22:49:40.270` (`:8076`) | finished | `2026-07-14 22:55:27.782` (`:8105`) | 347.512 s | 332.938 s |
| 80 | 1 | `2026-07-14 22:55:37.173` (`:8177`) | finished | `2026-07-14 23:01:21.125` (`:8206`) | 343.952 s | 327.249 s |
| 81 | 4 | `2026-07-14 23:01:30.053` (`:8280`) | finished | `2026-07-14 23:07:19.588` (`:8309`) | 349.535 s | 329.409 s |
| 82 | 1 | `2026-07-14 23:07:29.594` (`:8381`) | finished | `2026-07-14 23:13:14.544` (`:8410`) | 344.950 s | 327.972 s |
| 83 | 5 | `2026-07-14 23:13:24.233` (`:8482`) | finished | `2026-07-14 23:19:20.489` (`:8511`) | 356.256 s | 335.863 s |
| 84 | 1 | `2026-07-14 23:19:30.290` (`:8585`) | finished | `2026-07-14 23:25:15.497` (`:8614`) | 345.207 s | 326.882 s |
| 85 | 5 | `2026-07-14 23:25:25.758` (`:8686`) | finished | `2026-07-14 23:30:41.183` (`:8715`) | 315.425 s | 295.093 s |
| 86 | 6 | `2026-07-14 23:30:51.071` (`:8789`) | finished | `2026-07-14 23:35:30.809` (`:8818`) | 279.738 s | 264.321 s |
| 87 | 4 | `2026-07-14 23:35:40.136` (`:8890`) | finished | `2026-07-14 23:40:31.248` (`:8919`) | 291.112 s | 270.728 s |
| 88 | 1 | `2026-07-14 23:40:38.884` (`:8991`) | finished | `2026-07-14 23:45:31.853` (`:9020`) | 292.969 s | 276.120 s |
| 89 | 3 | `2026-07-14 23:45:40.468` (`:9094`) | finished | `2026-07-14 23:50:41.800` (`:9123`) | 301.332 s | 285.832 s |
| 90 | 5 | `2026-07-14 23:50:51.729` (`:9195`) | finished | `2026-07-14 23:55:54.521` (`:9224`) | 302.792 s | 282.118 s |
| 91 | 6 | `2026-07-14 23:56:04.220` (`:9296`) | finished | `2026-07-15 00:00:57.727` (`:9325`) | 293.507 s | 278.464 s |
| 92 | 5 | `2026-07-15 00:01:06.993` (`:9397`) | finished | `2026-07-15 00:06:11.569` (`:9426`) | 304.576 s | 284.349 s |
| 93 | 2 | `2026-07-15 00:06:21.382` (`:9500`) | finished | `2026-07-15 00:11:16.056` (`:9529`) | 294.674 s | 279.165 s |
| 94 | 3 | `2026-07-15 00:11:25.730` (`:9601`) | finished | `2026-07-15 00:16:30.181` (`:9630`) | 304.451 s | 288.846 s |
| 95 | 1 | `2026-07-15 00:16:40.290` (`:9704`) | finished | `2026-07-15 00:21:45.158` (`:9733`) | 304.868 s | 287.188 s |
| 96 | 5 | `2026-07-15 00:21:54.076` (`:9805`) | finished | `2026-07-15 00:26:59.414` (`:9834`) | 305.338 s | 285.342 s |
| 97 | 3 | `2026-07-15 00:27:10.162` (`:9908`) | finished | `2026-07-15 00:32:19.134` (`:9937`) | 308.972 s | 294.155 s |
| 98 | 5 | `2026-07-15 00:32:29.065` (`:10009`) | finished | `2026-07-15 00:37:38.774` (`:10038`) | 309.709 s | 288.476 s |
| 99 | 2 | `2026-07-15 00:37:48.448` (`:10112`) | finished | `2026-07-15 00:42:56.386` (`:10141`) | 307.938 s | 292.245 s |
| 100 | 6 | `2026-07-15 00:43:05.398` (`:10213`) | finished | `2026-07-15 00:48:05.003` (`:10242`) | 299.605 s | 284.101 s |
| 101 | 5 | `2026-07-15 00:48:13.805` (`:10314`) | finished | `2026-07-15 00:53:22.067` (`:10343`) | 308.262 s | 287.375 s |
| 102 | 2 | `2026-07-15 00:53:31.999` (`:10417`) | finished | `2026-07-15 00:58:34.614` (`:10446`) | 302.615 s | 287.019 s |
| 103 | 5 | `2026-07-15 00:58:44.293` (`:10518`) | finished | `2026-07-15 01:03:50.921` (`:10547`) | 306.628 s | 286.030 s |
| 104 | 2 | `2026-07-15 01:04:00.208` (`:10621`) | finished | `2026-07-15 01:09:05.958` (`:10650`) | 305.750 s | 289.096 s |
| 105 | 3 | `2026-07-15 01:09:15.668` (`:10722`) | finished | `2026-07-15 01:14:22.339` (`:10751`) | 306.671 s | 290.964 s |
| 106 | 1 | `2026-07-15 01:14:32.240` (`:10825`) | finished | `2026-07-15 01:19:40.882` (`:10854`) | 308.642 s | 290.872 s |
| 107 | 3 | `2026-07-15 01:19:50.840` (`:10926`) | finished | `2026-07-15 01:24:58.048` (`:10955`) | 307.208 s | 292.026 s |
| 108 | 1 | `2026-07-15 01:25:05.868` (`:11027`) | finished | `2026-07-15 01:30:15.503` (`:11056`) | 309.635 s | 292.741 s |
| 109 | 2 | `2026-07-15 01:30:24.623` (`:11128`) | finished | `2026-07-15 01:35:32.542` (`:11157`) | 307.919 s | 291.880 s |
| 110 | 3 | `2026-07-15 01:35:42.365` (`:11231`) | finished | `2026-07-15 01:40:52.531` (`:11260`) | 310.166 s | 295.602 s |
| 111 | 2 | `2026-07-15 01:41:02.091` (`:11332`) | finished | `2026-07-15 01:46:11.476` (`:11361`) | 309.385 s | 293.578 s |
| 112 | 6 | `2026-07-15 01:46:20.612` (`:11435`) | finished | `2026-07-15 01:51:32.721` (`:11464`) | 312.109 s | 297.322 s |
| 113 | 2 | `2026-07-15 01:51:41.705` (`:11536`) | finished | `2026-07-15 01:56:56.105` (`:11565`) | 314.400 s | 297.442 s |
| 114 | 6 | `2026-07-15 01:57:06.213` (`:11637`) | finished | `2026-07-15 02:02:24.345` (`:11666`) | 318.132 s | 303.008 s |
| 115 | 1 | `2026-07-15 02:02:33.232` (`:11738`) | finished | `2026-07-15 02:07:50.059` (`:11767`) | 316.827 s | 298.449 s |
| 116 | 5 | `2026-07-15 02:07:58.669` (`:11841`) | finished | `2026-07-15 02:13:23.538` (`:11870`) | 324.869 s | 303.982 s |
| 117 | 4 | `2026-07-15 02:13:33.151` (`:11942`) | finished | `2026-07-15 02:18:51.754` (`:11971`) | 318.603 s | 297.546 s |
| 118 | 5 | `2026-07-15 02:19:00.670` (`:12045`) | finished | `2026-07-15 02:24:21.178` (`:12074`) | 320.508 s | 299.535 s |
| 119 | 4 | `2026-07-15 02:24:29.911` (`:12146`) | finished | `2026-07-15 02:29:52.675` (`:12175`) | 322.764 s | 301.898 s |
| 120 | 2 | `2026-07-15 02:30:02.556` (`:12249`) | finished | `2026-07-15 02:35:14.992` (`:12278`) | 312.436 s | 296.475 s |
| 121 | 6 | `2026-07-15 02:35:23.963` (`:12350`) | finished | `2026-07-15 02:40:46.970` (`:12379`) | 323.007 s | 306.697 s |
| 122 | 2 | `2026-07-15 02:40:55.857` (`:12451`) | finished | `2026-07-15 02:46:18.903` (`:12480`) | 323.046 s | 306.820 s |
| 123 | 5 | `2026-07-15 02:46:27.944` (`:12554`) | finished | `2026-07-15 02:51:51.030` (`:12583`) | 323.086 s | 301.945 s |
| 124 | 2 | `2026-07-15 02:52:00.769` (`:12655`) | finished | `2026-07-15 02:57:16.089` (`:12684`) | 315.320 s | 298.909 s |
| 125 | 1 | `2026-07-15 02:57:26.087` (`:12756`) | finished | `2026-07-15 03:02:51.741` (`:12785`) | 325.654 s | 307.998 s |
| 126 | 5 | `2026-07-15 03:03:01.424` (`:12859`) | finished | `2026-07-15 03:08:33.391` (`:12888`) | 331.967 s | 311.315 s |
| 127 | 6 | `2026-07-15 03:08:43.561` (`:12960`) | finished | `2026-07-15 03:14:07.166` (`:12989`) | 323.605 s | 308.561 s |
| 128 | 5 | `2026-07-15 03:14:16.906` (`:13061`) | finished | `2026-07-15 03:19:42.486` (`:13090`) | 325.580 s | 305.162 s |
| 129 | 3 | `2026-07-15 03:19:52.410` (`:13164`) | finished | `2026-07-15 03:25:22.196` (`:13193`) | 329.786 s | 314.214 s |
| 130 | 2 | `2026-07-15 03:25:31.379` (`:13265`) | finished | `2026-07-15 03:30:57.780` (`:13294`) | 326.401 s | 310.125 s |
| 131 | 4 | `2026-07-15 03:31:07.673` (`:13366`) | finished | `2026-07-15 03:36:39.790` (`:13395`) | 332.117 s | 311.995 s |
| 132 | 3 | `2026-07-15 03:36:49.812` (`:13469`) | finished | `2026-07-15 03:42:17.311` (`:13498`) | 327.499 s | 311.463 s |
| 133 | 1 | `2026-07-15 03:42:27.262` (`:13570`) | finished | `2026-07-15 03:47:59.286` (`:13599`) | 332.024 s | 314.244 s |
| 134 | 2 | `2026-07-15 03:48:09.533` (`:13673`) | finished | `2026-07-15 03:53:43.204` (`:13702`) | 333.671 s | 316.750 s |
| 135 | 1 | `2026-07-15 03:53:52.073` (`:13774`) | finished | `2026-07-15 03:59:23.319` (`:13803`) | 331.246 s | 313.766 s |
| 136 | 2 | `2026-07-15 03:59:32.942` (`:13877`) | finished | `2026-07-15 04:05:01.531` (`:13906`) | 328.589 s | 312.470 s |
| 137 | 4 | `2026-07-15 04:05:11.310` (`:13978`) | finished | `2026-07-15 04:10:49.145` (`:14007`) | 337.835 s | 316.865 s |
| 138 | 1 | `2026-07-15 04:10:58.359` (`:14081`) | finished | `2026-07-15 04:16:31.054` (`:14110`) | 332.695 s | 314.675 s |
| 139 | 6 | `2026-07-15 04:16:39.727` (`:14182`) | finished | `2026-07-15 04:22:09.154` (`:14211`) | 329.427 s | 313.912 s |
| 140 | 3 | `2026-07-15 04:22:18.829` (`:14285`) | finished | `2026-07-15 04:27:56.697` (`:14314`) | 337.868 s | 321.774 s |
| 141 | 5 | `2026-07-15 04:28:05.570` (`:14386`) | finished | `2026-07-15 04:33:43.970` (`:14415`) | 338.400 s | 316.535 s |
| 142 | 2 | `2026-07-15 04:33:54.048` (`:14487`) | finished | `2026-07-15 04:39:29.765` (`:14516`) | 335.717 s | 319.217 s |
| 143 | 5 | `2026-07-15 04:39:39.592` (`:14590`) | finished | `2026-07-15 04:45:18.077` (`:14619`) | 338.485 s | 316.753 s |
| 144 | 3 | `2026-07-15 04:45:27.628` (`:14691`) | finished | `2026-07-15 04:51:01.680` (`:14720`) | 334.052 s | 317.698 s |
| 145 | 5 | `2026-07-15 04:51:11.581` (`:14794`) | finished | `2026-07-15 04:56:53.413` (`:14823`) | 341.832 s | 320.658 s |
| 146 | 6 | `2026-07-15 04:57:02.408` (`:14895`) | finished | `2026-07-15 05:02:44.358` (`:14924`) | 341.950 s | 325.803 s |
| 147 | 4 | `2026-07-15 05:02:54.184` (`:14996`) | finished | `2026-07-15 05:08:24.890` (`:15025`) | 330.706 s | 315.149 s |
| 148 | 5 | `2026-07-15 05:08:34.718` (`:15097`) | finished | `2026-07-15 05:14:11.716` (`:15126`) | 336.998 s | 316.398 s |
| 149 | 1 | `2026-07-15 05:14:19.814` (`:15200`) | finished | `2026-07-15 05:20:03.512` (`:15229`) | 343.698 s | 326.632 s |
| 150 | 6 | `2026-07-15 05:20:14.380` (`:15301`) | finished | `2026-07-15 05:25:53.045` (`:15330`) | 338.665 s | 322.714 s |
| 151 | 2 | `2026-07-15 05:26:02.227` (`:15404`) | finished | `2026-07-15 05:31:45.751` (`:15433`) | 343.524 s | 326.413 s |
| 152 | 5 | `2026-07-15 05:31:55.481` (`:15505`) | finished | `2026-07-15 05:37:40.450` (`:15534`) | 344.969 s | 322.576 s |
| 153 | 1 | `2026-07-15 05:37:49.605` (`:15608`) | finished | `2026-07-15 05:43:39.895` (`:15637`) | 350.290 s | 332.142 s |
| 154 | 6 | `2026-07-15 05:43:50.054` (`:15709`) | finished | `2026-07-15 05:49:50.024` (`:15738`) | 359.970 s | 344.341 s |
| 155 | 5 | `2026-07-15 05:49:59.761` (`:15812`) | finished | `2026-07-15 05:55:57.831` (`:15841`) | 358.070 s | 336.652 s |
| 156 | 4 | `2026-07-15 05:56:07.701` (`:15913`) | finished | `2026-07-15 06:01:59.781` (`:15942`) | 352.080 s | 336.931 s |
| 157 | 1 | `2026-07-15 06:02:09.157` (`:16016`) | finished | `2026-07-15 06:08:06.405` (`:16045`) | 357.248 s | 338.243 s |
| 158 | 6 | `2026-07-15 06:08:16.430` (`:16117`) | finished | `2026-07-15 06:14:07.746` (`:16146`) | 351.316 s | 335.058 s |
| 159 | 5 | `2026-07-15 06:14:17.610` (`:16218`) | finished | `2026-07-15 06:20:15.819` (`:16247`) | 358.209 s | 336.840 s |
| 160 | 1 | `2026-07-15 06:20:25.753` (`:16319`) | finished | `2026-07-15 06:26:18.880` (`:16348`) | 353.127 s | 334.813 s |
| 161 | 3 | `2026-07-15 06:26:29.048` (`:16422`) | finished | `2026-07-15 06:32:14.715` (`:16451`) | 345.667 s | 331.250 s |
| 162 | 5 | `2026-07-15 06:32:24.804` (`:16523`) | finished | `2026-07-15 06:38:20.228` (`:16552`) | 355.424 s | 332.872 s |
| 163 | 4 | `2026-07-15 06:38:29.922` (`:16626`) | finished | `2026-07-15 06:44:21.212` (`:16655`) | 351.290 s | 334.956 s |
| 164 | 2 | `2026-07-15 06:44:31.271` (`:16727`) | finished | `2026-07-15 06:50:24.303` (`:16756`) | 353.032 s | 336.943 s |
| 165 | 3 | `2026-07-15 06:50:34.158` (`:16828`) | finished | `2026-07-15 06:56:26.579` (`:16857`) | 352.421 s | 336.313 s |
| 166 | 2 | `2026-07-15 06:56:35.561` (`:16929`) | finished | `2026-07-15 07:02:24.023` (`:16958`) | 348.462 s | 333.575 s |
| 167 | 5 | `2026-07-15 07:02:34.108` (`:17030`) | finished | `2026-07-15 07:08:29.080` (`:17059`) | 354.972 s | 332.671 s |
| 168 | 2 | `2026-07-15 07:08:38.938` (`:17133`) | finished | `2026-07-15 07:14:28.462` (`:17162`) | 349.524 s | 334.106 s |
| 169 | 4 | `2026-07-15 07:14:38.252` (`:17234`) | finished | `2026-07-15 07:20:27.816` (`:17263`) | 349.564 s | 333.191 s |
| 170 | 2 | `2026-07-15 07:20:37.572` (`:17335`) | finished | `2026-07-15 07:26:38.464` (`:17364`) | 360.892 s | 344.173 s |
| 171 | 1 | `2026-07-15 07:26:48.462` (`:17438`) | finished | `2026-07-15 07:32:52.184` (`:17467`) | 363.722 s | 344.628 s |
| 172 | 4 | `2026-07-15 07:33:02.045` (`:17539`) | finished | `2026-07-15 07:38:56.110` (`:17568`) | 354.065 s | 337.411 s |
| 173 | 5 | `2026-07-15 07:39:04.818` (`:17640`) | finished | `2026-07-15 07:44:57.827` (`:17669`) | 353.009 s | 336.013 s |
| 174 | 1 | `2026-07-15 07:45:07.720` (`:17741`) | finished | `2026-07-15 07:51:23.526` (`:17770`) | 375.806 s | 347.608 s |
| 175 | 2 | `2026-07-15 07:51:33.973` (`:17844`) | finished | `2026-07-15 07:57:27.945` (`:17873`) | 353.972 s | 338.335 s |
| 176 | 4 | `2026-07-15 07:57:37.679` (`:17945`) | finished | `2026-07-15 08:03:33.838` (`:17974`) | 356.159 s | 340.949 s |
| 177 | 5 | `2026-07-15 08:03:43.730` (`:18048`) | finished | `2026-07-15 08:09:36.148` (`:18077`) | 352.418 s | 336.493 s |
| 178 | 1 | `2026-07-15 08:09:44.891` (`:18149`) | finished | `2026-07-15 08:15:46.183` (`:18178`) | 361.292 s | 341.594 s |
| 179 | 6 | `2026-07-15 08:15:56.090` (`:18250`) | finished | `2026-07-15 08:21:56.923` (`:18279`) | 360.833 s | 344.638 s |
| 180 | 4 | `2026-07-15 08:22:06.925` (`:18351`) | finished | `2026-07-15 08:27:58.514` (`:18380`) | 351.589 s | 336.018 s |
| 181 | 3 | `2026-07-15 08:28:08.795` (`:18454`) | finished | `2026-07-15 08:34:08.854` (`:18483`) | 360.059 s | 343.340 s |
| 182 | 6 | `2026-07-15 08:34:18.031` (`:18555`) | finished | `2026-07-15 08:40:01.242` (`:18584`) | 343.211 s | 326.932 s |
| 183 | 2 | `2026-07-15 08:40:11.129` (`:18658`) | finished | `2026-07-15 08:46:10.336` (`:18687`) | 359.207 s | 340.923 s |
| 184 | 6 | `2026-07-15 08:46:19.390` (`:18759`) | finished | `2026-07-15 08:52:13.007` (`:18788`) | 353.617 s | 337.256 s |
| 185 | 2 | `2026-07-15 08:52:23.036` (`:18862`) | finished | `2026-07-15 08:58:14.301` (`:18891`) | 351.265 s | 333.251 s |
| 186 | 6 | `2026-07-15 08:58:24.402` (`:18963`) | finished | `2026-07-15 09:04:15.365` (`:18992`) | 350.963 s | 334.936 s |
| 187 | 4 | `2026-07-15 09:04:23.325` (`:19064`) | finished | `2026-07-15 09:10:16.468` (`:19093`) | 353.143 s | 336.402 s |
| 188 | 3 | `2026-07-15 09:10:24.608` (`:19165`) | finished | `2026-07-15 09:16:20.699` (`:19194`) | 356.091 s | 339.829 s |
| 189 | 5 | `2026-07-15 09:16:29.616` (`:19268`) | finished | `2026-07-15 09:22:28.936` (`:19297`) | 359.320 s | 343.301 s |
| 190 | 6 | `2026-07-15 09:22:39.105` (`:19369`) | finished | `2026-07-15 09:28:40.245` (`:19398`) | 361.140 s | 344.691 s |
| 191 | 2 | `2026-07-15 09:28:49.302` (`:19472`) | finished | `2026-07-15 09:34:46.697` (`:19501`) | 357.395 s | 339.521 s |
| 192 | 6 | `2026-07-15 09:34:56.662` (`:19573`) | finished | `2026-07-15 09:40:53.006` (`:19602`) | 356.344 s | 339.869 s |
| 193 | 1 | `2026-07-15 09:41:02.969` (`:19676`) | finished | `2026-07-15 09:46:57.400` (`:19705`) | 354.431 s | 335.447 s |
| 194 | 3 | `2026-07-15 09:47:07.169` (`:19777`) | finished | `2026-07-15 09:52:59.782` (`:19806`) | 352.613 s | 336.590 s |
| 195 | 1 | `2026-07-15 09:53:09.622` (`:19880`) | finished | `2026-07-15 09:59:07.499` (`:19909`) | 357.877 s | 338.675 s |
| 196 | 5 | `2026-07-15 09:59:16.287` (`:19981`) | finished | `2026-07-15 10:05:15.066` (`:20010`) | 358.779 s | 342.440 s |
| 197 | 2 | `2026-07-15 10:05:25.169` (`:20082`) | finished | `2026-07-15 10:11:23.002` (`:20111`) | 357.833 s | 341.192 s |
| 198 | 1 | `2026-07-15 10:11:32.979` (`:20185`) | finished | `2026-07-15 10:17:41.025` (`:20214`) | 368.046 s | 348.146 s |
| 199 | 3 | `2026-07-15 10:17:51.238` (`:20286`) | finished | `2026-07-15 10:23:55.944` (`:20315`) | 364.706 s | 348.495 s |
| 200 | 6 | `2026-07-15 10:24:06.226` (`:20387`) | finished | `2026-07-15 10:30:09.695` (`:20416`) | 363.469 s | 346.665 s |
| 201 | 2 | `2026-07-15 10:30:17.581` (`:20490`) | finished | `2026-07-15 10:36:17.748` (`:20519`) | 360.167 s | 341.536 s |
| 202 | 4 | `2026-07-15 10:36:26.867` (`:20591`) | finished | `2026-07-15 10:42:26.464` (`:20620`) | 359.597 s | 342.835 s |
| 203 | 5 | `2026-07-15 10:42:36.126` (`:20692`) | finished | `2026-07-15 10:48:36.501` (`:20721`) | 360.375 s | 343.105 s |
| 204 | 1 | `2026-07-15 10:48:46.100` (`:20793`) | finished | `2026-07-15 10:54:55.598` (`:20822`) | 369.498 s | 349.922 s |
| 205 | 4 | `2026-07-15 10:55:05.881` (`:20894`) | finished | `2026-07-15 11:01:11.440` (`:20923`) | 365.559 s | 349.474 s |
| 206 | 5 | `2026-07-15 11:01:21.222` (`:20997`) | finished | `2026-07-15 11:07:27.523` (`:21026`) | 366.301 s | 348.973 s |
| 207 | 1 | `2026-07-15 11:07:36.173` (`:21098`) | finished | `2026-07-15 11:13:45.219` (`:21127`) | 369.046 s | 349.484 s |
| 208 | 5 | `2026-07-15 11:13:55.828` (`:21199`) | finished | `2026-07-15 11:20:05.255` (`:21228`) | 369.427 s | 352.715 s |
| 209 | 1 | `2026-07-15 11:20:15.606` (`:21300`) | finished | `2026-07-15 11:26:35.654` (`:21329`) | 380.048 s | 359.782 s |
| 210 | 3 | `2026-07-15 11:26:46.067` (`:21401`) | finished | `2026-07-15 11:32:54.797` (`:21430`) | 368.730 s | 352.194 s |
| 211 | 2 | `2026-07-15 11:33:03.474` (`:21502`) | finished | `2026-07-15 11:39:12.745` (`:21531`) | 369.271 s | 352.959 s |
| 212 | 4 | `2026-07-15 11:39:21.403` (`:21605`) | finished | `2026-07-15 11:45:34.561` (`:21634`) | 373.158 s | 356.342 s |
| 213 | 2 | `2026-07-15 11:45:44.428` (`:21706`) | finished | `2026-07-15 11:51:55.976` (`:21735`) | 371.548 s | 354.145 s |
| 214 | 5 | `2026-07-15 11:52:05.847` (`:21807`) | finished | `2026-07-15 11:58:15.820` (`:21836`) | 369.973 s | 352.920 s |
| 215 | 3 | `2026-07-15 11:58:24.339` (`:21908`) | finished | `2026-07-15 12:04:35.060` (`:21937`) | 370.721 s | 354.615 s |
| 216 | 6 | `2026-07-15 12:04:43.768` (`:22009`) | finished | `2026-07-15 12:10:55.801` (`:22038`) | 372.033 s | 354.794 s |
| 217 | 4 | `2026-07-15 12:11:04.709` (`:22110`) | finished | `2026-07-15 12:17:22.237` (`:22139`) | 377.528 s | 361.657 s |
| 218 | 6 | `2026-07-15 12:17:32.258` (`:22213`) | finished | `2026-07-15 12:23:45.824` (`:22242`) | 373.566 s | 356.599 s |
| 219 | 1 | `2026-07-15 12:23:55.591` (`:22314`) | finished | `2026-07-15 12:30:13.713` (`:22343`) | 378.122 s | 358.173 s |
| 220 | 6 | `2026-07-15 12:30:22.784` (`:22417`) | finished | `2026-07-15 12:36:40.969` (`:22446`) | 378.185 s | 360.809 s |
| 221 | 2 | `2026-07-15 12:36:50.447` (`:22518`) | finished | `2026-07-15 12:43:13.869` (`:22547`) | 383.422 s | 366.602 s |
| 222 | 4 | `2026-07-15 12:43:23.568` (`:22621`) | finished | `2026-07-15 12:49:43.523` (`:22650`) | 379.955 s | 363.294 s |
| 223 | 3 | `2026-07-15 12:49:53.380` (`:22722`) | finished | `2026-07-15 12:56:16.560` (`:22751`) | 383.180 s | 365.767 s |
| 224 | 4 | `2026-07-15 12:56:26.257` (`:22823`) | finished | `2026-07-15 13:02:48.535` (`:22852`) | 382.278 s | 364.679 s |
| 225 | 1 | `2026-07-15 13:02:58.715` (`:22926`) | finished | `2026-07-15 13:09:23.281` (`:22955`) | 384.566 s | 364.904 s |
| 226 | 2 | `2026-07-15 13:09:32.205` (`:23027`) | finished | `2026-07-15 13:15:55.961` (`:23056`) | 383.756 s | 366.346 s |
| 227 | 5 | `2026-07-15 13:16:04.839` (`:23128`) | finished | `2026-07-15 13:22:29.306` (`:23157`) | 384.467 s | 366.203 s |
| 228 | 1 | `2026-07-15 13:22:39.182` (`:23231`) | finished | `2026-07-15 13:29:15.673` (`:23260`) | 396.491 s | 375.520 s |
| 229 | 4 | `2026-07-15 13:29:24.932` (`:23332`) | finished | `2026-07-15 13:35:49.032` (`:23361`) | 384.100 s | 366.983 s |
| 230 | 5 | `2026-07-15 13:35:58.416` (`:23435`) | finished | `2026-07-15 13:42:19.944` (`:23464`) | 381.528 s | 363.576 s |
| 231 | 1 | `2026-07-15 13:42:28.036` (`:23536`) | finished | `2026-07-15 13:48:50.437` (`:23565`) | 382.401 s | 362.278 s |
| 232 | 2 | `2026-07-15 13:49:01.572` (`:23639`) | finished | `2026-07-15 13:55:23.413` (`:23668`) | 381.841 s | 364.482 s |
| 233 | 1 | `2026-07-15 13:55:33.668` (`:23740`) | finished | `2026-07-15 14:01:59.309` (`:23769`) | 385.641 s | 368.888 s |
| 234 | 4 | `2026-07-15 14:02:09.105` (`:23843`) | finished | `2026-07-15 14:08:30.197` (`:23872`) | 381.092 s | 364.074 s |
| 235 | 5 | `2026-07-15 14:08:39.803` (`:23944`) | finished | `2026-07-15 14:14:59.826` (`:23973`) | 380.023 s | 361.718 s |
| 236 | 3 | `2026-07-15 14:15:08.652` (`:24047`) | finished | `2026-07-15 14:21:51.730` (`:24076`) | 403.078 s | 376.146 s |
| 237 | 2 | `2026-07-15 14:22:01.163` (`:24148`) | finished | `2026-07-15 14:28:22.748` (`:24177`) | 381.585 s | 363.773 s |
| 238 | 5 | `2026-07-15 14:28:31.472` (`:24251`) | finished | `2026-07-15 14:34:56.569` (`:24280`) | 385.097 s | 366.171 s |
| 239 | 4 | `2026-07-15 14:35:06.328` (`:24352`) | finished | `2026-07-15 14:41:27.018` (`:24381`) | 380.690 s | 363.848 s |
| 240 | 2 | `2026-07-15 14:41:36.540` (`:24453`) | finished | `2026-07-15 14:47:55.325` (`:24482`) | 378.785 s | 361.355 s |
| 241 | 1 | `2026-07-15 14:48:05.014` (`:24554`) | finished | `2026-07-15 14:54:28.008` (`:24583`) | 382.994 s | 365.419 s |
| 242 | 3 | `2026-07-15 14:54:36.700` (`:24655`) | finished | `2026-07-15 15:01:05.160` (`:24684`) | 388.460 s | 370.889 s |
| 243 | 1 | `2026-07-15 15:01:14.735` (`:24756`) | finished | `2026-07-15 15:07:38.131` (`:24785`) | 383.396 s | 365.900 s |
| 244 | 4 | `2026-07-15 15:07:47.707` (`:24857`) | finished | `2026-07-15 15:14:12.224` (`:24886`) | 384.517 s | 366.862 s |
| 245 | 5 | `2026-07-15 15:14:21.964` (`:24960`) | finished | `2026-07-15 15:20:52.046` (`:24989`) | 390.082 s | 370.917 s |
| 246 | 1 | `2026-07-15 15:21:00.645` (`:25061`) | finished | `2026-07-15 15:27:33.800` (`:25090`) | 393.155 s | 375.712 s |
| 247 | 6 | `2026-07-15 15:27:43.181` (`:25164`) | finished | `2026-07-15 15:34:08.934` (`:25193`) | 385.753 s | 367.492 s |
| 248 | 2 | `2026-07-15 15:34:18.737` (`:25265`) | finished | `2026-07-15 15:40:53.807` (`:25294`) | 395.070 s | 377.075 s |
| 249 | 5 | `2026-07-15 15:41:02.718` (`:25368`) | finished | `2026-07-15 15:47:30.092` (`:25398`) | 387.374 s | 368.836 s |
| 250 | 2 | `2026-07-15 15:47:39.883` (`:25470`) | finished | `2026-07-15 15:54:12.264` (`:25499`) | 392.381 s | 374.336 s |
| 251 | 3 | `2026-07-15 15:54:22.072` (`:25573`) | finished | `2026-07-15 16:00:51.572` (`:25602`) | 389.500 s | 371.046 s |
| 252 | 1 | `2026-07-15 16:01:01.336` (`:25674`) | finished | `2026-07-15 16:07:23.933` (`:25703`) | 382.597 s | 365.497 s |
| 253 | 4 | `2026-07-15 16:07:31.585` (`:25775`) | finished | `2026-07-15 16:13:58.942` (`:25804`) | 387.357 s | 370.330 s |
| 254 | 6 | `2026-07-15 16:14:06.840` (`:25878`) | finished | `2026-07-15 16:20:31.069` (`:25907`) | 384.229 s | 365.926 s |
| 255 | 2 | `2026-07-15 16:20:40.858` (`:25979`) | finished | `2026-07-15 16:27:10.000` (`:26008`) | 389.142 s | 372.116 s |
| 256 | 5 | `2026-07-15 16:27:20.083` (`:26082`) | finished | `2026-07-15 16:33:47.308` (`:26111`) | 387.225 s | 369.770 s |
| 257 | 1 | `2026-07-15 16:33:56.988` (`:26183`) | finished | `2026-07-15 16:40:30.321` (`:26212`) | 393.333 s | 375.461 s |
| 258 | 6 | `2026-07-15 16:40:39.713` (`:26286`) | finished | `2026-07-15 16:47:09.919` (`:26315`) | 390.206 s | 372.156 s |
| 259 | 2 | `2026-07-15 16:47:19.804` (`:26387`) | finished | `2026-07-15 16:53:55.030` (`:26416`) | 395.226 s | 377.177 s |
| 260 | 4 | `2026-07-15 16:54:04.033` (`:26488`) | finished | `2026-07-15 17:00:46.559` (`:26517`) | 402.526 s | 384.852 s |
| 261 | 2 | `2026-07-15 17:00:56.316` (`:26589`) | finished | `2026-07-15 17:07:31.119` (`:26618`) | 394.803 s | 377.298 s |
| 262 | 3 | `2026-07-15 17:07:39.824` (`:26692`) | finished | `2026-07-15 17:14:15.802` (`:26721`) | 395.978 s | 378.004 s |
| 263 | 2 | `2026-07-15 17:14:24.225` (`:26793`) | finished | `2026-07-15 17:21:01.560` (`:26822`) | 397.335 s | 380.426 s |
| 264 | 3 | `2026-07-15 17:21:10.363` (`:26896`) | finished | `2026-07-15 17:27:43.353` (`:26925`) | 392.990 s | 374.600 s |
| 265 | 5 | `2026-07-15 17:27:53.219` (`:26997`) | finished | `2026-07-15 17:34:16.026` (`:27026`) | 382.807 s | 365.042 s |
| 266 | 4 | `2026-07-15 17:34:24.294` (`:27098`) | finished | `2026-07-15 17:40:48.025` (`:27127`) | 383.731 s | 365.078 s |
| 267 | 5 | `2026-07-15 17:40:57.958` (`:27201`) | finished | `2026-07-15 17:47:28.039` (`:27230`) | 390.081 s | 370.485 s |
| 268 | 3 | `2026-07-15 17:47:37.641` (`:27302`) | finished | `2026-07-15 17:54:08.289` (`:27331`) | 390.648 s | 372.210 s |
| 269 | 6 | `2026-07-15 17:54:17.192` (`:27405`) | finished | `2026-07-15 18:00:50.015` (`:27434`) | 392.823 s | 373.249 s |
| 270 | 1 | `2026-07-15 18:00:59.760` (`:27506`) | finished | `2026-07-15 18:07:29.061` (`:27535`) | 389.301 s | 370.801 s |
| 271 | 3 | `2026-07-15 18:07:38.669` (`:27609`) | finished | `2026-07-15 18:14:07.698` (`:27638`) | 389.029 s | 370.417 s |
| 272 | 5 | `2026-07-15 18:14:17.311` (`:27710`) | finished | `2026-07-15 18:20:48.839` (`:27739`) | 391.528 s | 373.579 s |
| 273 | 3 | `2026-07-15 18:20:58.349` (`:27813`) | finished | `2026-07-15 18:27:33.416` (`:27842`) | 395.067 s | 376.474 s |
| 274 | 5 | `2026-07-15 18:27:42.239` (`:27914`) | finished | `2026-07-15 18:34:16.905` (`:27943`) | 394.666 s | 377.278 s |
| 275 | 4 | `2026-07-15 18:34:25.802` (`:28015`) | finished | `2026-07-15 18:41:00.141` (`:28044`) | 394.339 s | 376.635 s |
| 276 | 1 | `2026-07-15 18:41:09.672` (`:28118`) | finished | `2026-07-15 18:47:38.201` (`:28147`) | 388.529 s | 369.443 s |
| 277 | 6 | `2026-07-15 18:47:47.118` (`:28219`) | finished | `2026-07-15 18:54:15.965` (`:28248`) | 388.847 s | 369.134 s |
| 278 | 2 | `2026-07-15 18:54:23.570` (`:28320`) | finished | `2026-07-15 19:00:55.962` (`:28349`) | 392.392 s | 374.975 s |
| 279 | 4 | `2026-07-15 19:01:05.395` (`:28421`) | finished | `2026-07-15 19:07:39.206` (`:28450`) | 393.811 s | 376.090 s |
| 280 | 3 | `2026-07-15 19:07:48.027` (`:28524`) | finished | `2026-07-15 19:14:22.972` (`:28553`) | 394.945 s | 375.677 s |
| 281 | 2 | `2026-07-15 19:14:32.632` (`:28625`) | finished | `2026-07-15 19:21:00.769` (`:28654`) | 388.137 s | 369.211 s |
| 282 | 1 | `2026-07-15 19:21:09.361` (`:28728`) | finished | `2026-07-15 19:27:34.162` (`:28757`) | 384.801 s | 366.559 s |
| 283 | 5 | `2026-07-15 19:27:43.868` (`:28829`) | finished | `2026-07-15 19:34:16.186` (`:28858`) | 392.318 s | 373.407 s |
| 284 | 6 | `2026-07-15 19:34:25.902` (`:28930`) | finished | `2026-07-15 19:40:59.895` (`:28959`) | 393.993 s | 374.383 s |
| 285 | 4 | `2026-07-15 19:41:08.222` (`:29033`) | finished | `2026-07-15 19:47:41.506` (`:29062`) | 393.284 s | 375.196 s |
| 286 | 5 | `2026-07-15 19:47:50.185` (`:29134`) | finished | `2026-07-15 19:54:19.382` (`:29163`) | 389.197 s | 371.440 s |
| 287 | 6 | `2026-07-15 19:54:28.143` (`:29237`) | finished | `2026-07-15 20:01:02.224` (`:29266`) | 394.081 s | 375.013 s |
| 288 | 3 | `2026-07-15 20:01:11.096` (`:29338`) | finished | `2026-07-15 20:07:34.068` (`:29367`) | 382.972 s | 364.223 s |
| 289 | 4 | `2026-07-15 20:07:42.585` (`:29439`) | finished | `2026-07-15 20:14:17.817` (`:29468`) | 395.232 s | 377.628 s |
| 290 | 3 | `2026-07-15 20:14:26.871` (`:29540`) | finished | `2026-07-15 20:21:07.624` (`:29569`) | 400.753 s | 381.125 s |
| 291 | 2 | `2026-07-15 20:21:17.682` (`:29641`) | finished | `2026-07-15 20:27:42.965` (`:29670`) | 385.283 s | 366.782 s |
| 292 | 3 | `2026-07-15 20:27:52.355` (`:29744`) | finished | `2026-07-15 20:34:26.346` (`:29773`) | 393.991 s | 375.395 s |
| 293 | 4 | `2026-07-15 20:34:35.098` (`:29845`) | finished | `2026-07-15 20:41:08.035` (`:29874`) | 392.937 s | 374.808 s |
| 294 | 3 | `2026-07-15 20:41:17.982` (`:29948`) | finished | `2026-07-15 20:47:53.425` (`:29977`) | 395.443 s | 376.897 s |
| 295 | 1 | `2026-07-15 20:48:02.471` (`:30049`) | finished | `2026-07-15 20:54:35.507` (`:30078`) | 393.036 s | 373.543 s |
| 296 | 2 | `2026-07-15 20:54:45.258` (`:30150`) | finished | `2026-07-15 21:01:18.201` (`:30179`) | 392.943 s | 374.825 s |
| 297 | 1 | `2026-07-15 21:01:26.823` (`:30251`) | finished | `2026-07-15 21:07:56.094` (`:30280`) | 389.271 s | 369.873 s |
| 298 | 6 | `2026-07-15 21:08:04.602` (`:30352`) | finished | `2026-07-15 21:14:34.021` (`:30381`) | 389.419 s | 370.231 s |
| 299 | 5 | `2026-07-15 21:14:42.892` (`:30453`) | finished | `2026-07-15 21:21:11.054` (`:30482`) | 388.162 s | 369.205 s |
| 300 | 4 | `2026-07-15 21:21:20.709` (`:30556`) | finished | `2026-07-15 21:27:51.251` (`:30585`) | 390.542 s | 372.616 s |
| 301 | 2 | `2026-07-15 21:28:01.083` (`:30657`) | finished | `2026-07-15 21:34:30.189` (`:30686`) | 389.106 s | 370.443 s |
| 302 | 6 | `2026-07-15 21:34:39.818` (`:30760`) | finished | `2026-07-15 21:41:11.537` (`:30789`) | 391.719 s | 372.362 s |
| 303 | 4 | `2026-07-15 21:41:21.107` (`:30861`) | finished | `2026-07-15 21:47:45.601` (`:30890`) | 384.494 s | 366.727 s |
| 304 | 3 | `2026-07-15 21:47:54.552` (`:30962`) | finished | `2026-07-15 21:54:19.743` (`:30991`) | 385.191 s | 366.954 s |
| 305 | 5 | `2026-07-15 21:54:29.513` (`:31065`) | finished | `2026-07-15 22:00:42.673` (`:31094`) | 373.160 s | 354.159 s |
| 306 | 4 | `2026-07-15 22:00:50.597` (`:31166`) | finished | `2026-07-15 22:07:16.752` (`:31195`) | 386.155 s | 367.636 s |
| 307 | 6 | `2026-07-15 22:07:26.427` (`:31267`) | finished | `2026-07-15 22:13:51.045` (`:31296`) | 384.618 s | 366.173 s |
| 308 | 5 | `2026-07-15 22:13:59.661` (`:31370`) | finished | `2026-07-15 22:20:25.915` (`:31399`) | 386.254 s | 367.680 s |
| 309 | 4 | `2026-07-15 22:20:35.618` (`:31471`) | finished | `2026-07-15 22:27:01.561` (`:31500`) | 385.943 s | 367.794 s |
| 310 | 5 | `2026-07-15 22:27:11.431` (`:31574`) | finished | `2026-07-15 22:33:37.824` (`:31603`) | 386.393 s | 367.054 s |
| 311 | 4 | `2026-07-15 22:33:47.467` (`:31675`) | finished | `2026-07-15 22:40:13.739` (`:31704`) | 386.272 s | 367.480 s |
| 312 | 2 | `2026-07-15 22:40:22.594` (`:31778`) | finished | `2026-07-15 22:46:43.595` (`:31807`) | 381.001 s | 362.337 s |
| 313 | 4 | `2026-07-15 22:46:53.209` (`:31879`) | finished | `2026-07-15 22:53:20.630` (`:31908`) | 387.421 s | 369.458 s |
| 314 | 3 | `2026-07-15 22:53:30.255` (`:31982`) | finished | `2026-07-15 22:59:54.594` (`:32011`) | 384.339 s | 367.081 s |
| 315 | 4 | `2026-07-15 23:00:04.380` (`:32083`) | finished | `2026-07-15 23:06:43.008` (`:32112`) | 398.628 s | 364.126 s |
| 316 | 6 | `2026-07-15 23:06:51.563` (`:32186`) | finished | `2026-07-15 23:13:01.913` (`:32215`) | 370.350 s | 351.478 s |
| 317 | 3 | `2026-07-15 23:13:11.459` (`:32287`) | finished | `2026-07-15 23:19:36.808` (`:32316`) | 385.349 s | 365.794 s |
| 318 | 2 | `2026-07-15 23:19:44.760` (`:32388`) | finished | `2026-07-15 23:26:01.905` (`:32417`) | 377.145 s | 358.744 s |
| 319 | 1 | `2026-07-15 23:26:10.640` (`:32491`) | finished | `2026-07-15 23:32:45.509` (`:32520`) | 394.869 s | 375.757 s |
| 320 | 2 | `2026-07-15 23:32:54.239` (`:32592`) | finished | `2026-07-15 23:39:12.092` (`:32621`) | 377.853 s | 359.001 s |
| 321 | 6 | `2026-07-15 23:39:21.923` (`:32695`) | finished | `2026-07-15 23:45:37.958` (`:32724`) | 376.035 s | 357.296 s |
| 322 | 5 | `2026-07-15 23:45:46.635` (`:32796`) | finished | `2026-07-15 23:52:10.520` (`:32825`) | 383.885 s | 366.077 s |
| 323 | 3 | `2026-07-15 23:52:20.427` (`:32899`) | finished | `2026-07-15 23:58:43.587` (`:32928`) | 383.160 s | 364.565 s |
| 324 | 5 | `2026-07-15 23:58:52.324` (`:33000`) | finished | `2026-07-16 00:05:16.523` (`:33029`) | 384.199 s | 366.083 s |
| 325 | 4 | `2026-07-16 00:05:25.924` (`:33101`) | finished | `2026-07-16 00:11:37.557` (`:33130`) | 371.633 s | 352.539 s |
| 326 | 1 | `2026-07-16 00:11:47.210` (`:33202`) | finished | `2026-07-16 00:18:11.045` (`:33231`) | 383.835 s | 364.332 s |
| 327 | 6 | `2026-07-16 00:18:20.717` (`:33305`) | finished | `2026-07-16 00:24:43.292` (`:33334`) | 382.575 s | 363.970 s |
| 328 | 1 | `2026-07-16 00:24:53.081` (`:33406`) | finished | `2026-07-16 00:31:10.781` (`:33435`) | 377.700 s | 359.351 s |
| 329 | 6 | `2026-07-16 00:31:20.600` (`:33509`) | finished | `2026-07-16 00:37:41.690` (`:33538`) | 381.090 s | 361.951 s |
| 330 | 3 | `2026-07-16 00:37:50.689` (`:33610`) | finished | `2026-07-16 00:44:15.779` (`:33639`) | 385.090 s | 366.215 s |
| 331 | 1 | `2026-07-16 00:44:24.477` (`:33711`) | finished | `2026-07-16 00:50:45.122` (`:33740`) | 380.645 s | 359.835 s |
| 332 | 5 | `2026-07-16 00:50:54.866` (`:33812`) | finished | `2026-07-16 00:57:15.690` (`:33841`) | 380.824 s | 362.834 s |
| 333 | 1 | `2026-07-16 00:57:25.954` (`:33913`) | finished | `2026-07-16 01:03:53.571` (`:33942`) | 387.617 s | 367.470 s |
| 334 | 5 | `2026-07-16 01:04:03.031` (`:34016`) | finished | `2026-07-16 01:10:24.935` (`:34045`) | 381.904 s | 363.532 s |
| 335 | 6 | `2026-07-16 01:10:34.729` (`:34117`) | finished | `2026-07-16 01:16:52.088` (`:34146`) | 377.359 s | 358.319 s |
| 336 | 4 | `2026-07-16 01:17:00.824` (`:34220`) | finished | `2026-07-16 01:23:17.332` (`:34249`) | 376.508 s | 357.666 s |
| 337 | 3 | `2026-07-16 01:23:27.108` (`:34321`) | finished | `2026-07-16 01:29:56.994` (`:34350`) | 389.886 s | 370.785 s |
| 338 | 4 | `2026-07-16 01:30:06.567` (`:34422`) | finished | `2026-07-16 01:36:37.477` (`:34451`) | 390.910 s | 359.305 s |
| 339 | 2 | `2026-07-16 01:36:47.114` (`:34523`) | finished | `2026-07-16 01:43:01.266` (`:34552`) | 374.152 s | 355.365 s |
| 340 | 6 | `2026-07-16 01:43:09.802` (`:34626`) | finished | `2026-07-16 01:49:27.610` (`:34655`) | 377.808 s | 358.837 s |
| 341 | 2 | `2026-07-16 01:49:37.311` (`:34727`) | finished | `2026-07-16 01:55:58.476` (`:34756`) | 381.165 s | 361.909 s |
...
|---|---:|---|---|
| Receiver lifecycle | ambiguous | Iterations `1..341` finish. Iteration `342` starts, but learner logging ends `74.751 s` later—only `21.824 s` after target initialization/root response—shorter than the observed minimum completed receiver lifecycle (`219.739 s`, iteration 4) and completed synchronization (`200.229 s`, iteration 4). The start is unmatched in this artifact, but learner-log coverage is insufficient to classify it as a receiver failure. | [Learner Evidence](#learner-evidence); [Reconnect Episodes And Iterations](#reconnect-episodes-and-iterations) |
...
| Exact mutation composition | ambiguous | path-range and work-shape counter rows | Dirty counters do not distinguish append, modify, and remove causes. |
