...
- **Allowed in `consensus-state` and `consensus-transaction-handling` only:** `swirlds-state-api`, `swirlds-state-impl`, `swirlds-virtualmap`
...
- `consensus-state` — will move to the execution layer alongside the signed-state machinery it holds.
