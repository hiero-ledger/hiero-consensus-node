...
#
# Runs the full LongList snapshot campaign without timing the final force, writing about 33 TB of target data.
# Invocation teardown forces every target before the next write, so pending work cannot cross measurements.
...
SCRATCH_PARENT="${MODULE_DIR}/build/tmp/long-list-snapshot-without-force-campaign"
RESULTS_PARENT="${MODULE_DIR}/build/results/jmh/long-list-snapshot-without-force-campaign"
...
# Record enough source, runtime, and hardware context to compare with the forced campaign.
{
...
run_jmh() {
    ...
    local leaf_count="$4"
    local chunk_size="$5"
    local disk_cache_state="${6:-UNCHANGED}"
    local size_directory="${SCRATCH_DIR}/leaves-${leaf_count}"
    local java_options
    java_options="$(jvm_args "${leaf_count}" "${chunk_size}")"

    echo
    echo "Running ${result_name}, threadsPerLongList={${threads}}, forceToDisk=false"
...
for leaf_count in "${LEAF_COUNTS[@]}"; do
    ...
    for block in "${BLOCKS[@]}"; do
        ...
        for chunk_size in "${CHUNK_SIZES[@]}"; do
            ...
            # One fork performs one warmup and two measured single-shot writes.
            run_jmh "${result_name}" "${IMPLEMENTATIONS}" "${threads}" "${leaf_count}" "${chunk_size}"
        done
    done

    # Repeat the forced campaign's verified warm/cold LongListDisk source-cache diagnostic.
    if (( leaf_count == 1000000000 )); then
        ...
        for block in "${BLOCKS[@]}"; do
            ...
            run_jmh "${result_name}" "LongListDisk" "${threads}" "${leaf_count}" "${chunk_size}" "${cache_states}"
        done
    fi
