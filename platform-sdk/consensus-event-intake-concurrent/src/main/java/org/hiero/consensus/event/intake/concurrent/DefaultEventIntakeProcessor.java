...
public class DefaultEventIntakeProcessor implements EventIntakeProcessor {
    ...
    /**
     * Deduplication map bucketed by birth round. Eviction iterates only the round keys
     * (~20 active rounds), not every event entry — O(active rounds) instead of O(total events).
     */
    ...
    /**
     * Generation counter incremented on every roster history update. Each thread compares this
     * against its local generation to detect staleness and clear its thread-local verifier cache.
     * Only written by the roster-update wire (single writer), read by concurrent worker threads.
     */
    private final AtomicLong verifierCacheGeneration = new AtomicLong(0);

    /**
     * Per-thread verifier cache. Each worker thread maintains its own plain {@link HashMap} of
     * verifiers keyed by {@link NodeId}, avoiding all {@link ConcurrentHashMap} overhead on the
     * hot path. The cache is invalidated when the thread detects that {@link #verifierCacheGeneration}
     * has advanced past its local snapshot.
     *
     * <p>Unlike a shared {@link ConcurrentHashMap}, this approach also guarantees that each
     * verifier instance is only ever used by a single thread, so verifier implementations do
     * not need to be thread-safe.
     */
    private final ThreadLocal<LocalVerifierCache> threadLocalVerifiers =
            ThreadLocal.withInitial(LocalVerifierCache::new);

    /**
     * Thread-local holder for cached {@link BytesSignatureVerifier} instances and the generation
     * at which they were populated.
     */
    private static class LocalVerifierCache {
        long generation = -1;
        final HashMap<NodeId, BytesSignatureVerifier> verifiers = new HashMap<>();
    }
    ...
    private boolean deduplicate(@NonNull final PlatformEvent event) {
        ...
        if (signatures.add(event.getSignature())) {
            // Either exact duplicate (same descriptor and signature) or different signature duplicate
    ...
    private boolean isSignatureValid(@NonNull final PlatformEvent event) {
        ...
        return verifier != null && verifier.verify(event.getHash().getBytes(), event.getSignature());
    }

    /**
     * Look up the cached signature verifier for a given node in the given roster. On a cache miss,
     * resolves the public key from the roster and creates a verifier via the factory. The result
     * (including {@code null} for unresolvable nodes) is cached in the thread-local map so the
     * expensive resolution only happens once per (thread, node) per roster generation.
     *
    ...
    private BytesSignatureVerifier lookupVerifier(@NonNull final Roster roster, @NonNull final NodeId nodeId) {
        final LocalVerifierCache localCache = threadLocalVerifiers.get();
        final long currentGeneration = verifierCacheGeneration.get();
        if (localCache.generation != currentGeneration) {
            localCache.verifiers.clear();
            localCache.generation = currentGeneration;
        }

        // "not cached" returns false, cached as null means the node is not on the roster
        if (localCache.verifiers.containsKey(nodeId)) {
            return localCache.verifiers.get(nodeId);
        }

        final BytesSignatureVerifier verifier = createVerifier(roster, nodeId);
        localCache.verifiers.put(nodeId, verifier);
        return verifier;
    }
    ...
    private BytesSignatureVerifier createVerifier(@NonNull final Roster roster, @NonNull final NodeId nodeId) {
        ...
        return verifierFactory.apply(cert.getPublicKey());
    }
    ...
    public void updateRosterHistory(@NonNull final RosterHistory rosterHistory) {
        this.rosterHistory = Objects.requireNonNull(rosterHistory);
        // Increment generation — each worker thread will detect staleness on its next event
        // and clear its own thread-local verifier map.
        this.verifierCacheGeneration.incrementAndGet();
    }
