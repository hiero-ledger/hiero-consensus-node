...
| [ADR-008](ADR-008-replace-ngen-with-sequence-number.md)               | Replace nGen with a Monotonic Event Sequence Number and Remove nGen                                       | 2026-04-08 | Accepted | The orphan buffer stamps each released event with an ever-increasing sequence number assigned at the buffer's exit. Because `nGen` resets to 1 when an event's parents have already gone ancient (the "almost falling behind" case), breaking the per-creator monotonic ordering that event creation, consensus, sync, and `cGen` all rely on, every `nGen` consumer is migrated to the sequence number — which never resets — and `nGen` is then removed entirely. Migration is staged across separate changes (tipset done; consensus, sync, `cGen`, tools, and final removal pending).                                                                |
