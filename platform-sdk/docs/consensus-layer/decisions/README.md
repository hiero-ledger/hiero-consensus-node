...
| [ADR-008](ADR-008-replace-ngen-with-sequence-number.md)                   | Replace nGen with a Monotonic Event Sequence Number and Remove nGen                                       | 2026-04-08 | Accepted | The orphan buffer stamps each released event with an ever-increasing sequence number at its exit. Because `nGen` resets to 1 when an event's parents are already ancient, breaking the per-creator monotonic ordering its consumers rely on, every `nGen` consumer is migrated to the sequence number and `nGen` is then removed.                                                                         |
