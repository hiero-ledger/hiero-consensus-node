...
| [ADR-007](ADR-007-save-reconnect-state-before-resuming-event-creation.md) | Save the Reconnect State to Disk Before Resuming Event Creation                                           | 2026-06-04 | Accepted | After a reconnect, the learner writes the received signed state to disk before it is allowed to resume event creation, so a node that crashes immediately after reconnecting recovers its latest self event from its own saved state and cannot branch.                                                                                                                                                   |
| [ADR-008](ADR-008-replace-ngen-with-sequence-number.md)                   | Adopt a Monotonic Event Sequence Number as the Local Ordering Key, Retaining nGen for Graph-Height Consumers | 2026-04-08 | Accepted | The orphan buffer stamps each released event with a non-resetting sequence number at its exit, used as the local ordering key for consumers that need only a topological order (tipset advancement, sync). `nGen` is retained for consumers that need graph height — the consensus-relevant threshold and the event-creator `lastSelfEvent` — after keying those on the sequence number caused an ISS (SCN-002, #26319) and branching (SCN-003, #26376). Full `nGen` removal is dropped.                                                                         |
