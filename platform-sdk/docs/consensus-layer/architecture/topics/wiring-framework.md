...
A single `WiringModel` instance flows through the whole consensus layer: it is constructed once in `ConsensusLayerFactory` (via `WiringModelBuilder`), threaded into each module's `initialize(model, configuration, ...)` so the module can build its internal `ComponentWiring`s on it, and passed to `ConsensusLayerWiring.wire(...)` so inter-module solders share the same graph. Reaching back into the model directly happens in two places: `ConsensusLayerWiring` solders the model-owned health-monitor wire to its consumers, and each module builds its own heartbeat wire off the model.
