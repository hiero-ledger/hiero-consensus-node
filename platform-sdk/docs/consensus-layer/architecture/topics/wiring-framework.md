...
A `TaskScheduler<OUT>` ([`TaskScheduler`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/schedulers/TaskScheduler.java)) owns a queue, a thread-execution policy, and a built-in primary `OutputWire<OUT>`. The framework primitive for obtaining one is `WiringModel.schedulerBuilder(name).withType(...).build()`. Consensus-layer code rarely calls that directly: the standard wrapper is `ComponentWiring<COMPONENT, OUT>` ([`ComponentWiring`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/component/ComponentWiring.java)), which combines a scheduler with method-reference-based input-wire creation and a deferred binding step.

`TaskSchedulerType` ([`TaskSchedulerType`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/schedulers/builders/TaskSchedulerType.java)) chooses the threading policy. Six values exist; see the enum's javadoc for the authoritative descriptions:
...
The `Class` argument is the component's interface (which `ComponentWiring` requires so it can resolve method references reflectively). `wiringConfig.eventDeduplicator()` returns a `TaskSchedulerConfiguration` ([`TaskSchedulerConfiguration`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/schedulers/builders/TaskSchedulerConfiguration.java)) — the scheduler's type, capacity, flushing, and other options come from configuration.
...
`flush()` is opt-in via `withFlushingEnabled(true)` and calling it on a scheduler that did not opt in throws `UnsupportedOperationException` ([`TaskScheduler`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/schedulers/TaskScheduler.java)). It waits for the scheduler's on-ramp counter to drain to zero — every task that has entered the scheduler has finished processing — and does **not** transitively flush downstream-soldered schedulers ([`SequentialTaskScheduler`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/schedulers/internal/SequentialTaskScheduler.java) / [`ConcurrentTaskScheduler`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/schedulers/internal/ConcurrentTaskScheduler.java)). `flush()` is often paired with `startSquelching()` / `stopSquelching()`, the scheduler's "drop new tasks" toggle, when a caller must both stop accepting new work and drain the backlog: squelch new arrivals, flush the existing backlog, then stop squelching to resume acceptance. A module-wide flush walks each `ComponentWiring` in turn — [`DefaultEventIntakeModule.flush()`](../../../../consensus-event-intake-impl/src/main/java/org/hiero/consensus/event/intake/impl/DefaultEventIntakeModule.java) flushes its components in order — while squelching is applied separately by the callers that need it (for example the reconnect clear path in [`ReconnectCoordinator`](../../../../consensus-reconnect-impl/src/main/java/org/hiero/consensus/reconnect/impl/ReconnectCoordinator.java)). The combined pattern is intricate and a known candidate for future simplification.

The default `UncaughtExceptionHandler` (installed when none is supplied to the builder) logs at ERROR via log4j and the scheduler continues with the next task ([`ExceptionHandlers`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/schedulers/ExceptionHandlers.java)); `DIRECT` / `DIRECT_THREADSAFE` schedulers, having no queue, invoke the handler on the calling thread but otherwise behave the same.

### InputWire and OutputWire

`InputWire<IN>` ([`InputWire`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/wires/input/InputWire.java)) is the entry point of a component. It exposes three put-paths (`put(data)`, `offer(data)`, and `inject(data)`) that an edge selects between via its `SolderType`; their handoff semantics are described under [Soldering](#soldering).

Normally, modules do not call `BindableInputWire.bind(...)` directly. They obtain an `InputWire` via `componentWiring.getInputWire(MethodReference)`; `ComponentWiring` lazily creates a `BindableInputWire<IN, OUT>` ([`BindableInputWire`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/wires/input/BindableInputWire.java)) for the targeted method on first use, caches it, and binds it later when `componentWiring.bind(component)` is called. The framework method `BindableInputWire.bind(Function<IN, OUT>)` / `bindConsumer(Consumer<IN>)` is therefore an implementation detail that consensus-code normally doesn't touch directly.

`OutputWire<OUT>` ([`OutputWire`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/wires/output/OutputWire.java)) is the source of every connection out of a component. Soldering, filters, transformers, and splitters are all methods on `OutputWire`. A scheduler always owns one primary output wire (`getOutputWire()`); secondary output wires for additional output streams are created with `buildSecondaryOutputWire()`.

The two are fed differently: the scheduler forwards a handler's return value onto the primary wire automatically ([`BindableInputWire`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/wires/input/BindableInputWire.java)), whereas a secondary wire is pushed explicitly by the component's own code — a rare case, used only when business logic must emit something a return value cannot carry. Primary and secondary are output-wire terms; there is no secondary input wire.

### Soldering

Soldering connects an `OutputWire<T>` to one or more `InputWire<T>`s. The call is `outputWire.solderTo(inputWire)` (defaults to `SolderType.PUT`) or `outputWire.solderTo(inputWire, solderType)`. `SolderType` ([`SolderType`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/wires/SolderType.java)) has three values:
...
Modules also use a free-standing `WireTransformer<A, B>` ([`WireTransformer`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/transformers/WireTransformer.java)) as a fan-out dispatcher — a single input wire whose output wire is soldered to several consumers. [`DefaultEventIntakeModule`](../../../../consensus-event-intake-impl/src/main/java/org/hiero/consensus/event/intake/impl/DefaultEventIntakeModule.java) constructs identity dispatchers for event-window updates and clear commands so each value is delivered to every component that subscribes:
...
`WireListSplitter<T>` ([`WireListSplitter`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/transformers/WireListSplitter.java)) is the framework's list-splitting transformer: a `List<T>` arriving on its input wire produces one task per element on its output wire. It is built via `OutputWire.buildSplitter(...)`, and `ComponentWiring` exposes it as `getSplitOutput()` for any component whose primary output type is a list. For example, [`DefaultEventIntakeModule.validatedEventsOutputWire()`](../../../../consensus-event-intake-impl/src/main/java/org/hiero/consensus/event/intake/impl/DefaultEventIntakeModule.java) returns the orphan buffer's split output so that downstream components receive one event at a time rather than a batch.

### WiringModel

`WiringModel` ([`WiringModel`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/model/WiringModel.java)) owns every scheduler and every wire in a process. It is an interface (constructed via [`WiringModelBuilder`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/model/WiringModelBuilder.java)); concrete implementations are [`StandardWiringModel`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/model/StandardWiringModel.java) and [`DeterministicWiringModel`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/model/DeterministicWiringModel.java). Its responsibilities are:
...
A few specifics worth pinning down. The default `unhandledTaskCapacity` on the framework builder is `1` ([`AbstractTaskSchedulerBuilder`](../../../../consensus-wiring-framework/src/main/java/org/hiero/consensus/wiring/framework/schedulers/builders/internal/AbstractTaskSchedulerBuilder.java)); consensus-layer schedulers always override this via `TaskSchedulerConfiguration`, so the default is effectively never used in production. The capacity is per-scheduler: every input wire on the same scheduler shares a single on-ramp counter, so the limit applies to total backlog across all inputs, not independently per wire.
