...
- `ignorePreconsensusSignatures`: when true, peer signature
  ...
  been called. **Testing only** (set via the
...
- PCES-replay rounds do not produce a `StateSignatureTransaction`
  (state signing is skipped during replay), so no self-hash is ever
  reported for them. They are tracked but pass out of the window
  via `LACK_OF_DATA`.

## ISS handling

`DefaultIssHandler#issObserved` consumes each `IssNotification` and
chooses a response based on `StateConfig`
([`StateConfig.java`](../../../../consensus-state/src/main/java/org/hiero/consensus/state/config/StateConfig.java)):

|        Type        | Default response | With `haltOnAnyIss` |              With `automatedSelfIssRecovery`               | With `haltOnCatastrophicIss` |
|--------------------|------------------|---------------------|------------------------------------------------------------|------------------------------|
| `OTHER_ISS`        | record metric    | halt                | —                                                          | —                            |
| `SELF_ISS`         | record metric    | halt                | restart via `FatalErrorConsumer` with `SystemExitCode.ISS` | —                            |
| `CATASTROPHIC_ISS` | record metric    | halt                | —                                                          | halt                         |

The handler is *one-shot*: once `halted` becomes true, subsequent ISS
notifications are ignored.

For `SELF_ISS` and `CATASTROPHIC_ISS`, the round number is written to a
persistent `IssScratchpad` ([`IssScratchpad.java`](../../../../swirlds-platform-core/src/main/java/com/swirlds/platform/state/iss/IssScratchpad.java))
...
The `DefaultIssDetector` also rate-limits its log output via three
separate `RateLimiter`s — one each for self-ISS, catastrophic-ISS, and
lack-of-data — sharing a minimum interval of
`stateConfig.secondsBetweenIssLogs` (default 300s).

## Wiring

In [`PlatformWiring.java`](../../../../swirlds-platform-core/src/main/java/com/swirlds/platform/wiring/PlatformWiring.java)
...
handler's system-transaction stream. Its `getSplitOutput()` is
soldered to:
