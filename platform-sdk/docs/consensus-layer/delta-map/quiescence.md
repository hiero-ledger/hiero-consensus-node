...
is canonical. The feature's division of labour — Execution decides,
Consensus obeys a single command — already matches the proposal's
minimal-boundary philosophy. The concrete gaps are that the proposed
public Consensus API has no quiescence operation, and that quiescence's
interaction with the `nextRound` pull model is unspecified.
