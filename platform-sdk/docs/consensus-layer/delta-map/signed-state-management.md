...
lifecycle sits in the consensus-layer `consensus-state` module, with
Execution a passive observer. The extraction of state types and
components into `consensus-state` is internal modularization, not
progress on (or movement away from) the proposed end state.
