...
| `nextRound` module-level backpressure | Execution paces Consensus by pulling each round; Consensus never blocks on slow Execution (§ Slow Execution). | [§ Slow Execution](../../proposals/consensus-layer/Consensus-Layer.md#slow-execution), [§ nextRound](../../proposals/consensus-layer/Consensus-Layer.md#nextround) | Consensus output is soldered directly to execution-side handlers; no `nextRound` exists. | **not-started** | `PlatformWiring` (`swirlds-platform-core`) — push wiring intact, no `nextRound` in code |
