...
| ISS response under the Execution-owned lifecycle | With Execution owning state and restart, the response to an ISS is an Execution concern. | The handler lives in platform-core, and the offline ISS-recovery procedure remains part of platform startup — the same gap recorded in [restart-and-pces.md](restart-and-pces.md). | **not-started** | `DefaultIssHandler` (`swirlds-platform-core`) — pre-proposal shape intact |
