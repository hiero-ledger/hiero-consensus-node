...
public final class HashgraphInfo {
    ...
    private long numVoteD = 0; // metric: how many rounds had voteD==2
    private long numUsedCoin = 0; // metric: how many rounds had usedCoin==true
    private long numPrevJudgesCopied = 0; // metric: how many rounds set prevJudgesCopied==true
    ...
    private int parentsCapacity = 0; // rough estimate of current capacity: largest size stored since last reset
    private int judgesCapacity = 0; // rough estimate of current capacity
    private int consensusEventsCapacity = 0; // rough estimate of current capacity
     ...
     * Never change an array element inside this record.
     */
     ...
     * <p>
     * This should never be instantiated by code outside this file. For round 1, use
     * {@link HashgraphInfo#FIRST_ROUND_INFO_PREV}. For round R>1, use the one in
     * {EventInfo#UpdateResults}.nextRoundInfoPrev, which was returned by a call to
     * {@link EventInfo#update} in round R-1.
     * <p>
     * Never change an array element inside this record.
     */
    ...
    public static final class EventInfo {
        ...
        private int creatorIndex; // index into nodes[] for this event's creator, from the birth round's nodes[]
        ...
        private boolean wasJudge; // has this event ever been a judge? (Reconnecting nodes may have a different value).
         ...
         * then it is ok to pass in an array of all nulls, or an empty array. After an EventInfo is instantiated,
         * never change any element of its parents array.
         * <p>
         * The payload parameter is not used here but could be used by the caller to hold data such as a list of
         * transactions. The eventID parameter is not used for consensus but can be useful for
         * testing / debugging / monitoring. The coin parameter should be randomly generated when the event is created,
         * before it is signed. It should be a uniform random int in the range [0,numNodes], inclusive, based on the
         * number of nodes for the round that matches the birthRound. But it is also OK to choose it as a uniform
         * random int. It should be generated by a CSPRNG seeded by a TRNG. A useful metric for a healthy hashgraph
         * is that a large fraction of the events should have {@link EventInfo#getWasJudge getWasJudge()}==true
         * at the moment they become ancient.
         *
         ...
         * @param creatorNodeID the nodeID of the creatorNodeID of this event
         * @param timeCreated   when this event was created, as claimed by its creatorNodeID node
         * @param birthRound    birth round of this event (which was the pending round at the moment it was created)
         * @param coin          the coin flip results for this event
         * @param parents       array of parents, in the same order as in the signed event that is gossiped.
         * @param payload       an optional payload that is ignored by the consensus algorithm. It can be null.
         */
        public EventInfo(
        ...
        public boolean getWasJudge() { return wasJudge; }
        // spotless:on
        ...
        public UpdateResults update(@NonNull RoundInfo roundInfo, @NonNull RoundInfoPrev roundInfoPrev) {
            ...
            // function parents  /--------------------------------------------------------------------------------
            // put in the h.parents list only parents that are a descendent of at least one judge in the prev round
            h.parents.clear();
            ...
            { // function votingRound /---------------------------------------------------------------------------
                ...
                if (r.pendingRound == p + 1) {
                    ...
                    for (int m = 0; m < r.nodes.length; m++) {
                        if (((creatorIndex == m) && (selfParent != null) && (selfParent.votingRound == p))
                                || ((creatorIndex != m) && (lastSee[m] != null) && (lastSee[m].votingRound == p))) {
                            stakeSum += r.stake[m];
                        }
                    }
            ...
            if (witness && (votingRound == r.pendingRound) && (creatorIndex >= 0)) {
            ...
            if (prevJudgesCopied) { // if not a supermajority, copy previous judges. This is VERY rare.
                ...
                for (EventInfo judge : h.judges) {
                    ...
                    judge.wasJudge = true; // set true in case a reconnecting node didn't already set it.
                }
            } else {
                for (int m = 0; m < numNodes; m++) {
                    if (voteE[m] != null) {
                        h.judges.add(voteE[m]);
                        voteE[m].wasJudge = true;
                    }
                }
            }
