...
public final class IssStateJsonCodec implements JsonCodec<IssState> {
        ...
        boolean temp_recoverable_on_restart = false;
            ...
            for (JSONParser.PairContext kvPair : root.pair()) {
                switch (kvPair.STRING().getText()) {
                    case "issState" /* [1] */ : temp_iss_state = parseLong(kvPair.value()); break;
    case "recoverableOnRestart" /* [2] */ : temp_recoverable_on_restart = parseBoolean(kvPair.value()); break;
    ...
    public String toJSON(@NonNull IssState data, String indent, boolean inline) {
        ...
        if (data.recoverableOnRestart() != false) fieldLines.add(field("recoverableOnRestart", data.recoverableOnRestart()));
