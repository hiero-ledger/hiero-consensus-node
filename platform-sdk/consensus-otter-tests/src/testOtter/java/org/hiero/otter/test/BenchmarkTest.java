...
public class BenchmarkTest {
    ...
    void testBenchmarkLatency(@NonNull final TestEnvironment env) {
        ...
        for (int i = 0; i < TRANSACTION_COUNT; i++) {
            final OtterTransaction tx = TransactionFactory.createBenchmarkTransaction(NONCE_GENERATOR.incrementAndGet());
    ...
    private static long countBenchmarkLogs(@NonNull final List<StructuredLog> logs) {
        return logs.stream()
                .filter(log -> log.marker() != null && "BENCHMARK".equals(log.marker().getName()))
                .count();
    }
}