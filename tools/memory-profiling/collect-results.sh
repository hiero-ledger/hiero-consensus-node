...
# CSV header
echo "test_name,peak_mib,peak_gib,end_mib,end_gib,duration_secs,duration_min,exit_code,timestamp,git_sha" > "$SUMMARY"
...
for f in "$RESULTS_DIR"/*.txt; do
    ...
    end_mib=$(grep '^end_mib=' "$f" 2>/dev/null | cut -d= -f2)
    end_gib=$(grep '^end_gib=' "$f" 2>/dev/null | cut -d= -f2)
    ...
    if [[ -n "${duration_secs}" ]]; then
        duration_min=$(awk "BEGIN {printf \"%.1f\", ${duration_secs}/60}")
    else
        duration_min=""
    fi
    echo "${test_name},${peak_mib},${peak_gib},${end_mib},${end_gib:-},${duration_secs:-},${duration_min},${exit_code},${timestamp},${git_sha}" >> "$SUMMARY"
...
sort "$SUMMARY" | tail -n +2 | while IFS=, read -r name pmib pgib cmib dur dmin ecode ts sha; do
    printf "%-45s %10s %10s %10s %10s %6s\n" "$name" "$pmib" "$pgib" "${dur:-n/a}" "${dmin:-n/a}" "$ecode"
done
