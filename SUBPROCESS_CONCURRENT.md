...
- `hapiTestCrypto` depends on `testSubprocessConcurrent` (parallel test classes)
- `testSubprocessConcurrent` excludes XTS tests except `CONCURRENT_SUBPROCESS_VALIDATION`
...
- Uses `@Isolated` to ensure it runs last
- Runs log validation first (reads files, network still up)
- Runs stream validation second (freezes the network)
- Tagged with `CONCURRENT_SUBPROCESS_VALIDATION` to bypass XTS exclusion
...
- `@OrderedInIsolation` classes are tagged with XTS and run separately via `hapiTestCryptoXts`
- `testSubprocessConcurrent` excludes XTS to avoid state conflicts during parallel execution

## Test Flow

1. Network starts
2. 367 crypto tests run concurrently
