...
public class CustomSelfDestructOperation extends AbstractOperation {
    ...
    public OperationResult execute(@NonNull final MessageFrame frame, @NonNull final EVM evm) {
        try {
            ...
            return execute(frame, beneficiaryAddress);
        } catch (UnderflowException ignore) {
            return UNDERFLOW_RESPONSE;
        }
    }

    // Bonneville entry point, with stack access done outside the "execute".
    public OperationResult execute(@NonNull MessageFrame frame, Address beneficiaryAddress) {
        ...
        final var maybeReasonToHalt = validateHederaRestrictionsOnBeneficiary(tbdAddress, beneficiaryAddress, frame)
                ...
                .or(() -> proxyWorldUpdater.tryTrackingSelfDestructBeneficiary(tbdAddress, beneficiaryAddress, frame))
    ...
    protected @NonNull Optional<ExceptionalHaltReason> validateHederaRestrictionsOnBeneficiary(
        ...
        return Stream.of(
                ...
                .findFirst()
                .flatMap(op -> {
                    FrameUtils.invalidAddressContext(frame).set(beneficiary, InvalidAddressType.NON_CALL_TARGET);
