 ...
 * for the Ethereum verifier system contract.
 */
@Singleton
public class EthereumVerifyBundleTranslator extends AbstractCallTranslator<EthereumVerifierCallAttempt> {
    private static final Logger log = LogManager.getLogger(EthereumVerifyBundleTranslator.class);
    ...
    public static final SystemContractMethod VERIFY_BUNDLE =
            SystemContractMethod.declare("verifyBundle(bytes,bytes)", "(bytes)").withCategories(Category.ETHEREUM);
    ...
    @Inject
    public EthereumVerifyBundleTranslator(
    ...
    public Call callFrom(@NonNull final EthereumVerifierCallAttempt attempt) {
        if (attempt.isMethod(VERIFY_BUNDLE_V2).isPresent()) {
            try {
                ...
                log.warn(
                        "EthereumVerifyBundleTranslator failed to decode verifyBundle V2 calldata: input={} bytes ({})",
