...
public class ClprQueueEnqueueMessageResponseCall extends AbstractCall {
    private static final Logger log = LogManager.getLogger(ClprQueueEnqueueMessageResponseCall.class);
    ...
    public @NonNull PricedResult execute(@NonNull final MessageFrame frame) {
        ...
        // TEMP-OBSERVABILITY (delete before production): traces queue-system-contract entry for CLPR response enqueue.
        log.info("CLPR_OBS|component=clpr_queue_enqueue_message_response_call|stage=execute_start|sender={}", senderId);
        ...
        try {
            routeHeader = ClprRouteHeaderCodec.decodeResponseHeaderFromResponseTuple(clprMessageResponse);
        } catch (final RuntimeException ignore) {
        ...
        // TEMP-OBSERVABILITY (delete before production): traces decoded response route envelope before queue append.
        log.info(
                "CLPR_OBS|component=clpr_queue_enqueue_message_response_call|stage=route_decoded|remoteLedgerId={}|routeVersion={}|originalMessageId={}",
        ...
        final var remoteLedgerId = ClprLedgerId.newBuilder()
                ...
                .build();
        final var canonicalResponseData = stripSelector(callData);
        final var payload = ClprMessagePayload.newBuilder()
                .messageReply(ClprMessageReply.newBuilder()
                        .messageId(originalMessageId)
                        .messageReplyData(com.hedera.pbj.runtime.io.buffer.Bytes.wrap(canonicalResponseData))
                        .build())
                .build();
        ...
        final long expectedResponseId = queueMetadata.nextMessageId();
        if (expectedResponseId < 1) {
            return revertWithReason(INVALID_TRANSACTION_BODY.protoName());
        }
        // TEMP-OBSERVABILITY (delete before production): traces synthetic enqueue dispatch intent for response payload.
        log.info(
                "CLPR_OBS|component=clpr_queue_enqueue_message_response_call|stage=dispatch_enqueue|remoteLedgerId={}|expectedMessageId={}|originalMessageId={}",
                Bytes.wrap(routeHeader.remoteLedgerId()).toHexString(),
                expectedResponseId,
                originalMessageId);
        ...
        final var enqueueOp = ClprEnqueueMessageTransactionBody.newBuilder()
                ...
                .payload(payload)
                .expectedMessageId(expectedResponseId)
                .build();
        final var synthBody =
        ...
        log.info(
                "CLPR_OBS|component=clpr_queue_enqueue_message_response_call|stage=dispatch_result|status={}",
        ...
        final var output = ClprQueueEnqueueMessageResponseTranslator.ENQUEUE_MESSAGE_RESPONSE
                .getOutputs()
                .encode(Tuple.singleton(BigInteger.valueOf(expectedResponseId)));
        // TEMP-OBSERVABILITY (delete before production): confirms successful queue enqueue response id.
        log.info(
                "CLPR_OBS|component=clpr_queue_enqueue_message_response_call|stage=execute_success|assignedMessageId={}|originalMessageId={}",
                expectedResponseId,
                originalMessageId);
    ...
    private PricedResult revertWithReason(@NonNull final String reason) {
        ...
        log.info(
                "CLPR_OBS|component=clpr_queue_enqueue_message_response_call|stage=execute_revert|reason={}",
