...
package com.hedera.node.app.service.contract.impl.test.exec.systemcontracts.clpr.queue.enqueuemessage;
...
class ClprQueueEnqueueMessageTranslatorTest extends CallAttemptTestBase {
    private static final TupleType<Tuple> ROUTE_HEADER_TYPE = TupleType.parse("(uint8,bytes32,address,address)");
    private static final TupleType<Tuple> BAD_ROUTE_HEADER_TYPE = TupleType.parse("(uint8,bytes32,address)");
    ...
    private ClprQueueEnqueueMessageTranslator subject;

    @BeforeEach
    void setUp() {
        subject = new ClprQueueEnqueueMessageTranslator(systemContractMethodRegistry, contractMetrics);
    ...
    void malformedCalldataReturnsTypedReason() {
        final var call = subject.callFrom(attemptWithInput(Bytes.fromHexString("8cfaaa60")));
    ...
    @Test
    void requestPathEnqueuesAndReturnsAssignedMessageId() {
        given(queueStore.get(any())).willReturn(initialQueueMetadata(REMOTE_LEDGER_ID));

        final var input = validEnqueueInput((byte) 1, REMOTE_LEDGER_ID);
        final var call = subject.callFrom(attemptWithInput(input));
        final var result = call.execute(frame);
        final var decodedOutput = ClprQueueEnqueueMessageTranslator.ENQUEUE_MESSAGE
                .getOutputs()
                .decode(result.fullResult().output().toArrayUnsafe());

        assertThat(decodedOutput.get(0) instanceof BigInteger).isTrue();
        assertThat(((Number) decodedOutput.get(0)).longValue()).isEqualTo(1L);
        ...
        assertThat(op.payloadOrThrow().messageOrThrow().messageData())
    ...
    void unsupportedRouteVersionReturnsTypedReason() {
        final var call = subject.callFrom(attemptWithInput(validEnqueueInput((byte) 2, REMOTE_LEDGER_ID)));
        final var result = call.execute(frame);
        assertThat(new String(result.fullResult().output().toArrayUnsafe(), UTF_8))
    ...
    void zeroRouteLedgerIdReturnsTypedReason() {
        final var call = subject.callFrom(attemptWithInput(validEnqueueInput((byte) 1, new byte[32])));
        final var result = call.execute(frame);
        assertThat(new String(result.fullResult().output().toArrayUnsafe(), UTF_8))
    ...
    void malformedRouteHeaderBytesReturnsTypedReason() {
        final var call =
                subject.callFrom(attemptWithInput(validEnqueueInputWithRouteHeader(new byte[] {0x01, 0x02, 0x03})));
        final var result = call.execute(frame);
        assertThat(new String(result.fullResult().output().toArrayUnsafe(), UTF_8))
    ...
    void wrongRouteHeaderTupleShapeReturnsTypedReason() {
        final var badRouteHeaderBuffer =
                BAD_ROUTE_HEADER_TYPE.encode(Tuple.of(1, REMOTE_LEDGER_ID, asHeadlongAddress(OWNER_BESU_ADDRESS)));
        final var badRouteHeaderBytes = new byte[badRouteHeaderBuffer.remaining()];
        badRouteHeaderBuffer.get(badRouteHeaderBytes);

        final var call = subject.callFrom(attemptWithInput(validEnqueueInputWithRouteHeader(badRouteHeaderBytes)));
        final var result = call.execute(frame);
        assertThat(new String(result.fullResult().output().toArrayUnsafe(), UTF_8))
                .isEqualTo("CLPR_QUEUE_BAD_ROUTE_ENVELOPE");
    }
    ...
    private Bytes validEnqueueInput(final byte routeVersion, final byte[] routeLedgerId) {
        final var routeHeaderBuffer = ROUTE_HEADER_TYPE.encode(Tuple.of(
                toUnsignedInt(routeVersion),
                routeLedgerId,
        ...
        return validEnqueueInputWithRouteHeader(routeHeaderBytes);
    }

    private Bytes validEnqueueInputWithRouteHeader(final byte[] routeHeaderBytes) {
        ...
        final var clprMessage = Tuple.of(
                ...
                connectorMessage,
                middlewareMessage);
        return Bytes.wrapByteBuffer(ClprQueueEnqueueMessageTranslator.ENQUEUE_MESSAGE.encodeCallWithArgs(clprMessage));
    }
