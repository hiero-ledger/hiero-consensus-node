...
class SeiVerifierCallFactoryTest extends CallTestBase {
    @Mock
    ...
    private SeiVerifierCallFactory subject;

    @BeforeEach
    void setUp() {
        subject = new SeiVerifierCallFactory(syntheticIds, addressChecks, verificationStrategies, List.of(), registry);
    }
    ...
    void createsAttemptFromFrameContext() {
        ...
        final var contractId = ContractID.newBuilder()
                .contractNum(numberOfLongZero(Address.fromHexString(SEI_VERIFIER_EVM_ADDRESS)))
                .build();
        ...
        given(frame.getRecipientAddress()).willReturn(Address.fromHexString(SEI_VERIFIER_EVM_ADDRESS));
