...
contract LambdaplexHook is IHieroAccountAllowanceHook {
    ...
    function _oracleFactor(
        ...
        (address tokenA, address tokenB) = supra.getPair(pi.pairs[0]);
        require(
            (inToken == tokenA && outToken == tokenB) ||
            (inToken == tokenB && outToken == tokenA),
            "oracle: wrong pair"
        );

        uint256 decimals = pi.decimal[0];
        require(decimals < 39, "oracle: decimals");
        uint256 oraclePrice = pi.prices[0];
        uint256 scale = 10 ** decimals;

        // exec price from order's num/den, oriented to oracle (A,B)
        uint256 nmr = d.numerator();
        uint256 dnm = d.denominator();
        uint256 execPrice = (inToken == tokenA)
...
// ------------------------------- Apply one order (small scope) -------------------------------
    ...
    function checkTokenCreditAndDebit(
        ...
        address outToken,
        address inToken
    ) pure public returns (int64 sumOut, int64 sumIn) {
