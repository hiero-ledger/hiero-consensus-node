...
class BatchConstraintsNegative {
    ...
    public Stream<DynamicTest> resubmitAfterInsufficientPayerBalance() {
        return hapiTest(
                ...
                cryptoTransfer(movingHbar(FIVE_HBARS).between(GENESIS, "alice")).payingWith(GENESIS),
                // resubmit the batch
                atomicBatch(
                                cryptoCreate("foo")
                                        .txnId("innerTxn1")
...
class FeesNegative {
    ...
    public Stream<DynamicTest> failingBatchStillChargesFees() {
        return hapiTest(
                ...
                // batch txn
                atomicBatch(
                                cryptoTransfer(TokenMovement.moving(1, "ftA").between("Bob", "receiver"))
                                        .withProtoStructure(TxnProtoStructure.NORMALIZED)
                                        .batchKey("Alice")
                                        .payingWith("Bob")
                                        .signedBy("Bob"),
                                // will fail because receiver is not associated with ftC
                                cryptoTransfer(TokenMovement.moving(1, "ftC").between("Bob", "receiver"))
                                        .withProtoStructure(TxnProtoStructure.NORMALIZED)
                                        .batchKey("Alice")
                                        .payingWith("Bob")
                                        .signedBy("Bob"))
