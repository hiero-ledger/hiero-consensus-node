                    ...
                    parts.outputIfPresent(TransactionOutput.TransactionOneOfType.ETHEREUM_CALL)
                            ...
                            .ifPresent(ethTxOutput -> {
                                if (ethTxOutput.ethereumHash().length() > 0) {
                                    ...
                                    final var ethTxData = EthTxData.populateEthTxData(parts.body().ethereumTransactionOrThrow().ethereumData().toByteArray());
                                    recordBuilder.ethereumHash(Bytes.wrap(requireNonNull(ethTxData).getEthereumHash()));
                                }
                                final var result =
                                        switch (ethTxOutput.txnResult().kind()) {
                                            ...
                                            case EVM_CALL_TRANSACTION_RESULT -> {
                                                final var derivedBuilder =
                                                        resultBuilderFrom(ethTxOutput.evmCallTransactionResultOrThrow());
                                                ...
                                                baseTranslator.addSignerNonce(ethTxOutput.evmCallTransactionResultOrThrow().senderId(), derivedBuilder, remainingStateChanges);
                                                final var fnResult = derivedBuilder.build();
                                                recordBuilder.contractCallResult(fnResult);
                                                yield fnResult;
                                            }
                                            case EVM_CREATE_TRANSACTION_RESULT -> {
                                                final var txCreateResult = ethTxOutput.evmCreateTransactionResultOrThrow();
                                                ...
                                                if (parts.status() == SUCCESS) {
                                                    ...
                                                    baseTranslator.addSignerNonce(txCreateResult.senderId(), derivedBuilder, remainingStateChanges);
