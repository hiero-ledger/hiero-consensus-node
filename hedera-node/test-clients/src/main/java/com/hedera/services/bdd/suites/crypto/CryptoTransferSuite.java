...
import static com.hedera.services.bdd.suites.contract.Utils.sdec;
...
import static com.hedera.services.bdd.suites.crypto.AutoAccountCreationSuite.AutoAccountCreate.A_TOKEN;
...
public class CryptoTransferSuite {
    ...
    @Nested
    @DisplayName("Crypto Transfer Suite")
            ...
            return hapiTest(
                    ...
                    sourcing(() -> getTxnRecord(HBAR_XFER)
                            .hasPriority(recordWith()
                                    .transfers(
                                            including(tinyBarsFromTo(partyLiteral.get(), counterLiteral.get(), 2))))),
        ...
        final Stream<DynamicTest> testTransferToSystemAccounts() {
            ...
            for (int i = 0; i < systemAccounts.size(); i++) {
                ...
                opsArray[i] = contractCall(
                                contract, "sendViaTransfer", mirrorAddrParamFunction(systemAccounts.get(index)))
                        .payingWith(SENDER)
                        .sending(ONE_HBAR * 10)
                        .gas(100000)
    ...
    class NftTransfer {
        ...
        private static HapiSpecOperation createTokenTypes() {
            return blockingOrder(
                    createAccounts("token-treasury-", NUM_TOKEN_TYPES),
                    parFor(0, NUM_TOKEN_TYPES, id -> tokenCreate(tokenTypeName(id))
                            .tokenType(NON_FUNGIBLE_UNIQUE)
                            .supplyKey(KEY)
                            .initialSupply(0L)
        ...
        private static SpecOperation associateAccountsWithTokenTypes() {
            String[] tokenNames = IntStream.range(0, NUM_TOKEN_TYPES)
                    .mapToObj(NftTransfer::tokenTypeName)
                    .toArray(String[]::new);
            return parFor(0, NUM_ACCOUNTS, id -> tokenAssociate(userAccountName(id), tokenNames)
                    .noLogging());
        }
        ...
        private static HapiSpecOperation createAccountsAndNfts() {
            return blockingOrder(
                    // Create user accounts to partake in crypto transfers
                    inParallel(createAccounts(USER_ACCOUNT_PREFIX, NUM_ACCOUNTS), createTokenTypes()),
                    inParallel(
                            associateAccountsWithTokenTypes(),
                            parFor(0, NUM_TOKEN_TYPES, id -> mintTokensFor(tokenTypeName(id), NUM_ACCOUNTS))));
        }
        ...
        private static HapiSpecOperation seqFor(int from, int to, IntFunction<HapiSpecOperation> functionToRun) {
            return blockingOrder(
                    IntStream.range(from, to).mapToObj(functionToRun).toArray(HapiSpecOperation[]::new));
        }

        private static SpecOperation transferInitial() {
            return inParallel(IntStream.range(0, NUM_ACCOUNTS)
                    .mapToObj(accountId -> opsFor(0, NUM_TOKEN_TYPES, tokenId -> cryptoTransfer(
        ...
        private static HapiSpecOperation transferRound(int roundNum) {
            ...
            final int halfAccounts = NUM_ACCOUNTS / 2;
        ...
        final Stream<DynamicTest> transferNfts() {
            return hapiTest(setupNftTest(), transferInitial(), seqFor(0, NUM_ROUNDS, NftTransfer::transferRound));
        }
    }
}
