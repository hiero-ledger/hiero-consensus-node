...
public class BuildUpgradeZipOp extends UtilOp {
    ...
    @Nullable
    private final String targetServicesVersion;

    public BuildUpgradeZipOp(@NonNull final Path path) {
        this(path, null);
    }

    public BuildUpgradeZipOp(@NonNull final Path path, @Nullable final String targetServicesVersion) {
        this.path = requireNonNull(path);
        this.targetServicesVersion = targetServicesVersion;
    }
    ...
    protected boolean submitOp(@NonNull final HapiSpec spec) throws Throwable {
        try {
            ...
            if (targetServicesVersion != null) {
                writeSemanticVersionProperties(targetServicesVersion);
            }
    ...
    private static void writeSemanticVersionProperties(@NonNull final String targetServicesVersion) throws IOException {
        final var properties = loadSemanticVersionProperties();
        properties.setProperty(SERVICES_VERSION_PROPERTY, requireNonNull(targetServicesVersion));
        properties.setProperty(HAPI_PROTO_VERSION_PROPERTY, targetServicesVersion);
        try (final var out = Files.newOutputStream(UPGRADE_ZIP_PATH.resolve(SEMANTIC_VERSION_RESOURCE))) {
            properties.store(out, "Generated by BuildUpgradeZipOp");
        }
    }

    private static Properties loadSemanticVersionProperties() throws IOException {
        final var properties = new Properties();
        try (final var in =
                Thread.currentThread().getContextClassLoader().getResourceAsStream(SEMANTIC_VERSION_RESOURCE)) {
            if (in != null) {
                properties.load(in);
            }
        }
