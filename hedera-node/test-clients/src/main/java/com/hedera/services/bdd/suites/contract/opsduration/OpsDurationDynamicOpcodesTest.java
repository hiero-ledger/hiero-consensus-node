...
public class OpsDurationDynamicOpcodesTest {
    ...
    private static final TestInput[] OPS_DURATION_BENCHMARK_DYNAMIC_FUNCTIONS = {
            ...
            new TestInput("benchCreate", BigInteger.valueOf(1048576), SUCCESS, Map.of("contracts.maxInitcodeSize", "1048576")),
            new TestInput("benchCreate2", BigInteger.valueOf(1048576), SUCCESS, Map.of("contracts.maxInitcodeSize", "1048576")),
    ...
    final Stream<DynamicTest> dynamicOpcodesTest() {
        return Stream.of(OPS_DURATION_BENCHMARK_DYNAMIC_FUNCTIONS)
                .flatMap(test -> Stream.of(compareGasTest(test.name(), test.length(), test.status(), test.configsChange())));
    }

    private static DynamicTest compareGasTest(
            final String name, final BigInteger length, final ResponseCodeEnum status, final Map<String, String> configsChange) {
        ...
        ops.add(withOpContext((spec, _) -> {
            ...
            if (configsChange != null && !configsChange.isEmpty()) {
                allRunFor(spec, configsChange.keySet().stream().map(UtilVerbs::restoreDefault).toList());
            }
        }));
    ...
    private static CallContractOperation call(
        ...
        return opsDurationDynamicOpcodesContract.call(name, length)
                .gas(15_000_000)
