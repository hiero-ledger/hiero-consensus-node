...
public class AtomicBatchAutoAccountCreationEndToEndTests {
    ...
    class AtomicBatchAutoAccountCreationHollowAccountsTests {
        ...
        public Stream<DynamicTest> autoCreateHollowAccountInOneBatchAndFinalizeInAnotherBatch() {
            ...
            return hapiTest(flattened(
                    ...
                    withOpContext((spec, opLog) -> {
                        ...
                        final HapiGetTxnRecord hapiGetTxnRecord = getTxnRecord("batchTxnSecond")
                                .andAllChildRecords()
                        ...
                        final var getNewAccountInfo = getAccountInfo(VALID_ALIAS_HOLLOW)
                                .has(accountWith()
                        ...
                        final var getNewAccountBalance = getAccountBalance(VALID_ALIAS_HOLLOW)
                                .hasTokenBalance(FT_FOR_AUTO_ACCOUNT, 2L)
                                .hasTokenBalance(NFT_FOR_AUTO_ACCOUNT, 1L);

                        allRunFor(spec, getNewAccountInfo, getNewAccountBalance);
                    })));
        }
        ...
        public Stream<DynamicTest> autoCreateHollowAccountInOneBatchFinalizeAndTokenTransferInAnotherBatch() {
            ...
            return hapiTest(flattened(
                    ...
                    withOpContext((spec, opLog) -> {
                        ...
                        // validate finalized account token balance
                        getAccountBalance(VALID_ALIAS_HOLLOW)
        ...
        public Stream<DynamicTest> autoCreateHollowAccountInOneBatchFinalizeOutsideBatch() {
            ...
            return hapiTest(flattened(
                    ...
                    withOpContext((spec, opLog) -> {
                        ...
                        final HapiGetTxnRecord hapiGetTxnRecord = getTxnRecord("finalizeHollowAccount")
                                .andAllChildRecords()
                                .assertingNothingAboutHashes();
                        allRunFor(spec, hapiGetTxnRecord);
        ...
        // Can't create and finalize hollow account in the same batch - check that it fails
        @HapiTest
        @DisplayName("Auto Create Hollow Account and Finalize it in the same Batch Fails in Atomic Batch")
        public Stream<DynamicTest> autoCreateHollowAccountAndFinalizeInTheSameBatchFailsInBatch() {
            ...
            return hapiTest(flattened(
                    ...
                    withOpContext((spec, opLog) -> {
                        ...
                        // validate the hollow account is not created and transfers are not successful
                        //                        final var noAccountCreatedCheck =
                        ...
                        allRunFor(
                                spec, atomicBatchTransactionFirst, batchTxnFeeCheck
                                //                                noAccountCreatedCheck
                                //                                senderBalanceCheck,
                                //                                ownerBalanceCheck
                                );

                        final HapiGetTxnRecord hapiGetTxnRecord = getTxnRecord("batchTxnFirst")
                                .andAllChildRecords()
                                .assertingNothingAboutHashes();
                        allRunFor(spec, hapiGetTxnRecord);
    ...
    // Mint token, transfer it to a public key alias and transfer from the auto-created account to another account in
    // the same batch

    // Mint token, transfer it to a public key alias and transfer from the auto-created account to another public key
