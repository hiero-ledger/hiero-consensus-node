...
public class V069AddressBookSchema extends Schema<SemanticVersion> {
    ...
    public void migrate(@NonNull final MigrationContext ctx) {
        if (!ctx.isGenesis()) {
            // TODO: Delete this schema. We will not remove the deleted nodes from the state.
            // 1) Initialize the highest node id from previous nodes in [0,100)
            //            final var highestNodeIdState =
            // ctx.newStates().getSingleton(V0690EntityIdSchema.HIGHEST_NODE_ID_STATE_ID);
            //            long highest = -1;
