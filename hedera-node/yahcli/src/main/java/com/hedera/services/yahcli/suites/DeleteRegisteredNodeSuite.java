...
public class DeleteRegisteredNodeSuite extends HapiSuite {
    private static final Logger log = LogManager.getLogger(DeleteRegisteredNodeSuite.class);
    ...
    public DeleteRegisteredNodeSuite(
    ...
    final Stream<DynamicTest> doDelete() {
        ...
        final var spec = new HapiSpec(
                "DeleteRegisteredNode", new MapPropertySource(configManager.asSpecConfig()), new SpecOperation[] {
                    adminKeyLoc == null
                            ? noOp()
                            : keyFromFile(adminKey, adminKeyLoc).yahcliLogged(),
                    registeredNodeDelete(() -> nodeId).signedBy(availableSigners())
                });
