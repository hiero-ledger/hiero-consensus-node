...
class BlockProvenStateAccessorTest {
    ...
    @Test
    void latestSnapshotEmptyUntilStateAndMetadataAvailable() {
        final var stateHash = newHashBytes(10);
        final var state = mockState(stateHash);

        final var nowSeconds = nowSeconds();
        final var timestamp =
                Timestamp.newBuilder().seconds(nowSeconds).nanos(123_456_789).build();
        final var merklePath = MerklePath.newBuilder().nextPathIndex(1).build();

        subject.registerBlockMetadata(stateHash, BLOCK_HASH_A, TSS_SIGNATURE_A, timestamp, merklePath);
    ...
    @Test
    void latestSnapshotEmptyWithoutBothStateAndMetadata() {
        final var stateHash = newHashBytes(10);
        final var state = mockState(stateHash);

        // Only observing state without metadata should not produce a snapshot
        subject.observeImmutableState(state);
        assertThat(subject.latestSnapshot()).isEmpty();
        assertThat(subject.latestState()).isEmpty();
    }
    ...
    void updateStoresSnapshot() {
        ...
        subject.registerBlockMetadata(
                secondStateHash, BLOCK_HASH_B, TSS_SIGNATURE_B, secondTimestamp, secondMerklePath);
    ...
    @Test
    void observeIgnoresDestroyedMutableOrHashlessStates() {
    ...
    @Test
    void expiredMetadataDoesNotYieldSnapshot() {
        final var stateHash = newHashBytes(30);
        final var state = mockState(stateHash);

        subject.observeImmutableState(state);

        final var expiredTimestamp =
                Timestamp.newBuilder().seconds(nowSeconds() - 10_000L).nanos(0).build();
    ...
    @Test
    void registeringMetadataBeforeStateDoesNotClearLatestSnapshot() {
    ...
    @Test
    void constructorRegistersObserverAndObserverUpdatesState() {
    ...
    void blockSignedSnapshotReturnsBlockHashAsTssSignature() {
        ...
        final var state = mockState(stateHash);
        final var timestamp = Timestamp.newBuilder().seconds(nowSeconds()).build();
        final var path = MerklePath.newBuilder().nextPathIndex(7).build();
    ...
    @Test
    void observeImmutableStateRejectsNull() {
        org.assertj.core.api.Assertions.assertThatThrownBy(() -> subject.observeImmutableState(null))
                .isInstanceOf(NullPointerException.class);
    }

    @Test
    void registerBlockMetadataRejectsNullArgs() {
        final var stateHash = newHashBytes(70);
        final var timestamp = Timestamp.newBuilder().seconds(nowSeconds()).build();
        final var path = MerklePath.newBuilder().nextPathIndex(8).build();

        org.assertj.core.api.Assertions.assertThatThrownBy(
                        () -> subject.registerBlockMetadata(null, BLOCK_HASH_A, TSS_SIGNATURE_A, timestamp, path))
                .isInstanceOf(NullPointerException.class);
        org.assertj.core.api.Assertions.assertThatThrownBy(
                        () -> subject.registerBlockMetadata(stateHash, null, TSS_SIGNATURE_A, timestamp, path))
                .isInstanceOf(NullPointerException.class);
        org.assertj.core.api.Assertions.assertThatThrownBy(
                        () -> subject.registerBlockMetadata(stateHash, BLOCK_HASH_A, null, timestamp, path))
                .isInstanceOf(NullPointerException.class);
        org.assertj.core.api.Assertions.assertThatThrownBy(
                        () -> subject.registerBlockMetadata(stateHash, BLOCK_HASH_A, TSS_SIGNATURE_A, null, path))
                .isInstanceOf(NullPointerException.class);
        org.assertj.core.api.Assertions.assertThatThrownBy(
                        () -> subject.registerBlockMetadata(stateHash, BLOCK_HASH_A, TSS_SIGNATURE_A, timestamp, null))
                .isInstanceOf(NullPointerException.class);
    }

    @Test
    void latestSnapshotReflectsLatestCompleteStateNotLatestObservedState() {
        // Observe two states, but only register metadata for the first.
        // latestSnapshot should return the first state, even after observing the second.
        final var firstHash = newHashBytes(80);
        final var firstState = mockState(firstHash);
        final var secondHash = newHashBytes(81);
        final var secondState = mockState(secondHash);
        final var timestamp = Timestamp.newBuilder().seconds(nowSeconds()).build();
        final var path = MerklePath.newBuilder().nextPathIndex(9).build();
