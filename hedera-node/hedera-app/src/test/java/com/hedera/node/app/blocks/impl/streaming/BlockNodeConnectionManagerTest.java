...
class BlockNodeConnectionManagerTest extends BlockNodeCommunicationTestBase {
    ...
    //                    .findVarHandle(BlockNodeConnectionManager.class, "isConnectionManagerActive",
    // AtomicBoolean.class);
    //            activeConnectionRefHandle = MethodHandles.privateLookupIn(BlockNodeConnectionManager.class, lookup)
    //                    .findVarHandle(BlockNodeConnectionManager.class, "activeConnectionRef",
    ...
    //            nodeStatusTaskConnectionHandle = MethodHandles.privateLookupIn(RetrieveBlockNodeStatusTask.class,
    // lookup)
    //                    .findVarHandle(
    ...
    // MethodHandles.privateLookupIn(BlockNodeConnectionManager.class, lookup)
    //                    .findVarHandle(BlockNodeConnectionManager.class, "globalActiveStreamingConnectionCount",
    // AtomicInteger.class);
    ...
    //        nodes.clear();
    //        nodes.put(node1Config.address(), new BlockNode(node1Config, globalActiveStreamingConnectionCount));
    //        nodes.put(node2Config.address(), new BlockNode(node2Config, globalActiveStreamingConnectionCount));
    //    }
    //
    //    @BeforeEach
    //    void beforeEach() {
    ...
    //
    //        // introduce a failure on one of the connection closes to ensure the shutdown process does not fail
    // prematurely
    ...
    //        isActive.set(false);
    //
    //        blockNodes().clear(); // remove all available nodes from config
    //
    //        assertThat(isActive).isFalse();
    //
    //        verifyNoMoreInteractions(bufferService);
    ...
    //        Files.writeString(
    //                file, valid, StandardCharsets.UTF_8, StandardOpenOption.CREATE,
    ...
    //                .withValue("blockStream.writerMode", "FILE_AND_GRPC")
    //                .withValue("blockNode.blockNodeConnectionFileDir", "/tmp/non-existent-test-dir-" +
    // System.nanoTime())
    //                .getOrCreateConfig();
    //        final ConfigProvider configProvider = () -> new VersionedConfigImpl(config, 1L);
    //
    ...
    //
    //        // Verify exactly 1 schedule call was made (only the retry, no new node selection since there's only one
    // node)
    ...
    //        final List<BlockNodeConfiguration> availableNodes = availableNodes();
    //
    //        final BlockNodeConfiguration newConnectionConfig = newBlockNodeConfig("invalid.hostname.for.test", 50211,
    // 1);
    //        final BlockNodeStreamingConnection newConnection = mock(BlockNodeStreamingConnection.class);
    //        doReturn(newConnectionConfig).when(newConnection).configuration();
    //
    ...
    //        final BlockNodeConfiguration nodeConfig = newBlockNodeConfig(PBJ_UNIT_TEST_HOST, 8080, 1);
    //
    //        final boolean limitExceeded = connectionManager.recordBehindPublisherAndCheckLimit(nodeConfig,
    // Instant.now());
    //        assertThat(limitExceeded).isFalse();
    //
    ...
    //
    //        // since one of the nodes in priority group 1 was chosen, we should only interact with the buffer service
    // once
    ...
    //        /*
    //        This test validates that when all the block nodes respond with -1 as the latest block, we treat it as a
    // wildcard
    //        meaning the block node will accept whatever we send it - unless later the block node tells us something
    //        different via a response message like SkipBlock or BehindPublisher. Thus, block nodes with no known latest
    // block
    ...
    //        /*
    //        This test validates a scenario in which the consensus node has no blocks in the buffer. This may be due to
    // a
    //        restart in which no blocks were previously persisted (e.g. they were all acked) or it may due to the node
    // being
    //        initialized for the first time. In such a scenario, any reachable block node will be considered a
    // candidate to
    //        connect to.
    //         */
    //        final BlockNodeConfiguration node1Config = newBlockNodeConfig(8080, 1);
    ...
    //        doAnswer(invocation -> {
    //                    // return a successful null for node 1 and then successful, non-null response for the rest of
    // the
    //                    // nodes
    //                    final List<RetrieveBlockNodeStatusTask> tasks = invocation.getArgument(0);
    //                    final List<CompletableFuture<BlockNodeStatus>> futures = new ArrayList<>();
    ...
    //            final RetrieveBlockNodeStatusTask task = connectionManager.new
    // RetrieveBlockNodeStatusTask(nodeConfig);
    //            final BlockNodeServiceConnection connection =
    ...
    //            final AtomicInteger globalActiveStreamingConnectionCount = globalActiveStreamingConnectionCount();
    //            nodes.clear();
    //            blockNodes.forEach(bnCfg -> nodes.put(bnCfg.address(), new BlockNode(bnCfg,
    ...
    //        connectionManager =
    //                new BlockNodeConnectionManager(disabledProvider, bufferService, metrics,
    // blockingIoExecutorSupplier);
    //    }
    //
