...
public class TransactionExecutorsTest {
    ...
    void executesTransactionsAsExpected() {
        ...
        // Now execute a ContractCall against the contract, with an extra StreamingOperationTracer whose output we
        ...
        final var streamingTracer = new StreamingOperationTracer(
                printWriter,
                OpCodeTracerConfigBuilder.create().traceOpcodes(Set.of()).build());
        final var addOnTracer = new OperationTracerAdapter(streamingTracer);
     ...
     * This allows using Besu's standard tracers (like {@code StreamingOperationTracer}) with the Hedera
