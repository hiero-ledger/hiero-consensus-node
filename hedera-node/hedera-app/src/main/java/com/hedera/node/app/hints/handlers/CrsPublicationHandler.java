...
public class CrsPublicationHandler implements TransactionHandler {
    private static final Logger log = LogManager.getLogger(CrsPublicationHandler.class);
    ...
    public void handle(@NonNull final HandleContext context) throws HandleException {
        ...
        controllers.getAnyInProgress().ifPresent(controller -> {
            ...
            if (!crsState.hasNextContributingNodeId() || creatorId != crsState.nextContributingNodeIdOrThrow()) {
                return;
            }
            // Only persist well-formed publications; the controller still advances past malformed ones.
            final var expectedCrsLength = crsState.crs().length();
            if (HintsLibrary.isValidCrsUpdateFraming(crsState.crs(), op.newCrs(), op.proof())) {
                hintsStore.addCrsPublication(creatorId, op);
            } else {
                log.warn(
                        "Not persisting malformed CRS from node{}: newCrs.length={} (expected {}), proof.length={} (expected {})",
                        creatorId,
                        op.newCrs().length(),
                        expectedCrsLength,
                        op.proof().length(),
