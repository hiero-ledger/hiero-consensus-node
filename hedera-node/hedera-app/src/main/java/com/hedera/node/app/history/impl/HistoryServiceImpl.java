...
public class HistoryServiceImpl implements HistoryService {
    ...
    public boolean doPostUpgradeSetup(
        ...
        requireNonNull(writableStates);
        requireNonNull(configuration);
        if (!configuration.getConfigData(TssConfig.class).historyEnabled()) {
            return false;
        }
        final var ledgerId =
                writableStates.<ProtoBytes>getSingleton(LEDGER_ID_STATE_ID).get();
        ...
        final var nextConstruction = writableStates
                .<HistoryProofConstruction>getSingleton(NEXT_PROOF_CONSTRUCTION_STATE_ID)
                .get();
        final var wrapsProvingKeyHash = writableStates
