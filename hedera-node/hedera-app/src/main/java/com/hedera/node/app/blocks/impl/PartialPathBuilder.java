...
class PartialPathBuilder {
     ...
     * @param siblingHashes the full sibling hash array produced by the block-proof pipeline
     *                      (e.g., from {@code combine()}). Index 2 is the root of branches 5-8
     *                      and index 3 is the reserved root of branches 9-16.
     * @return the partial {@link MerklePath} from the starting-state subroot up to (but not
     *         including) the block timestamp leaf, with {@code nextPathIndex = 2} pointing to the
    ...
    static MerklePath startingStateToBlockRoot(
        ...
        final var blockAccessorSiblings = new SiblingNode[4];
        ...
        // Sibling 3: same as the block proof's fourth sibling, the reserved branches 9-16 root
        blockAccessorSiblings[3] = SiblingNode.newBuilder()
                .isLeft(false)
                .hash(siblingHashes[3].siblingHash())
                .build();
