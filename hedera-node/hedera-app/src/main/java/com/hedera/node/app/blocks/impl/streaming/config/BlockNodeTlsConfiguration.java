 ...
 * <p>
 * When a certificate fingerprint is supplied, the endpoint's certificate is accepted if and only if its SHA-256 hash
...
public class BlockNodeTlsConfiguration {
     ...
     */
    private static final int FINGERPRINT_LENGTH_BYTES = 32;
    ...
    /**
     * SHA-256 fingerprint of the certificate this endpoint is expected to present, or null to verify the certificate
    ...
    private BlockNodeTlsConfiguration(final Builder builder) {
        ...
        certificateSha256 = builder.certificateSha256;

        if (!enabled && certificateSha256 != null) {
            throw new IllegalArgumentException(
                    "A certificate fingerprint must not be specified for an endpoint that does not use TLS");
        }
        if (certificateSha256 != null && certificateSha256.length != FINGERPRINT_LENGTH_BYTES) {
            throw new IllegalArgumentException("Certificate fingerprint must be " + FINGERPRINT_LENGTH_BYTES
                    + " bytes, but was " + certificateSha256.length);
        }
    }
     ...
     */
    public @Nullable byte[] certificateSha256() {
        return certificateSha256 == null ? null : certificateSha256.clone();
    }
    ...
    /**
     * @return an SSL context that trusts exactly one certificate, identified by its SHA-256 fingerprint
     */
    private @NonNull SSLContext pinnedSslContext() {
        try {
            final SSLContext sslContext = SSLContext.getInstance("TLS");
            sslContext.init(null, new TrustManager[] {new PinnedCertificateTrustManager(certificateSha256)}, null);
            return sslContext;
    ...
    public boolean equals(final Object o) {
        ...
        return enabled == that.enabled && Arrays.equals(certificateSha256, that.certificateSha256);
    }

    @Override
    public int hashCode() {
        return 31 * Boolean.hashCode(enabled) + Arrays.hashCode(certificateSha256);
    }

    @Override
    public String toString() {
        return "BlockNodeTlsConfiguration{" + "enabled="
                + enabled + ", certificateSha256="
                + (certificateSha256 == null ? null : "'" + HexFormat.of().formatHex(certificateSha256) + "'") + '}';
    }
    ...
    public static @NonNull BlockNodeTlsConfiguration from(@Nullable final BlockNodeTlsConfig tlsConfig) {
        ...
        return newBuilder()
                ...
                .certificateSha256(tlsConfig.certificateSha256())
                .build();
    }

    /**
     * Parses a hex-encoded certificate fingerprint, tolerating the colon-separated form commonly emitted by
     * certificate tooling (for example {@code openssl x509 -fingerprint -sha256}).
     *
     * @param hexFingerprint the hex-encoded fingerprint
     * @return the decoded fingerprint
     * @throws IllegalArgumentException if the value is not a valid SHA-256 fingerprint
     */
    private static @NonNull byte[] parseHexFingerprint(@NonNull final String hexFingerprint) {
        ...
        try {
            fingerprint = HexFormat.of().parseHex(normalized);
        ...
        if (fingerprint.length != FINGERPRINT_LENGTH_BYTES) {
            throw new IllegalArgumentException("Certificate fingerprint '" + hexFingerprint + "' decodes to "
                    + fingerprint.length + " bytes, but a SHA-256 fingerprint is " + FINGERPRINT_LENGTH_BYTES
                    + " bytes");
        }
        return fingerprint;
    }
    ...
    public static class Builder {
        ...
        private byte[] certificateSha256;
         ...
         *
         * @param hexFingerprint the hex-encoded SHA-256 fingerprint, or null
         * @return this builder
         * @throws IllegalArgumentException if the value is not a valid SHA-256 fingerprint
         */
        public @NonNull Builder certificateSha256(@Nullable final String hexFingerprint) {
