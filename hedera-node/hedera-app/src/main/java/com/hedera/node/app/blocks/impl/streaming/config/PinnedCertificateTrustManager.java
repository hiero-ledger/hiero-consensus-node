...
/**
 * A trust manager that accepts exactly one server certificate, identified by its SHA-256 fingerprint.
 * <p>
...
final class PinnedCertificateTrustManager implements X509TrustManager {
    ...
    /**
     * SHA-256 fingerprint of the only certificate this trust manager accepts.
     */
    private final byte[] expectedFingerprint;

    /**
     * @param expectedFingerprint the SHA-256 fingerprint of the certificate to accept
     */
    ...
    public void checkServerTrusted(@Nullable final X509Certificate[] chain, @Nullable final String authType)
        ...
        final byte[] actualFingerprint = sha256(certificate);
        if (!MessageDigest.isEqual(expectedFingerprint, actualFingerprint)) {
            throw new CertificateException("Block node certificate does not match the configured fingerprint; expected "
                    + HexFormat.of().formatHex(expectedFingerprint) + " but was "
                    + HexFormat.of().formatHex(actualFingerprint));
        }
     ...
     * @return the SHA-256 hash of the certificate's DER encoding
     * @throws CertificateException if the certificate cannot be encoded
     */
    private static @NonNull byte[] sha256(@NonNull final X509Certificate certificate) throws CertificateException {
        try {
            ...
            // SHA-256 is required to be present in every JRE
