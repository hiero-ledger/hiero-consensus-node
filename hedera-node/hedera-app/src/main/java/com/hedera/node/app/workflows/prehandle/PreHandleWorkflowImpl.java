...
public class PreHandleWorkflowImpl implements PreHandleWorkflow {
    ...
    private PreHandleResult expandAndVerifySignatures(
        ...
        try {
            ...
            // Then, mirroring the ingest ordering (pure checks before privilege), reject a privileged file
            // operation whose payer lacks the required privilege. That is decidable from the payer and body
            // alone, so reaching consensus means the submitting node failed its due diligence and is charged.
            // This must precede the batch-key check below, so a stray batch key on an unauthorized privileged
            // file update cannot mask the authorization failure and shift the charge from the node to the payer.
