...
public final class BlockRecordManagerImpl implements BlockRecordManager {
    ...
    /**
     * Construct BlockRecordManager with an optional pre-computed wrapped-record block hash migration
     * result that seeds the live wrapped-record hash chain so it matches the chain that will be agreed
     * by {@link com.hedera.node.app.records.handlers.MigrationRootHashVoteHandler} once voting completes.
     * Pass {@code null} for {@code migrationResult} when no jumpstart migration was performed.
     */
    public BlockRecordManagerImpl(
        ...
        if (initTrigger != InitTrigger.GENESIS && liveWritePrevWrappedRecordHashes()) {
            ...
            if (seededFromMigration && !queuedHashes.isEmpty()) {
                ...
                logger.info(
                        "Replayed {} queued migration wrapped-hash entries on live hasher (final leafCount={}, prevHash={})",
                        queuedHashes.size(),
