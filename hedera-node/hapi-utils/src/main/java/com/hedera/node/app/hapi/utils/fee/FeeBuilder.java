...
public class FeeBuilder {
    ...
    public static FeeObject getFeeObject(
        ...
        // get the Network Fee
        long networkFee = getComponentFeeInTinyCents(feeData.networkdata(), feeMatrices.networkdata());
        long nodeFee = getComponentFeeInTinyCents(feeData.nodedata(), feeMatrices.nodedata());
        long serviceFee = getComponentFeeInTinyCents(feeData.servicedata(), feeMatrices.servicedata());
    ...
    public static int[] calculateKeysMetadata(final Key key, final int[] count) {
        ...
        if (key.hasKeyList()) {
            ...
            final List<Key> keyList = key.thresholdKey().keys().keys();
            workingCount[1]++;
            for (final Key value : keyList) {
                workingCount = calculateKeysMetadata(value, workingCount);
            }
    ...
    public static int getSignatureSize(final Transaction transaction) {
        try {
            return CommonUtils.extractSignatureMap(transaction).hashCode();
