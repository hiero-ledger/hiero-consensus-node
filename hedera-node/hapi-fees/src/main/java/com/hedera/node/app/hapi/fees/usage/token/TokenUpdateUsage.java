...
public class TokenUpdateUsage extends TokenTxnUsage<TokenUpdateUsage> {
    ...
    public FeeData get() {
        ...
        long newLifetime =
                ESTIMATOR_UTILS.relativeLifetime(this.op, Math.max(op.expiry().seconds(), currentExpiry));
    ...
    private int noRbImpactBytes(final TokenUpdateTransactionBody op) {
        return ((op.expiry().seconds() > 0) ? AMOUNT_REPR_BYTES : 0)
                + ((op.autoRenewPeriod().seconds() > 0) ? AMOUNT_REPR_BYTES : 0)
