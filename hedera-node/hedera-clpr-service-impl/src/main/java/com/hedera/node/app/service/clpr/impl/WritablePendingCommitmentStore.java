...
/**
 * Store for CLPR pending ownership commitments.
 * Acts as a set — the key and value are both the commitment hash.
 */
public class WritablePendingCommitmentStore {

    private final WritableKVState<ProtoBytes, ProtoBytes> state;

    public WritablePendingCommitmentStore(@NonNull final WritableStates states) {
        requireNonNull(states);
        this.state = states.get(PENDING_COMMITMENTS_STATE_ID);
    }
