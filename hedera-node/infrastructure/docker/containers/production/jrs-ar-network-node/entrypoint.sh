...
echo "/usr/bin/env java --enable-native-access=ALL-UNNAMED --sun-misc-unsafe-memory-access=allow ${JAVA_HEAP_OPTS} ${JAVA_OPTS} --module-path \"${JAVA_MODULE_PATH}\" --module \"${JAVA_MAIN_MODULE}\" ${CONSENSUS_NODE_ARGS} 1> >(tee logs/stdout.log) 2> >(tee logs/stderr.log >&2)"
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< END JAVA COMMAND   <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
echo

/usr/bin/env java --enable-native-access=ALL-UNNAMED --sun-misc-unsafe-memory-access=allow ${JAVA_HEAP_OPTS} ${JAVA_OPTS} --module-path "${JAVA_MODULE_PATH}" --module "${JAVA_MAIN_MODULE}" ${CONSENSUS_NODE_ARGS} 1> >(tee logs/stdout.log) 2> >(tee logs/stderr.log >&2)
