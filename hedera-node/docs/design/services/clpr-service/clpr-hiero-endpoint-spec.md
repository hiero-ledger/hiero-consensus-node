...
│  ┌──────────────────────────────────────────────────────────┐│
│  │              Merkle State Tree                           ││
│  │  (CLPR state: channels, queues, endpoints, config)    ││
│  └──────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────┘
```
...
| **State APIs**                       | `ReadableStates`, `WritableStates`, etc.                        | All CLPR state (channels, queues, endpoints, configuration) is accessed here                                                                 |
...
- `ClprChannelManager` — manages the lifecycle of peer channels and sync scheduling
...
- **Peer verification.** TLS provides transport encryption. Peer identity verification is NOT based on TLS
  ...
  authentication is proof-based: any peer that provides a valid bundle proof (verified by the Channel's
  verifier contract) is legitimate regardless of identity. Local shunning of endpoints that send invalid data is
  optional and gated by `clpr.syncPeerExclusionEnabled`.
...
**Outbound sync trigger.** The orchestrator monitors each active Channel's queue state. When a Channel has
...
**Sync interval computation.** The outbound sync interval for a Channel is derived from the peer ledger's
`max_syncs_per_sec` configuration and the number of local endpoints registered on that Channel:
...
If `max_syncs_per_sec` is not yet known (e.g., the Channel is newly registered and no `ConfigUpdate` has been
...
this ledger. The endpoint module tracks inbound sync frequency per `(channel_id, remote_endpoint_account_id)`
pair to detect abuse (see Section 12.1).

**Channel status filtering.** Per cross-platform spec §2.1.1, Channels have five statuses. On Hiero, the
sync orchestrator filters as follows: ACTIVE, PAUSED, CLOSING, and DRAINED Channels all participate in syncs
(both inbound and outbound). PAUSED Channels continue outbound syncs and accept inbound syncs, but reject
inbound bundles containing out-of-order responses. CLOSING and DRAINED Channels continue syncing to drain
queues. Only CLOSED Channels are skipped entirely (see Section 13.4 for PAUSED, CLOSING, and DRAINED behavior).

## 3.2 Peer Endpoint Selection

When initiating a sync for a Channel, the endpoint must select a peer endpoint from the Channel's peer roster.
...
`(channel_id, peer_endpoint_account_id)`. This table is not persisted to state — it is rebuilt from scratch
...
channels, CPU for proof construction) on any single node.

**Per-Channel limit.** At most one outbound sync per Channel may be in progress at any time. This prevents
redundant work — if a sync is already in progress for a Channel, the orchestrator skips it on the current tick.
...
streams configuration. If `clpr.syncPeerExclusionEnabled=true` and a peer endpoint exceeds the local ledger's
`max_syncs_per_sec`, the excess calls SHOULD be rejected with a gRPC `RESOURCE_EXHAUSTED` status; when the flag
is false, this local exclusion path fails open.
...
1. Read latest immutable state (via LatestCompleteStateNexus):
   - Channel metadata, unacknowledged messages, queue metadata
...
**Asymmetry note.** The initiator pre-computes its payload before opening the channel. The responder computes
...
|-----------------|---------------------------------------------------------------------|--------------------------------------------------------|
| Queue metadata  | `CLPR_QUEUE_METADATA / {channel_id}`                                | Proves queue metadata fields per spec §1.5             |
| Message entries | `CLPR_MESSAGES / {channel_id, message_id}` for each unacked message | Proves message payloads and per-message running hashes |
| Channel config  | `CLPR_CHANNELS / {channel_id}`                                      | Proves Channel status and verifier binding             |
...
`received_message_id` to update the local Channel's `acked_message_id` (see cross-platform spec §4.2, step 5).
...
TSS public key changes (resulting from roster changes) are communicated through the proof itself. The
`TssSignedBlockProof.block_signature` embeds WRAPS chain-of-trust data that carries roster transitions.
The verifier bootstraps from a known TSS public key and accepts transitions when a proof is validly signed by
...
The Hiero proof bytes are a serialized `StateProof` message, defined in
`block/stream/state_proof.proto`:

```protobuf
// From block/stream/state_proof.proto.
// For CLPR, each proven state element (queue metadata, message entries,
// channel config) is carried as a state_item_leaf within a MerklePath.
message StateProof {
  // Merkle paths proving state leaves and the consensus timestamp.
  // Paths are ordered: previous-block-root path first, then timestamp path,
  // then the block merkle root path, followed by per-state-item paths.
  repeated MerklePath paths = 1;

  oneof proof {
    // TSS threshold signature over the block.
    // block_signature embeds WRAPS chain-of-trust data for roster
    // transitions, allowing verifiers to track key rotations.
    TssSignedBlockProof signed_block_proof = 2;
  }
}

message MerklePath {
  // Sibling nodes ordered from leaf to root.
  repeated SiblingNode siblings = 1;

  // Index into StateProof.paths of the next parent path (UINT32_MAX = root).
  uint32 next_path_index = 2;

  oneof content {
    bytes hash            = 3; // internal node hash (no leaf)
    bytes state_item_leaf = 4; // serialized StateItem (CLPR queue/message/channel data)
...
// From block/stream/tss_signed_block_proof.proto.
message TssSignedBlockProof {
  // Aggregated TSS signature. Includes WRAPS data for address book transitions.
...
1. **Verify TSS signature.** Validate `signed_block_proof.block_signature` against the verifier's tracked trust
   anchor for the source Hiero network. The WRAPS data embedded in `block_signature` conveys any roster
   transitions; the verifier applies these to advance its trust anchor before checking the signature.

2. **Verify Merkle paths.** For each `MerklePath` with a `state_item_leaf`, recompute the root hash by hashing
   the leaf value and walking the `siblings` entries upward through linked `next_path_index` paths. The computed
   root MUST match the block root authenticated by the TSS signature.

3. **Extract verified data.** Decode each `state_item_leaf` (a serialized `StateItem`) to obtain the
   `ClprQueueMetadata` and `ClprMessagePayload[]` return values.
...
reimbursed. On Hiero, the endpoint performs a **cost-saving pre-consensus check**: it executes the Channel's
...
(cross-platform spec §1.3), not by the endpoint module. The endpoint reads peer throttle values from Channel
...
The sync orchestrator reads Channel metadata (status, message IDs, peer roster) to decide which Channels
...
| `clpr.syncPeerExclusionEnabled`      | `bool` | `false` | Enables local peer-exclusion heuristics such as inbound shunning and outbound circuit-breaker filtering. |
...
3. **State initialization.** Read the latest signed state to load all active Channels, their peer rosters, and
...
5. **Sync orchestrator start.** Start the sync loop thread pool. After a brief startup delay (to allow state to
   settle), begin monitoring Channels for pending outbound messages.
...
- All Channel state (queue metadata, endpoint rosters) is read from the signed state.
- The sync orchestrator rebuilds its in-memory scheduling state from the Channel metadata.
...
| `clpr.bundles.rejected.closed`       | Counter | Bundles rejected because the Channel is CLOSED.                                     |
| `clpr.messages.sent`                 | Counter | Total messages enqueued in outbound queues (all Channels).                          |
...
|             Metric Name             | Type  |                   Description                   |
|-------------------------------------|-------|-------------------------------------------------|
| `clpr.channels.active`              | Gauge | Number of Channels in ACTIVE status.            |
| `clpr.channels.paused`              | Gauge | Number of Channels in PAUSED status.            |
| `clpr.channels.closing`             | Gauge | Number of Channels in CLOSING status.           |
| `clpr.channels.drained`             | Gauge | Number of Channels in DRAINED status.           |
| `clpr.queue.depth.{channel_id}`     | Gauge | Current outbound queue depth per Channel.       |
| `clpr.peers.reachable.{channel_id}` | Gauge | Number of reachable peer endpoints per Channel. |
...
|                 Indicator                  |                               Condition                               |
|--------------------------------------------|-----------------------------------------------------------------------|
| `clpr.healthy`                             | At least one active Channel has at least one reachable peer endpoint. |
| `clpr.stateStale`                          | The latest signed state is older than `maxStateAgeSeconds`.           |
| `clpr.allCircuitBreakersOpen.{channel_id}` | All peer endpoints for a Channel have open circuit breakers.          |
...
- **Request-level rate limiting.** The endpoint module tracks inbound sync frequency per `(channel_id,
  remote_endpoint_account_id)` pair. If `clpr.syncPeerExclusionEnabled=true` and a peer exceeds
  `max_syncs_per_sec`, subsequent calls are rejected with `RESOURCE_EXHAUSTED` without processing. The default
  false setting makes this path observational and fail-open.
...
- **TLS requirement.** All channels MUST use TLS. Plaintext channels to the CLPR endpoint are rejected.
...
2. **Proof verification.** Any peer that provides a valid bundle proof (verified by the Channel's verifier
...
4. **Optional shunning.** Peers that send invalid data or abuse rate limits are shunned locally only when
   `clpr.syncPeerExclusionEnabled=true`.
...
- If **all** peer endpoints for a Channel have open circuit breakers, the endpoint module logs a warning and
...
detected and `clpr.syncPeerExclusionEnabled=true`, the offending peer endpoint is shunned — no cross-ledger
report is sent. With the default false setting, the tracker cannot cause local sync admission to fail.
...
## 13.3 Channel-Level Resilience

When all peer endpoints for a Channel have open circuit breakers:
...
| No active Channels           | Endpoint module runs but does nothing. No resource consumption beyond the tick timer.                  |
| All Channels PAUSED          | Outbound syncs continue. Inbound bundles with out-of-order responses rejected. `sendMessage` rejected. |
| Channel PAUSED               | See PAUSED handling below.                                                                             |
| Channel CLOSING              | See CLOSING handling below.                                                                            |
...
**PAUSED Channel handling.** Per cross-platform spec §2.1.1, PAUSED indicates a response ordering violation
detected during `submitBundle`. Inbound bundles containing out-of-order responses are rejected — nothing is
processed, nothing dispatched, no acks updated. On Hiero, the sync orchestrator reads Channel status from
immutable state and:

- Continues initiating outbound syncs for the PAUSED Channel (queued messages and acks still flow).
- Continues accepting inbound sync calls for the PAUSED Channel.
- Rejects inbound bundles with out-of-order responses. When a bundle arrives with correctly ordered responses,
  the Channel auto-resumes to ACTIVE and the bundle is processed normally.

**CLOSING Channel handling.** CLOSING indicates an admin-initiated close that is draining queues. On Hiero:

- Continues initiating outbound syncs for the CLOSING Channel (acks and `CHANNEL_CLOSED` responses drain).
- Continues accepting inbound sync calls for the CLOSING Channel.
- When the peer has acknowledged all outbound messages, the Channel transitions to DRAINED. The
  `ClprQueueMetadata.state` reflects the Channel status and is propagated to the peer via the next sync.
- The CLPR Service generates `CHANNEL_CLOSED` responses for in-flight messages during `handle()`.

**DRAINED Channel handling.** DRAINED indicates this side's outbound queue is fully acknowledged. On Hiero:
...
- **Check latest state before submission.** Before submitting, re-read the Channel's `received_message_id`
...
verifier locally before submitting the transaction. However, the verifier needs access to the Channel's state
(to know which verifier contract to use, the Channel's status, etc.).

**Assumption:** The local verifier execution reads from the latest immutable state. If the Channel's verifier
...
Section 5 of this spec defines the `StateProof` format (from `block/stream/state_proof.proto`) as the
encoding for Hiero-sourced `proof_bytes`. However, the cross-platform spec notes (Section 1.5) that the
encoding format is "under review" and Jasper is examining XDR as an alternative for gas efficiency on
Ethereum. If the proof bytes format changes to XDR, the Hiero-side proof construction code must change,
but the Hiero verifier contract interface (`verifyBundle` accepting opaque `bytes`) remains stable.
...
account must have sufficient hbar balance to pay for bundle transactions. For high-traffic Channels, this
...
Each sync round from each Channel may generate a `submitBundle` transaction from every node on the network.
For a network with N nodes and C active Channels, this is up to `N * C` transactions per sync round. At the
derived sync interval (e.g., if `max_syncs_per_sec = 10` with 30 endpoints → 3-second interval) and a 30-node
network with 5 Channels, this could be up to `30 * 5 / 3 = 50` additional transactions per second when all
Channels have pending messages. The deduplication heuristics (Section 13.5) should reduce this significantly,
