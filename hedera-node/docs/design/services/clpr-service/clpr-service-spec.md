...
`received_message_id` = 0, and both running hashes are 32 bytes of zeros.
...
2. Verifies the **ZK proof** using the built-in ZK verifier, extracting the peer's configuration (including
   `chain_id`, `service_address`, `approved_verifiers`).
3. Checks the deployed verifier contract's code hash against the peer's `approved_verifiers`. Rejects if no match.
4. Creates the Connection: stores the Connection ID, peer's `chain_id` and `service_address`, peer configuration
