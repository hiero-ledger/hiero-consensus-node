...
ClprServiceProxy (EIP-1967 TransparentUpgradeableProxy)
  --> ClprService (implementation)
        |
        |--> IClprVerifier (one per Channel, externally deployed)
...
state. When a Channel enters PAUSED due to a response ordering violation (see section 8.2), the root
cause may be a bug in the ClprService logic itself. The EIP-1967 proxy enables deploying a fixed
implementation that can diagnose and resolve the issue, restoring the Channel to ACTIVE without
requiring a new Channel registration. Without upgradeability, a persistently PAUSED Channel would
...
| `ADMIN_ROLE`         | `DEFAULT_ADMIN_ROLE` | `setLedgerConfiguration`, `closeChannel`, `redactMessage` |
| `UPGRADER_ROLE`      | `DEFAULT_ADMIN_ROLE` | Reserved for future use if upgrade logic moves on-chain.  |

All permissionless functions (channel registration, endpoint registration, message sending, bundle submission)
...
/// @dev Maps to Spec section 6 pseudo-API reference. All channel_id parameters
///      are bytes32 (arbitrary 32-byte identifier, same on both ledgers).
interface IClprService {
    ...
    ///      ConfigUpdate Control Messages are lazily enqueued on each Channel at its
    ...
    // =========================================================================
    // Channel Management (Spec section 6.2)
    // =========================================================================

    /// @notice Phase 1 (Commit): Register an ownership commitment on this ledger. Permissionless.
    /// @dev Stores the commitment. No Channel state is created yet — neither the channelId
    ///      nor the public key is revealed at this stage. The commitment prevents cross-ledger
    ///      front-running (see cross-platform spec §5.1).
    /// @param commitment The ownership commitment: keccak256(channelId || publicKey).
    ...
    /// @notice Phase 2 (Reveal): Complete registration by revealing the Channel ID and
    ///         public key. Permissionless.
    /// @dev Validates keccak256(channelId || ecdsaPublicKey) matches a stored commitment,
    ///      verifies the ECDSA signature, and creates the Channel in ACTIVE state.
    /// @param channelId The Channel ID (arbitrary 32-byte value, same on both ledgers).
    ///        Must not already exist as a Channel.
    /// @param ecdsaPublicKey The uncompressed secp256k1 public key (64 bytes, x||y without
    ///        0x04 prefix). Used to verify the ownership commitment.
    /// @param ecdsaSignature ECDSA secp256k1 signature over keccak256(channelId),
    ///        proving the caller controls the keypair.
    ...
    /// @notice Initiate graceful close of a Channel. ADMIN_ROLE only.
    /// @dev When called on a PENDING Channel, deletes it immediately (no drain needed).
    ///      For ACTIVE or PAUSED → transitions to CLOSING. In-flight messages receive
    ///      CHANNEL_CLOSED responses without dispatch. The Channel transitions
    ///      through DRAINED to CLOSED automatically when all queues are fully drained.
    ///      If closed from PAUSED, the Channel cannot drain until the peer fixes
    ///      the response ordering; once fixed, bundles process with CHANNEL_CLOSED
    ///      responses and queues drain normally.
    /// @param channelId The Channel ID.
    function closeChannel(bytes32 channelId) external;

    // NOTE: getChannel is not exposed on-chain. Channel state is available
    ...
    /// @notice Query a Channel's current outbound queue depth.
    /// @param channelId The Channel ID.
    /// @return depth Current number of unacknowledged messages.
    /// @return max Maximum allowed queue depth.
    function getQueueDepth(bytes32 channelId) external view returns (uint64 depth, uint32 max);

    // =========================================================================
    // Connector Management (Spec section 6.3)
    // =========================================================================

    /// @notice Register a Connector on a Channel. Permissionless but requires ETH.
    ...
    ///        keccak256(abi.encodePacked(channelId, proofBytes)).
    ...
    function redactMessage(bytes32 channelId, uint64 messageId) external;

    // =========================================================================
    // Endpoint Management (Spec section 6.5)
    // =========================================================================

    /// @notice Register as an endpoint for a Channel. Permissionless; bond required.
    ...
    /// @notice Deregister an endpoint from a Channel and return the bond.
    /// @dev Caller must be the endpoint's account.
    /// @param channelId The Channel ID.
    function deregisterEndpoint(bytes32 channelId) external;
...
/// @notice Emitted when a new Channel is registered.
...
/// @notice Emitted when a Channel's status changes.
...
/// @notice Emitted when a Connector is registered on a Channel.
...
// --- Channels ---
// Slot N: mapping from channelId to Channel struct
mapping(bytes32 => Channel) internal _channels;

// --- Message Queue ---
// Slot M: mapping from (channelId, messageId) to MessageEntry
mapping(bytes32 => mapping(uint64 => MessageEntry)) internal _messageQueue;

// --- Connectors ---
// Slot P: mapping from (channelId, sourceConnectorHash) to Connector struct
...
// --- Endpoints ---
// Slot Q: mapping from (channelId, endpointAccount) to EndpointBond
mapping(bytes32 => mapping(address => EndpointBond)) internal _endpointBonds;

// --- Peer Endpoints ---
// Slot R: mapping from (channelId, accountHash) to PeerEndpoint
...
```solidity
/// @dev On-chain Channel state. Maps to Spec section 2.1.
///      Packed for gas efficiency: identity fields in slot 1, status+metadata in slot 2, etc.
struct Channel {
    // --- Identity (slots 1-3) ---
    // channelId is the mapping key, not stored here
    ...
    uint256 lastConfigTimestamp;        // Block timestamp of the last configuration propagated on this Channel
...
**Storage packing.** The `Channel` struct packs `status` (1 byte), `nextMessageId` (8 bytes), and
...
2. **ECDSA secp256k1 key (endpoint signing + Ethereum transactions)** -- Used for two purposes:
   ...
   `keccak256(channel_id || proof_bytes)`), and (b) Ethereum transaction signing (`msg.sender`).
...
> endpoint keys across all Channels.
...
```
registerEndpoint(channelId, endpoint) payable
  1. Require clprEnabled == true.
  2. Require _channels[channelId].status == ACTIVE.
  3. Decode endpoint; require endpoint.account_id == msg.sender (20 bytes).
  4. Require msg.value >= MIN_ENDPOINT_BOND.
  5. Require _endpointBonds[channelId][msg.sender].active == false (not already registered).
  6. Store EndpointBond { bondAmount: msg.value, registeredAt: block.number, active: true }.
  7. Emit EndpointRegistered(channelId, msg.sender, msg.value).
```

## 4.3 Deregistration Flow

```
deregisterEndpoint(channelId)
  1. Require _endpointBonds[channelId][msg.sender].active == true.
  ...
  6. Emit EndpointDeregistered(channelId, msg.sender, bondAmount).
```
...
2. Set _endpointBonds[channelId][offender].active = false.
3. Transfer slash proceeds to the protocol treasury.
4. Emit EndpointSlashed(channelId, offender, amount, reason).
```
...
```
submitBundle(channelId, proofBytes, remoteEndpointSignature, remoteEndpointPublicKey)
  // --- Step 0: Preconditions ---
  0a. Require clprEnabled == true.

  // --- Step 1: Verifier Call ---
  1. Load channel = _channels[channelId].
  2. Require channel.status == ACTIVE || channel.status == PAUSED
     || channel.status == CLOSING || channel.status == DRAINED.
     (PAUSED, CLOSING, and DRAINED all accept inbound bundles per Spec section 2.1.1.)
  3. Call channel.verifierContract.verifyBundle(proofBytes).
  ...
  6. Require messages.length <= channel.maxMessagesPerBundle.
  7. For each message, require payload.length <= channel.maxMessagePayloadBytes.

  // --- Step 3: Replay Defense ---
  8. Require messages[0].id == channel.receivedMessageId + 1.
  9. Require contiguous ascending IDs.
  10. Require messages[last].id == metadata.nextMessageId - 1.

  // --- Step 4: Running Hash Verification ---
  11. hash = channel.receivedRunningHash.
  12. For each message:
  ...
  14. Update channel.ackedMessageId = metadata.receivedMessageId.
  ...
  16a. If metadata.state is CLOSING or DRAINED, and channel.status == ACTIVE or PAUSED:
       - Set channel.status = CLOSING.
       - Emit ChannelStatusChanged(channelId, previousStatus, 3 /* CLOSING */).
  ...
  16b. If channel.lastConfigTimestamp < configTimestamp:
       - Enqueue a ConfigUpdate Control Message containing the current _ledgerConfiguration
         in the Channel's outbound queue.
       - Set channel.lastConfigTimestamp = configTimestamp.
  ...
  17. For each message in order:
      ...
      - If Data AND (channel.status == CLOSING || channel.status == DRAINED):
          Generate CHANNEL_CLOSED response without dispatching to application.
  ...
  18. Update channel.receivedMessageId and channel.receivedRunningHash.

  // --- Step 6a: Drain Check (two-phase) ---
  18a. If channel.status == CLOSING || channel.status == DRAINED:
       - If channel.status == CLOSING AND peer has acknowledged all our outbound messages:
           Set channel.status = DRAINED.
           Emit ChannelStatusChanged(channelId, 3 /* CLOSING */, 4 /* DRAINED */).
       - If channel.status == DRAINED AND peer's state == DRAINED:
           Set channel.status = CLOSED.
           Emit ChannelStatusChanged(channelId, 4 /* DRAINED */, 5 /* CLOSED */).

  // --- Step 7: Reimburse Submitter ---
  19. Transfer endpoint reimbursement from Connector charges to msg.sender.

  20. Emit BundleProcessed(channelId, firstId, lastId, msg.sender).
```

**Verification failures do not PAUSE the Channel.** If any of steps 1-4 (verifier call, bundle size
check, replay defense, running hash verification) fail, the transaction simply reverts. No Channel
state is modified, and the Channel remains in its current status (ACTIVE, PAUSED, CLOSING, or DRAINED). Only
...
| SLOAD channel metadata          | 2,100 (cold) + 100 (warm)  | ~3,000 for initial load            |
| SSTORE channel metadata update  | ~10,000                    | Warm stores for IDs and hashes     |
...
During `registerChannel`, the verifier contract's `EXTCODEHASH` is computed and stored on the Channel as
...
// Store as informational fingerprint on the Channel
```
...
the proxy's upgrade authority when deciding whether to trust a Channel (see cross-platform spec §8.8).
...
ensures no channel state is updated before the call (checks-effects-interactions). A global reentrancy
...
| 4th+       | 100% of remaining stake | Connector banned from Channel |
...
channel.receivedMessageId = messageId;
channel.receivedRunningHash = newHash;
connector.balance -= charge;

// External call with gas stipend
(bool success, bytes memory responseData) = targetApp.call{gas: channel.maxGasPerMessage}(
    abi.encodeCall(
        IClprApplication.onClprMessage,
        (channelId, message.sender, message.messageData)
    )
);
...
// Enqueue response
_enqueueResponse(channelId, messageId, replyStatus, responseData);
```
...
if (response.messageId != channel.nextResponseExpectedId) {
    ...
    if (channel.status == 1 /* ACTIVE */) {
        channel.status = 2; // PAUSED
        emit ChannelStatusChanged(channelId, 1 /* ACTIVE */, 2 /* PAUSED */);
    }
    // Revert regardless of status — CLOSING channels also reject out-of-order bundles.
...
// If admin closed the Channel while PAUSED (status is CLOSING), this block is skipped —
// the Channel stays CLOSING and processes bundles with CHANNEL_CLOSED responses.
if (channel.status == 2 /* PAUSED */) {
    channel.status = 1; // ACTIVE
    emit ChannelStatusChanged(channelId, 2 /* PAUSED */, 1 /* ACTIVE */);
}
...
if (targetApp.code.length > 0) {
    try IClprApplication(targetApp).onClprResponse(
        channelId, response.messageId, response.status, response.messageReplyData
    ) {} catch {
        emit ResponseDeliveryFailed(channelId, response.messageId);
    }
}

// Advance the expected response ID
channel.nextResponseExpectedId = _nextDataMessageId(channelId, response.messageId);

// Delete the matched Data Message from outbound queue
delete _messageQueue[channelId][response.messageId];
```
...
Applied to: `setLedgerConfiguration`, `registerChannel`, `completeChannel`,
`closeChannel`, `registerConnector`,
...
**PAUSED recovery.** PAUSED is auto-recoverable: the Channel resumes automatically when the next
bundle contains correctly ordered responses (see section 8.2), provided the admin hasn't closed it.
Admin may close a PAUSED Channel (`closeChannel` transitions it to CLOSING). While CLOSING,
bundles with out-of-order responses are still rejected; once the peer fixes the ordering, bundles
process with CHANNEL_CLOSED responses and queues drain normally through DRAINED to CLOSED.
...
2. **Fix deployment:** A new ClprService implementation is deployed that includes:
   ...
   - A migration function that corrects the corrupted Channel state (e.g., adjusting
...
4. **Recovery:** The migration function is called to restore the Channel to ACTIVE, and normal
   operations resume.

This is a concrete motivation for the upgradeability strategy described in section 1.2.

**CLOSING recovery.** CLOSING is irreversible. Once an admin calls `closeChannel`, the Channel
transitions to CLOSING. In-flight Data Messages receive CHANNEL_CLOSED responses without dispatch.
Acks continue flowing, queues drain, and the Channel transitions through DRAINED to CLOSED automatically
...
|------------------------------|------------------------|-------------------------------------|-----------------------------------------------------------|
| `registerChannel`            | 100,000 - 200,000      | Caller (permissionless)             | Commit phase only; no ECDSA verify. EXTCODEHASH recorded. |
| `completeChannel`            | 80,000 - 120,000       | Caller (permissionless)             | Reveal phase; ECDSA verify + state transition.            |
...
- **Channel metadata update:** ~10,000 gas per bundle (warm SSTORE for IDs and hashes).

**Storage growth bound:** With `maxQueueDepth = 1,000` messages per Channel and an average message payload
of 512 bytes, each Channel's queue occupies at most ~1,000 storage slots (~32 KB of storage). At 1,000
Channels, total queue storage is ~32 MB. This is manageable for Ethereum state, but aggressive cleanup
is essential.
...
- **Indexed parameters:** `channelId` is indexed on all events for per-Channel filtering.
...
|------------------------------|-------------------------------------------|---------------------|
| Messages sent on a Channel   | `MessageSent` filtered by channelId       | Topic 1 = channelId |
...
| All bundles on a Channel     | `BundleProcessed` filtered by channelId   | Topic 1 = channelId |
| Connector balance changes    | `ConnectorBalanceChanged`                 | Topic 1 = channelId |

## 11.3 Response Notification

Applications can learn about responses via two mechanisms:

1. **Event-based (recommended):** Subscribe to `ResponseDelivered` events filtered by the Channel and
...
3. **Pull-based:** Query Channel state via an off-chain indexer to check `receivedMessageId` and determine
...
**Spec says:** The signature scheme and exact signed payload format are platform-specific. The cross-platform
protocol requires that `keccak256(channel_id || public_key)` matches the stored ownership commitment.
The channel_id is an arbitrary 32-byte value chosen by the registrant, not derived from any key.

**Ethereum decision:** Use ECDSA secp256k1 (native `ecrecover` support). The signed payload for
`completeChannel` (the reveal phase) is:

```
signedPayload = keccak256(channelId)    // bytes32
```

The channel ID alone is sufficient — there is no replay risk since the channel ID is unique,
and simplifying the payload allows the same signature to work on both ledgers without
platform-specific payload construction.

Note: `registerChannel` (the commit phase) does NOT require a signature. The signature is only
verified during `completeChannel`.

Signature recovery uses `ecrecover` to derive the signer's address, then verifies
`keccak256(channelId || uncompressedPubkey)` matches the stored ownership commitment. This
requires the caller to provide the uncompressed public key alongside the signature for verification.
...
messages per Connector on a Channel is `maxQueueDepth / 2`, ensuring at least two Connectors can share
...
1. **Eager config propagation (was section 13.3.4).** The original spec used eager enqueue of ConfigUpdate
   on all Channels during `setLedgerConfiguration`, contradicting the cross-platform spec's lazy propagation
   mandate. **Fixed:** `setLedgerConfiguration` is now O(1); ConfigUpdate is lazily enqueued at each
   Channel's next interaction.
...
2. **Response ordering check after delivery (was section 8.2).** The original spec delivered the response
   ...
   misordered response could trigger application side effects before the Channel was PAUSED.
   **Fixed:** The ordering check now occurs BEFORE delivery. On mismatch, the Channel is PAUSED and no
   callback is made. The Channel auto-resumes when the peer corrects the ordering.
...
4. **Incomplete `clprEnabled` coverage.** The cross-platform spec §7 states "When disabled, all pseudo-API
   ...
   their first step. This includes: `registerChannel`, `registerConnector`,
   ...
   (`closeChannel`, `setLedgerConfiguration`) are exempt from the
...
**Impact:** On high-activity Channels, bundles compete for block space with all other Ethereum transactions.
...
Channels in parallel (each processing bundles independently) or adopt blob transactions for proof data.

### 13.2.2 Storage Costs for Message Queue

**Finding:** Storing message payloads on-chain is expensive (~25,000 gas per message). For a Channel
...
### 13.2.4 Signature Scheme for Channel Registration

**Finding:** The cross-platform spec requires that `keccak256(channel_id || public_key)` matches the
stored ownership commitment. The channel_id is an arbitrary 32-byte value, allowing different key types
on different ledgers. Ethereum uses ECDSA secp256k1, which maps naturally to the `ecrecover` precompile.
If post-quantum signature schemes are needed in the future, Ethereum would need precompile support for
the chosen scheme. No issues for current implementation.
...
same Channel simultaneously (possible on Ethereum where transactions are unordered in the mempool).
...
(enqueuing ConfigUpdate on every active Channel during `setLedgerConfiguration`) would be an O(N)
operation that could exceed the block gas limit with hundreds of Channels.

**Resolution:** `setLedgerConfiguration` is O(1). It stores the new configuration and records
`configTimestamp` (the current `block.timestamp`). ConfigUpdate Control Messages are lazily enqueued on each Channel at its
next interaction (`submitBundle` or `sendMessage`): if `channel.lastConfigTimestamp < configTimestamp`,
a ConfigUpdate is enqueued and `lastConfigTimestamp` is updated. This amortizes the enqueue cost across
subsequent interactions rather than concentrating it in a single transaction. No `channelIds` batching
parameter is needed.
...
|----------------------------------------|--------------|-----------|-----------------------------------------------|
| Channel registration                   | ~200,000     | Yes       | One-time cost; EXTCODEHASH check only         |
