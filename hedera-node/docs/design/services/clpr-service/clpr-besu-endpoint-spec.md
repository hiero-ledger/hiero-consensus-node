...
│  │  │  - Channel state, queues, endpoint bonds      │    │  │
...
| World State Trie | Read-only access to CLPR Service contract storage slots for queue metadata, endpoint rosters, and Channel state.                                        |
    ...
    clpr/
      src/main/java/org/hyperledger/besu/plugins/clpr/
        ...
        sync/
          ...
          ChannelSyncTask.java        # Per-Channel sync logic
        ...
        state/
          ...
          ChannelCache.java           # Caches Channel metadata
...
single-threaded scheduler per Channel.

Per cross-platform spec §2.1.1, Channel status determines behavior: PAUSED, CLOSING, and DRAINED Channels
continue syncing (PAUSED rejects inbound bundles with out-of-order responses; CLOSING and DRAINED drain in-flight
messages). Only CLOSED Channels are skipped entirely. ACTIVE Channels proceed normally. On Besu, the sync loop implementation is:

```
For each Channel C where this node is a registered endpoint:
...
The endpoint selects a peer from the Channel's endpoint roster. The selection strategy balances reliability with
Sybil resistance:
...
- Each Channel gets its own sync task running on a `ScheduledExecutorService`.
- Sync tasks for different Channels run **concurrently** — they are independent and share no state.
- Within a single Channel, syncs are **serialized** — a new sync does not start until the previous one completes
...
## 4.4 Multiple Channel Handling

A single Besu node may be registered as an endpoint on multiple Channels (e.g., Channels to different peer
ledgers, or multiple Channels to the same peer with different commitment levels). Each Channel is handled
independently:
...
- Separate metrics (tagged by `channel_id`).
...
formula per Channel using the `ScheduledExecutorService` delay.

If `max_syncs_per_sec` is not yet known (newly registered Channel, no `ConfigUpdate` received yet), the
endpoint uses a conservative default of 1 sync per second per endpoint (configurable via `clpr.default-syncs-per-sec`).

**Lazy config propagation awareness.** The endpoint reads peer throttle values from Channel state (via `eth_call`)
...
When this Besu node initiates or responds to a sync for a Channel where Ethereum is the source ledger, it must
...
The proof must establish, with cryptographic assurance rooted in Ethereum's consensus, the Channel's queue
...
Select the block to prove against. The choice depends on the Channel's commitment level:
...
The commitment level is a property of the verifier contract deployed for the Channel. The endpoint MUST know which
commitment level to use (configured per Channel or read from contract metadata).

**Recommendation:** Default to `finalized` for safety. Support `safe` and `latest` as opt-in configurations per
Channel for lower-latency use cases that accept reorg risk.
...
- The Channel struct fields (queue metadata).
...
1. Reads the current state at the latest block meeting the Channel's commitment level.
2. Constructs proof_bytes as described above.
3. Signs `(channel_id || proof_bytes)` with the endpoint's signing key.
...
The CLPR Service contract stores Channel state and message queue entries in Solidity mappings. The storage slot
...
**Example:** To read a Channel's `next_message_id` where channels are stored in
`mapping(bytes32 => Channel)` at slot 3, and `next_message_id` is the 5th field (offset 4):

```
base = keccak256(channel_id . uint256(3))
slot = base + 4
```
...
1. **Deserialize** the `ClprSyncPayload` and extract `channel_id`, `proof_bytes`, `endpoint_signature`.
...
data: abi.encode(channel_id, proof_bytes, remote_endpoint_signature)
```
...
When the endpoint is registered on multiple Channels, all Channels share the same `NonceTracker` (since they
...
2. **Handle revert.** If the transaction is included but reverted (status = 0), log the revert reason. Common
   ...
   - Channel status changed: paused, closing, or closed (spec §2.1.1, §4.5) between estimation and inclusion.

   Per spec §4.2, bundle verification failures (bad hash chain, replay, oversized payload) cause a simple revert
   with no state change. Response ordering violations (spec §4.5) transition the Channel to PAUSED (auto-recoverable
   when the peer fixes ordering). On Besu, the endpoint SHOULD perform a pre-submission `eth_call` read of Channel
   status to avoid paying gas for a guaranteed revert against a CLOSED Channel.
...
- The CLPR Service contract address and all storage slots that will be read/written (Channel metadata, queue
...
posting a bond. The Channel itself must already exist on-chain (Channel registration is an administrative
...
```solidity
/// @notice Register as an endpoint on a Channel, posting a bond.
/// @param channelId The Channel to register on.
...
If `clpr.auto-register = true`, the plugin automatically registers on startup for all configured Channels. The
plugin:
...
channels = [
    "0xabc123...",  # Channel IDs to register on
    "0xdef456..."
]
```
...
```solidity
function deregisterEndpoint(bytes32 channel_id) external;
```

The plugin provides a CLI command:

```bash
besu --clpr-deregister-endpoint --clpr-channel-id=0x...
```
...
```solidity
function endpointBond(bytes32 channel_id, address endpoint) external view returns (uint256);
```
...
Channel conn = clprContract.getChannel(channelId);

// Read endpoint roster
ClprEndpoint[] peers = clprContract.getPeerEndpoints(channelId);

// Read queue depth
QueueDepth depth = clprContract.getQueueDepth(channelId);
```
...
|------------------------------------------|--------------|----------------------------------------------------------|
| Channel metadata (queue metadata)        | 0 (no cache) | Read fresh before every sync                             |
...
2. **Pre-check before submission.** Immediately before submitting, the endpoint re-reads the Channel's
...
| Channel IDs       | `--clpr-channels`              | `clpr.channels`              | `[]`                        | List of Channel IDs to participate in.                                                        |

> **Channel discovery note:** The static `clpr.channels` configuration is authoritative — the endpoint ONLY
> serves Channels explicitly listed in its configuration. If a third party registers this endpoint on a new
> Channel, the endpoint will not automatically discover or serve it. Operators must update the configuration and
> restart (or trigger a config reload) to serve new Channels. This is a deliberate security choice: endpoints
> should explicitly opt in to serving Channels rather than automatically serving any Channel that references
> them.
...
## 11.2 Channel-Specific Overrides

Some parameters may be overridden per Channel:

```toml
[clpr.channel-overrides."0xabc123..."]
commitment-level = "safe"
max-gas-price-gwei = 50
```

This allows operators to use different commitment levels or gas tolerances for different Channels (e.g., lower
latency for a `safe`-level Channel to a high-throughput peer, higher gas tolerance for a critical Channel).
The sync frequency is NOT overridable per Channel — it is always derived from the peer's `max_syncs_per_sec`
...
5. **Registration verified.** The plugin checks whether the endpoint is registered on each configured Channel.
...
6. **Sync committee initialized.** The `SyncCommitteeTracker` fetches the current and next sync committees from the
   Beacon API.
7. **gRPC server started.** The gRPC server begins accepting incoming sync channels.
8. **Sync loops started.** A sync scheduler is created for each registered Channel.
...
1. **Sync loops stopped.** All sync schedulers are cancelled. In-progress syncs are allowed to complete (with a
   timeout).
2. **gRPC server stopped.** The gRPC server stops accepting new channels. Existing channels are gracefully
   drained.
...
remotely initiated inbound syncs per `(channel_id, remote_endpoint_account_id)` pair. If a peer exceeds the
...
All metrics are registered under the `clpr` subsystem in Besu's Prometheus metrics. Labels include `channel_id`
...
|------------------------------|-----------|---------------------------|-----------------------------------------------------------------|
| `clpr_syncs_initiated_total` | Counter   | `channel_id`, `status`    | Total syncs initiated. Status: `success`, `failure`, `timeout`. |
| `clpr_syncs_received_total`  | Counter   | `channel_id`, `status`    | Total syncs received (as responder).                            |
| `clpr_sync_duration_seconds` | Histogram | `channel_id`, `role`      | Duration of sync calls. Role: `initiator`, `responder`.         |
| `clpr_sync_payload_bytes`    | Histogram | `channel_id`, `direction` | Size of sync payloads. Direction: `sent`, `received`.           |

### Transaction Metrics

|               Metric                |   Type    |         Labels         |                             Description                             |
|-------------------------------------|-----------|------------------------|---------------------------------------------------------------------|
| `clpr_transactions_submitted_total` | Counter   | `channel_id`, `status` | Transactions submitted. Status: `confirmed`, `reverted`, `dropped`. |
| `clpr_transaction_gas_used`         | Histogram | `channel_id`           | Gas used by confirmed `submitBundle()` transactions.                |
| `clpr_transaction_gas_price_gwei`   | Gauge     | `channel_id`           | Effective gas price of the latest confirmed transaction.            |
| `clpr_transaction_inclusion_blocks` | Histogram | `channel_id`           | Blocks between submission and inclusion.                            |

### Proof Metrics

|              Metric               |   Type    |         Labels         |                            Description                            |
|-----------------------------------|-----------|------------------------|-------------------------------------------------------------------|
| `clpr_proof_construction_seconds` | Histogram | `channel_id`           | Time to construct proof_bytes.                                    |
| `clpr_proof_storage_slots`        | Histogram | `channel_id`           | Number of storage slots included in a proof.                      |
| `clpr_proof_validation_seconds`   | Histogram | `channel_id`, `result` | Time to pre-validate received proofs. Result: `valid`, `invalid`. |

### State Metrics

|             Metric              |  Type   |          Labels           |                        Description                         |
|---------------------------------|---------|---------------------------|------------------------------------------------------------|
| `clpr_queue_depth`              | Gauge   | `channel_id`, `direction` | Current queue depth. Direction: `outbound`, `inbound`.     |
| `clpr_messages_processed_total` | Counter | `channel_id`, `type`      | Messages processed by type: `data`, `response`, `control`. |
| `clpr_acked_message_id`         | Gauge   | `channel_id`              | Current acked_message_id per Channel.                      |

### Bond and Economic Metrics

|                 Metric                  |  Type   |    Labels    |                   Description                   |
|-----------------------------------------|---------|--------------|-------------------------------------------------|
| `clpr_endpoint_bond_wei`                | Gauge   | `channel_id` | Current endpoint bond balance.                  |
| `clpr_endpoint_reimbursement_wei_total` | Counter | `channel_id` | Cumulative reimbursement received.              |
| `clpr_transaction_cost_wei_total`       | Counter | `channel_id` | Cumulative gas costs for bundle submissions.    |
| `clpr_net_profit_wei`                   | Gauge   | `channel_id` | Reimbursement minus gas costs (rolling window). |

### Peer Health Metrics

|             Metric              |   Type    |        Labels        |            Description            |
|---------------------------------|-----------|----------------------|-----------------------------------|
| `clpr_peer_endpoints_known`     | Gauge     | `channel_id`         | Number of known peer endpoints.   |
| `clpr_peer_sync_failures_total` | Counter   | `channel_id`, `peer` | Sync failures per peer endpoint.  |
| `clpr_peer_latency_seconds`     | Histogram | `channel_id`, `peer` | gRPC round-trip latency per peer. |
...
1. **Rate limiting.** Limit incoming channels per IP address (default: 10 concurrent, 30 per minute). Configurable
...
4. **Deadline enforcement.** All gRPC calls have a server-side deadline (default: 60 seconds). Long-running calls are
   cancelled.
5. **IP allowlisting (optional).** The operator can restrict incoming channels to known peer endpoint IP addresses
   (read from the on-chain roster). This provides strong DDoS protection but breaks the ability to accept channels
...
provides a valid bundle proof (verified on-chain by the Channel's verifier contract) is legitimate regardless
...
- **Profitable cases:** High-value Channels with few competing endpoints, where the Connector margin is calibrated
  to cover gas costs with a healthy buffer.
- **Break-even cases:** Many competing endpoints on a popular Channel, where competition drives down effective
...
The protocol's economic model assumes rational endpoints that will exit unprofitable Channels, finding an
equilibrium where the number of active endpoints balances with the available margin. This equilibrium may take time
to develop and may result in Channels with only one or two active endpoints — which is acceptable for liveness
...
2. **Multi-Channel nonce contention.** When the endpoint is registered on many Channels, all sharing one
   signing account, nonce contention becomes a bottleneck. Each Channel's sync may want to submit a transaction
   simultaneously, but nonces must be strictly sequential. Should the plugin batch multiple Channel bundles into
