...
mutations. CLPR extends this with new transaction types (`registerChannel`, `sendMessage`, `submitBundle`,
...
mirror node data (channel state, queue metadata, message history, Connector balances).
...
| Cross-ledger integration  | Two or more networks (Hiero, Ethereum, etc.) via SOLO/K8s | End-to-end message flow, cross-ledger state consistency, channel lifecycle under traffic | Slow (minutes to hours) |
...
- **Hub-and-spoke** (A↔B, A↔C, A↔D) — tests Channel isolation and multi-peer behavior.
- **Ring** (A↔B, B↔C, C↔A) — tests multi-ledger atomic swap workflows.
- **Mesh** — stress tests with many simultaneous cross-channels.

## 2.2 Cross-Network Test Driver

A test driver that holds simultaneous channels to all networks under test and can orchestrate transactions
...
state (channel status, queue depth, message state), query application state (balances, escrow, contract
...
"message sent on A with ID N produced response on A with status SUCCESS," "channel status on A is ACTIVE
AND channel status on B is ACTIVE." The assertion API must accept ledger identifiers and query the
appropriate adapter.
...
### 3.1.1 `updateLedgerConfiguration`

- Admin key is required; unauthorized callers are rejected.
- `chain_id` and `protocol_version` are immutable (set at genesis from configuration properties);
  caller-supplied values for these fields are ignored.
...
- Configuration update triggers lazy propagation: each Channel's `last_config_timestamp` is compared
...
### 3.2.1 `registerChannel` (Commit Phase)

- Channel ID must be exactly 32 bytes.
- Verifier contract must be a deployed smart contract. If it does not exist, registration fails.
...
- Initial Channel status is **PENDING** (not ACTIVE). Queue metadata is initialized: `next_message_id = 1`,
  `acked_message_id = 0`, both running hashes are 32 bytes of zeros.
- Registration (commit phase) is permissionless — no special key required, no ECDSA signature.
- The verifier is immutable after registration; its address and code fingerprint are stored on the Channel.
- A PENDING Channel cannot be used for messaging (`sendMessage` and `submitBundle` are rejected).

### 3.2.1a `completeChannel` (Reveal Phase)

- Channel must exist and be in PENDING state.
- Handler verifies `keccak256(channel_id || public_key)` matches the stored ownership commitment.
  The channel_id is an arbitrary 32-byte value chosen by the registrant.
  The public key encoding is platform-specific (e.g., 64-byte uncompressed x||y for ECDSA secp256k1).
- Signature is verified over the completion data. Invalid signatures are rejected. The signature scheme
  is platform-specific (e.g., ECDSA secp256k1 on Hiero and Ethereum).
- Transitions the Channel from PENDING to ACTIVE.
- Completion is permissionless — no special key required (beyond the ECDSA proof of key ownership).

### 3.2.1b `closeChannel` on PENDING Channels

- Admin can call `closeChannel` on a PENDING Channel to clean up abandoned claims.
- The PENDING Channel is deleted immediately (no drain phase needed).
- Non-admin callers cannot close a PENDING Channel.

### 3.2.2 Channel Status Transitions
...
- ACTIVE → CLOSING via `closeChannel` (admin key required).
...
- PAUSED → CLOSING via `closeChannel` (admin key required). The Channel drains once the peer
...
- Admin MAY close a PAUSED Channel (`closeChannel` transitions to CLOSING). The Channel cannot
  drain until the peer fixes the ordering; once fixed, bundles process with `CHANNEL_CLOSED` responses
  and queues drain normally.

### 3.2.4 CLOSING Behavior

- No new outbound messages are accepted (`sendMessage` is rejected).
- In-flight Data Messages receive `CHANNEL_CLOSED` responses without dispatch. No slashing occurs.
- Acknowledgements continue flowing. Queues drain.
- The `ClprQueueMetadata.state` reflects the Channel status and is propagated to the peer via the next sync.
  The peer transitions its side to CLOSING as well.
- When the peer has acknowledged all outbound messages, the Channel transitions to `DRAINED`.
...
- `closeChannel` on a CLOSING, DRAINED, or CLOSED channel is rejected.
- `closeChannel` on an ACTIVE or PAUSED channel succeeds (transitions to CLOSING).
...
- Cross-chain mapping `(channel_id, source_connector_address) → local_connector` is established.
...
- Message IDs are per-Channel (independent counters across Channels).
...
- The CLPR Service calls the Channel's verifier contract with the proof bytes from the submitted payload.
...
- After a valid bundle is processed, the Channel's `received_message_id` and `received_running_hash`
...
- If the Channel's `last_config_timestamp` is less than the current configuration's
  `consensus_timestamp`, a ConfigUpdate Control Message is enqueued on the Channel's outbound queue
...
- If a response does not match the next expected Data Message, the Channel automatically transitions to
  PAUSED. The Channel auto-resumes when the next bundle contains correctly ordered responses.
...
- If the Connector's locked stake is exhausted, the Connector is banned from the Channel.
...
- If `max_sync_bytes` is configured too low (less than `max_message_payload_bytes` plus protocol overhead),
  no valid bundle can be constructed — the Channel is effectively deadlocked. This configuration should
  be prevented by throttle validation in `updateLedgerConfiguration`.
...
## 3.16 Multiple Channels Sharing a Verifier

### 3.16.1 Same Verifier on Multiple Channels

- Two Channels may reference the same verifier contract address. Each Channel maintains independent
...
### 4.1.3 Channel Lifecycle Under Traffic
...
- Auto-resume: submit a bundle with correctly ordered responses. Verify the Channel transitions back
to ACTIVE and normal operation resumes.
- Close the Channel via `closeChannel`. Verify the Channel transitions to CLOSING, the
`ClprQueueMetadata.state` carries the Channel status and propagates to the peer, in-flight messages
receive `CHANNEL_CLOSED` responses without dispatch, the Channel transitions to DRAINED when the
peer acks all outbound messages, and the Channel reaches CLOSED when both sides are DRAINED.
...
With multiple active Channels from Ledger A to different peers, verify that the ConfigUpdate is enqueued
on every active Channel (not just the one that happened to interact next).
...
### 4.1.6 Multiple Channels Between the Same Pair

Register two Channels between A and B with different verifiers (e.g., different commitment levels).
Verify that each Channel maintains independent queue state, independent message ID sequences, and
independent Connector registrations.

### 4.1.7 Multiple Connectors on the Same Channel

Register two Connectors on the same Channel. Send messages through each. Verify that slashing one
...
Measure and record Ethereum gas costs for: `registerChannel`, `registerConnector`, `sendMessage`,
`submitBundle`, `closeChannel`. Verify that costs are within acceptable bounds for production deployment.
...
Deploy the CLPR Service behind an upgradeable proxy. Perform a proxy upgrade while the Channel is active
with in-flight messages. Verify that the upgrade does not corrupt Channel state, queue metadata, or
Connector balances.
...
Deploy three networks: Hiero as hub, Ethereum and a second Hiero as spokes. Register Channels
Hub↔Spoke1 and Hub↔Spoke2. Send messages on both Channels simultaneously. Verify that traffic on one
Channel does not affect the other — independent queues, independent Connectors, independent throughput.

### 4.4.2 Triangle Topology

Deploy three networks with Channels A↔B, B↔C, and C↔A. Send messages around the ring. Verify each
Channel operates independently. This topology is the foundation for multi-ledger atomic swap testing
(Section 6.4).

### 4.4.3 Channel Isolation

In any multi-channel topology, verify that pausing, closing, or overloading one Channel has no effect
on other Channels — even when they share endpoints or Connectors.
...
transaction cost, the Channel remains ACTIVE (no state change from a rejected bundle), and the peer can
...
the endpoint pays the transaction cost, and the Channel remains ACTIVE.
...
automatically transitions the Channel to PAUSED.

### 5.4.2 PAUSED Channel Behavior
...
Channel automatically transitions back to ACTIVE and normal operation continues — messages can be
enqueued, bundles can be submitted, and responses are correctly ordered. No admin intervention is required.

### 5.4.4 Admin Closes a PAUSED Channel

While a Channel is PAUSED, call `closeChannel`. Verify the Channel transitions to CLOSING. Submit
...
`CHANNEL_CLOSED` responses (since the Channel is CLOSING), and the Channel drains through
DRAINED → CLOSED normally.

### 5.4.5 CLOSING Propagation via Metadata

Call `closeChannel` on an ACTIVE Channel. Verify: the Channel transitions to CLOSING, the
`ClprQueueMetadata.state` carries the Channel status, the peer sees the state during the next sync and
transitions its side to CLOSING as well. When the peer acknowledges all outbound messages, the Channel
transitions to DRAINED. When both sides are DRAINED, the Channel transitions to CLOSED.

### 5.4.6 In-Flight Message Resolution During CLOSING

While a Channel is CLOSING, Data Messages that arrive from the peer (sent before the peer knew about the
close) receive `CHANNEL_CLOSED` responses without dispatch. Verify: no Connector is charged, no slashing
occurs, and the response is returned to the source.

### 5.4.7 Connector Deregistration After Close

After a Channel transitions through CLOSING and DRAINED to CLOSED, verify that Connectors with no remaining in-flight
...
response without corrupting Channel state).
...
### 6.3.3 Channel Closure During Escrow

Close the Channel while assets are escrowed and the response has not arrived. Verify the application
detects the Channel closure, handles the ambiguous state, and provides a reconciliation path (e.g.,
...
Three ledgers (A, B, C) with Channels A↔B, B↔C, C↔A. Party on A sends token X to B, party on B
...
Multiple users execute trades in parallel across the same Channel. Verify that all trades settle
...
### 7.1.1 Single Channel Saturation

Send messages at increasing rates through a single Channel with a single Connector until the queue
reaches `max_queue_depth` or the system can no longer keep up. Record the maximum sustained message rate.

### 7.1.2 Multi-Channel Throughput

Send messages across multiple Channels simultaneously. Verify that per-Channel throughput is
independent and that total system throughput scales with the number of active Channels.
...
Measure the growth of CLPR state (Channel records, message store, Connector balances) over extended
...
### 8.2.1 New Channel with New Verifier

While the old Channel is active, register a new Channel with a verifier that supports the new proof
format. Verify the new Channel is independently operational.

### 8.2.2 Application Migration

Migrate application traffic from the old Channel to the new one. Verify that messages sent on the new
Channel are delivered and responses return correctly.

### 8.2.3 Old Channel Closure

Close the old Channel after traffic has been migrated. Verify the closure is clean and does not affect
the new Channel.

### 8.2.4 Combined Rotation and Format Change

Simultaneously rotate endpoints AND change the proof format. Verify that a new Channel with a new
verifier and new endpoints is operational.

## 8.3 Verifier Compromise (R5)

### 8.3.1 Close Affected Channel

Admin closes a Channel whose verifier is suspected of being compromised via `closeChannel`. Verify
the Channel transitions to CLOSING, in-flight messages receive `CHANNEL_CLOSED` responses, queues
drain through DRAINED, and the Channel reaches CLOSED.

### 8.3.2 Migrate to Patched Verifier

Register a new Channel with a patched verifier. Verify applications can migrate and the new Channel
is operational.

### 8.3.3 Verify Compromised Channel Reaches Terminal State

After the compromised Channel drains through CLOSING and DRAINED, verify it reaches CLOSED and no further
...
Inject an out-of-order response from the peer. Verify the Channel automatically transitions to PAUSED.

### 8.4.2 Auto-Resume After Fix

After the peer fixes the ordering bug, submit a bundle with correctly ordered responses. Verify the
Channel automatically transitions back to ACTIVE and subsequent responses are correctly ordered. No
admin intervention is required.

### 8.4.3 Persistent PAUSE When Peer Never Fixes

If the peer never fixes the response ordering, the Channel remains PAUSED indefinitely. Verify: syncs
continue both directions, and no new messages are accepted. The admin may close the PAUSED Channel
(transitions to CLOSING), but the Channel cannot drain until the peer fixes the ordering. While CLOSING
...
restart, Channels are intact, queue metadata is preserved, and syncs resume after the restart.
...
| §3.1.3 Channels              | Registration, lifecycle, status machine                       | 3.2.1, 3.2.2, 3.2.3, 3.2.4, 3.2.5, 3.2.6, 3.16.1 | 4.1.3, 4.1.6        | 5.4.4, 5.4.5, 5.4.6, 5.4.7 | 6.3.3             | —                   | 8.2.1, 8.3.1, 8.3.3 |
...
| Config propagation           | Lazy enqueue, timestamp, multi-Channel                        | 3.5.6                                            | 4.1.4               | —                          | —                 | —                   | —                   |
