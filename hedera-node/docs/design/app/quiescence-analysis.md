...
| `QuiescenceCommands`                    | `hedera-app/quiescence`              | Owns the single canonical `lastCommand`; dispatches commands to the platform. All poll sites route transitions through `update(...)` (CAS, last-writer-wins). Resets on reconnect. |
...
| `QuiescenceConfig`                      | `hedera-config`                      | `enabled` (default false), `tctDuration` (default 5s — gap before TCT when node must wake), and `gracePeriod` (default 5s — min observed inactivity before reporting `QUIESCE`).   |
...
1. Network is processing transactions normally; the canonical `lastCommand` in `QuiescenceCommands` is `DONT_QUIESCE`. Each ingested transaction increments `preFlight`/`inFlight`; pre-handle increments `pipelineTransactionCount`; `inProgressBlockTransaction` accumulates per-block; `blockFullySigned` subtracts the block's relevant count back out of `pipelineTransactionCount` and updates `nextTct` based on the block's `maxConsensusTime`.
2. Traffic drops. All in-flight transactions land, all blocks fully sign, `pipelineTransactionCount` returns to zero, `pendingTransactionCount` returns to zero. On the next block-signing callback `getQuiescenceStatus()` returns `QUIESCE`. `BlockStreamManagerImpl` routes the command through `QuiescenceCommands.update(...)`, which CAS-updates the canonical `lastCommand` from `DONT_QUIESCE` to `QUIESCE` and emits it to the platform; the manager then starts `QuiescedHeartbeat`.
...
- `TctProbeTest` — exhaustive on TCT-source priority, including the no-deadline contract: `findTctReturnsNullWhenNoStakePeriodAndNoScheduledTransactions` asserts the probe returns `null` (not an `Instant.EPOCH` sentinel) when there is no freeze, no positive stake period, and no scheduled transaction, and `findTctWithFreezeTimeAndNoStakePeriod` asserts the freeze time itself is returned rather than EPOCH.
