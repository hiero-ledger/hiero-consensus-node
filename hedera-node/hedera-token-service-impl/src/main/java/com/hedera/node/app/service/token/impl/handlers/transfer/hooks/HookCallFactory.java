...
public class HookCallFactory {
    ...
    public HookCalls from(HandleContext handleContext,
    ...
    private HookCalls getProposedTransfers(final CryptoTransferTransactionBody userTxn,
        ...
        return new HookCalls(new HookContext(Tuple.of(directTransfers, emptyTransfers), memo, txnFee), preOnly, prePost); // ((TransferList,TokenTransferList[]),(TransferList,TokenTransferList[]))
    }
    ...
    private Tuple[] encodeNftTransfers(final List<NftTransfer> nftTransfers,
        ...
        return nftTransfers.stream().map(nft -> {
            ...
            if (nft.hasPreTxSenderAllowanceHook()) {
                ...
                preOnly.add(new HookInvocation(nft.senderAccountID(),
                        hook.hookIdOrThrow(),
                        sender,
            ...
            if (nft.hasPrePostTxSenderAllowanceHook()) {
                ...
                prePost.add(new HookInvocation(nft.senderAccountID(),
                        hook.hookIdOrThrow(),
                        sender,
                        hook.evmHookCallOrThrow().data(),
                        hook.evmHookCallOrThrow().gasLimit()));
            ...
            if (nft.hasPreTxReceiverAllowanceHook()) {
                ...
                preOnly.add(new HookInvocation(nft.receiverAccountID(),
                        hook.hookIdOrThrow(),
                        receiver,
                        hook.evmHookCallOrThrow().data(),
                        hook.evmHookCallOrThrow().gasLimit()));
            ...
            if (nft.hasPrePostTxReceiverAllowanceHook()) {
                ...
                prePost.add(new HookInvocation(nft.receiverAccountID(),
                        hook.hookIdOrThrow(),
                        receiver,
                        hook.evmHookCallOrThrow().data(),
                        hook.evmHookCallOrThrow().gasLimit()));
    ...
    private Tuple[] encodeAccountAmounts(List<AccountAmount> items,
                                         ReadableAccountStore accountStore,
                                         List<HookInvocation> preOnly,
                                         List<HookInvocation> prePost) {
        return items.stream().map(aa -> {
            ...
            if (aa.hasPreTxAllowanceHook()) {
                ...
                preOnly.add(new HookInvocation(aa.accountID(),
                        hook.hookIdOrThrow(),
                        address,
                        hook.evmHookCallOrThrow().data(),
                        hook.evmHookCallOrThrow().gasLimit()));
            ...
            if (aa.hasPrePostTxAllowanceHook()) {
                ...
                prePost.add(new HookInvocation(aa.accountID(),
                        hook.hookIdOrThrow(),
                        address,
                        hook.evmHookCallOrThrow().data(),
                        hook.evmHookCallOrThrow().gasLimit()));
